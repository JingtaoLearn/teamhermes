"""Centralized logging setup for TeamHermes Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.teamhermes/logs/`` (profile-aware via ``get_hermes_home()``).
...
# Used by _ComponentFilter and exposed for ``th logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the TeamHermes logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the TeamHermes home directory.  Falls back to
