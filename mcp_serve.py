"""
TeamHermes MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (TeamHermes-specific extra)

Usage:
    th mcp serve
    th mcp serve --verbose
...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        return Path(os.environ.get("HERMES_HOME", Path.home() / ".teamhermes")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("HERMES_HOME", Path.home() / ".teamhermes")
...
class EventBridge:
    ...
    This is the TeamHermes equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("HERMES_HOME", Path.home() / ".teamhermes")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the TeamHermes MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "TeamHermes Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the TeamHermes MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
