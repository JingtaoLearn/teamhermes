/**
 * ChatPage — embeds `thm --tui` inside the dashboard.
 *
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__HERMES_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `thm dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner TeamHermes TUI has enabled xterm mouse-events
    ...
    // forwarding them into TeamHermes. Keyboard input, paste, and resize still
    // behave normally.
