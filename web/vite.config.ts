...
/**
 * In production the Python `th dashboard` server injects a one-shot
      ...
      try {
        ...
        if (!match) {
          console.warn(
            `[hermes] Could not find session token in ${BACKEND} — ` +
              `is \`th dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        console.warn(
          `[hermes] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`th dashboard\` or set HERMES_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      // Same host as `th dashboard` must serve these; Vite has no
