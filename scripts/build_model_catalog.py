#!/usr/bin/env python3
"""Build the TeamHermes Model Catalog — a centralized JSON manifest of curated models.
...
TeamHermes CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a TeamHermes release.
...
``https://thm-agent.nousresearch.com/docs/api/model-catalog.json``
"""
...
os.environ.setdefault("HERMES_HOME", os.path.join(os.path.expanduser("~"), ".teamhermes"))
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "thm-agent repo",
            "docs": "https://thm-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
