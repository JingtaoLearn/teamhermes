...
#   * Proper venv activation (probes .venv, venv, then ~/.teamhermes/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.teamhermes/hermes-agent/venv"; do
...
if [ -f "$HOME/.teamhermes/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.teamhermes"
