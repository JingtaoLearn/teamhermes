...
# Bootstrap Open WebUI against TeamHermes Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.teamhermes/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-thm.sh
...
#   OPEN_WEBUI_NAME='Johnny TeamHermes'
...
#   HERMES_API_MODEL_NAME='TeamHermes Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-TeamHermes Agent WebUI}"
...
HERMES_ENV_FILE="${HERMES_ENV_FILE:-$HOME/.teamhermes/.env}"
...
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-thm.sh"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.thm.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.thm</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.thm"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.thm"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-thm.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to TeamHermes Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-thm.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.teamhermes/logs/openwebui.log
StandardError=append:%h/.teamhermes/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-thm.service
}
...
main() {
  ...
  log 'Ensuring TeamHermes API server is configured...'
  ...
  thm gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    ...
    nohup thm gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  ...
  log "TeamHermes API endpoint: ${HERMES_API_BASE_URL}"
