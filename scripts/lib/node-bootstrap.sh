...
#   2. ~/.teamhermes/node/ from a prior TeamHermes-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.teamhermes/node/ (always works, zero shell rc edits)
#
# Usage:
...
#   HERMES_HOME               (default: $HOME/.teamhermes)
# ============================================================================
...
ensure_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ]; then
        export PATH="$HERMES_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (TeamHermes-managed)"
            HERMES_NODE_AVAILABLE=true
            return 0
        fi
    fi
