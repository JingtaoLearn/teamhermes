#!/usr/bin/env python3
"""TeamHermes Agent Release Script
...
AUTHOR_MAP = {
    "xiaozhang@openclaw.ai": "jingtaozhang18",
    "zhang@teamhermes.dev": "jingtaozhang18",
    ...
    "274096618+thm-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local TeamHermes runtime files)
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/hermes-agent",
    ...
    lines.append(f"# TeamHermes Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for TeamHermes Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="TeamHermes Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  TeamHermes Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"TeamHermes Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"TeamHermes Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'TeamHermes Agent v{new_version} ({calver_date})' "
