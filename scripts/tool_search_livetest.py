#!/usr/bin/env python3
"""Live test harness for TeamHermes Agent's Tool Search feature.
...
ORIGINAL_AUTH = Path.home() / ".teamhermes" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.teamhermes/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.teamhermes/.env`` so
    ...
    hermes_home = home_dir / ".teamhermes"
    ...
    real_env_file = Path.home() / ".teamhermes" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".teamhermes"))
