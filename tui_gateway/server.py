...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `th` shows the prompt before constructing AIAgent; the TUI used
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `th` is interactive — use `/th chat -q …` or run `th` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`th setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`th gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`th sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`th config edit` needs $EDITOR in a real terminal"
    return None
