...
The bootstrap module is imported at the top of every TeamHermes entry point
(hermes, hermes-agent, thm-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every TeamHermes entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the TeamHermes CLI
...
class TestEntryPointsImportBootstrap:
    """Every TeamHermes entry point must import hermes_bootstrap as its
    ...
    # Entry points that invoke TeamHermes as a process.  Each one must
    ...
    ENTRY_POINTS = [
        ...
        "acp_adapter/entry.py",  # thm-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``thm update`` (git-reset landed new code but
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # thm start even when hermes_bootstrap hasn't been
