"""Regression test: ``th dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``th dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$HERMES_GIT_SHA`` build-arg to ``/opt/hermes/.teamhermes_build_sha`` and
...
is absent and ``th dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/hermes/.teamhermes_build_sha`` exists in the image, assert that
  ``th dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/hermes/.teamhermes_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/hermes/.teamhermes_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    ...
    # CI path: build-arg was set, baked file exists.  ``th dump``
