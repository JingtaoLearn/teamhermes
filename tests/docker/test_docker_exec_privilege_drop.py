...
The shim (docker/hermes-exec-shim.sh, installed at /opt/hermes/bin/th)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> th login` would write /opt/data/auth.json as
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    We invoke `th` with a Python-style `-c` shim equivalent — there's no
    ...
    fact that the venv's `th` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `th`
    do anything that writes to disk, then check the file's owner.

    Use `th config set` which writes config.yaml under HERMES_HOME.
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    invokes `s6-setuidgid th hermes <args>` which resolves `th`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> th login`` (root) wrote
    ...
    for it by writing the same file shape via `th config set`-style
    ...
    Specifically: pretend the operator ran `th login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the hermes UID. We use `th auth list` since that touches the
    ...
    # `th auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `th config set` first to
