...
Every ``docker exec`` runs as ``thm`` per the conftest module
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    a poor user experience: the iconic "TeamHermes Gateway Starting…"
    ...
    assert "⚕" in combined or "TeamHermes Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "TeamHermes Gateway Starting" in file_contents, (
