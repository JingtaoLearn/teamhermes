"""Tests for ``th dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``th update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No th dashboard processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 th dashboard process(es) running" in out
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``th dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``th dashboard --stop`` / ``--status`` actually parse."""
