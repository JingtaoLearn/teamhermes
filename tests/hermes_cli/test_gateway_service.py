...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "th gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`th gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `th gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `th gateway restart`)."""
        unit_path = tmp_path / "hermes-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "th gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_hermes_home_for_target_user", lambda home: "/home/alice/.teamhermes")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.teamhermes.gateway.plist"
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: th gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup th gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.teamhermes.gateway"])
            ),
        )
...
class TestSystemUnitHermesHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'HERMES_HOME=/home/alice/.teamhermes' in unit
        assert '/root/.teamhermes' not in unit
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HERMES_HOME", "/root/.teamhermes/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HERMES_HOME=/home/alice/.teamhermes/profiles/coder' in unit
...
class TestHermesHomeForTargetUser:
    ...
    def test_remaps_default_home(self, monkeypatch):
        ...
        assert result == "/home/alice/.teamhermes"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._hermes_home_for_target_user("/home/alice")
        assert result == "/home/alice/.teamhermes/profiles/coder"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "th gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_hermes_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.teamhermes should not produce a --profile flag."""
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.teamhermes/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".teamhermes" / "profiles" / "mybot"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.teamhermes/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".teamhermes" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".teamhermes" / "profiles" / "My Bot!"
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".teamhermes" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.teamhermes.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".teamhermes" / "hermes-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".teamhermes" / "hermes-agent")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".teamhermes" / "hermes-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".teamhermes" / "hermes-agent"
        ...
        monkeypatch.setenv("HERMES_HOME", str(root_home / ".teamhermes"))
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: root_home / ".teamhermes")
        ...
        assert "WorkingDirectory=/home/alice/.teamhermes" in unit
        assert "WorkingDirectory=/home/alice/.teamhermes/hermes-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'th gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'th gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'th gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyHermesUnitDetection:
    ...
    older install left a ``th.service`` unit behind when the service was
    ...
    The detector must flag ``th.service`` ONLY when it actually runs our
    ...
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=TeamHermes Gateway\n[Service]\n"
    ...
    def test_ignores_unrelated_hermes_service(self, tmp_path, monkeypatch):
        """Third-party ``th.service`` that isn't ours stays untouched.

        If a user has some other package named ``th`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "th.service").write_text(
            "[Unit]\nDescription=Some Other TeamHermes\n[Service]\n"
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "th.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "th.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        variants = [
            ...
            "ExecStart=/usr/local/bin/th gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"th.service" if i == 0 else f"th.service"  # same name
            # Test each variant fresh
            (user_dir / "th.service").write_text(
                f"[Unit]\nDescription=Old TeamHermes\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_hermes_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "th.service" in out
        assert "th gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "th.service"
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop th.service" in c for c in cmds_joined)
        assert any("--user disable th.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo th gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop th.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable th.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "th.service"
        system_legacy = system_dir / "th.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `th gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `th gateway --help` shows it.
        import subprocess
        import sys
...
class TestSystemScopeWizardPreCheck:
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``th gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        ...
        assert "sudo th gateway uninstall --system" in out
        assert "th gateway install" in out
...
class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``th gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        missing = tmp_path / "does-not-exist" / ".teamhermes"
