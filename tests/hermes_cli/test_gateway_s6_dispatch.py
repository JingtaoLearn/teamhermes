...
`th gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`th -p typo gateway start` should print a clear message and
    ...
    assert "th profile create typo" in out
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `th gateway run --replace`, the
