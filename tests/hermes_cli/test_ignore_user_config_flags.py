"""Tests for --ignore-user-config and --ignore-rules flags on `thm chat`.
...
files. In TeamHermes the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.teamhermes/config.yaml`` in ``load_cli_config()``
...
class TestArgparseFlagsRegistered:
    ...
    def test_flags_present_in_chat_parser(self):
        ...
        import argparse
        parser = argparse.ArgumentParser(prog="thm")
