"""Tests for the post-pull syntax guard in ``th update``.
...
becomes unbootable — every ``th`` invocation imports those files at
...
markers in ``hermes_cli/config.py``; users who ran ``th update`` in
the 7-minute window before #28458 landed could not run any ``th``
command afterward.
"""
...
# release; if a future ``th update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    assert ok is True, (
        ...
        f"th update would brick users. Error: {error}"
    )
