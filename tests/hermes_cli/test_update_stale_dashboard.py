"""Tests for the stale-dashboard handling run at the end of ``thm update``.

``thm update`` detects ``thm dashboard`` processes left over from the
...
class TestKillStaleDashboardPosix:
    ...
    def test_permission_error_is_reported_not_raised(self, capsys):
        ...
        must not abort thm update — it's reported as a failure and we
        move on."""
        def fake_kill(pid, sig):
...
class TestWindowsWmicEncoding:
    ...
    `thm update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
