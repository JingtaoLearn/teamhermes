"""Tests for the multi-board kanban layer (``thm kanban boards …``).
...
* CLI surface: ``thm kanban boards list/create/switch/rm``.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``thm kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
