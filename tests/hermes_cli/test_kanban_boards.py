"""Tests for the multi-board kanban layer (``th kanban boards …``).
...
* CLI surface: ``th kanban boards list/create/switch/rm``.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``th kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
