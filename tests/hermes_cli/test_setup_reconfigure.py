...
- Bare `th setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `th setup --quick` runs the narrower "fill in missing items" flow.
- `th setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `th setup` on an existing install = full reconfigure wizard."""
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`th setup --reconfigure` behaves the same as bare `th setup`."""
