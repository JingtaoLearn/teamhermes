...
dicts) keeps the test independent of the user's ~/.teamhermes/config.yaml and
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``th config set foo bar``."""
...
def test_save_config_set_supports_critical_bridged_keys():
    """``th config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`th config set terminal.X` doesn't sync these load-bearing "
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on TeamHermes exit or just stops it (keeping it for the next
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether TeamHermes sweeps stale
