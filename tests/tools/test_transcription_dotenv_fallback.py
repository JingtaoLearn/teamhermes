...
``~/.teamhermes/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.teamhermes/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.teamhermes/.env`` carrying the key while ``os.environ`` does not.
