...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.teamhermes/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.teamhermes/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(hermes_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.teamhermes")

        assert mounts[0]["container_path"] == "/home/user/.teamhermes/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.teamhermes/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.teamhermes/skills/cat/myskill/scripts/run.sh" in paths
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.teamhermes/cache/documents" in paths
        assert "/root/.teamhermes/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.teamhermes/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        assert "/root/.teamhermes/cache/documents" in container_paths
        assert "/root/.teamhermes/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        assert entries[0]["container_path"] == "/root/.teamhermes/cache/screenshots/session_abc/screen1.png"
