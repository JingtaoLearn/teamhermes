...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"teamhermes[termux]"' in text
    assert '"teamhermes[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"teamhermes[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
