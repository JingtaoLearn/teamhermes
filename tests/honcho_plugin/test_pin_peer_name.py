...
Under a gateway (Telegram, Discord, Slack, ...) TeamHermes passes the
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to TeamHermes via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each TeamHermes profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `th -p partner` pinned to a different person's peer).
