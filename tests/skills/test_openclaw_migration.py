...
"""
    ...
    assert "Tyler Williams > Active Projects: TeamHermes Agent" in entries
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.teamhermes/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``TeamHermes``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "TeamHermes prefers Python 3.11"
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "TeamHermes remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "hermes prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "TeamHermes was configured for Spanish"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "TeamHermes said to ask TeamHermes about TeamHermes settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.teamhermes`` (the real TeamHermes home), not the broken
    ``.TeamHermes``.
    ...
    rewritten to ``~/.TeamHermes/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.teamhermes/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .teamhermes directory"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are TeamHermes" in result
