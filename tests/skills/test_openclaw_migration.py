...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `thm cron` to recreate scheduled tasks" in notes_text
...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    # real filesystem path ``~/.hermes`` (TeamHermes home) when rebranding
...
def test_rebrand_text_preserves_filesystem_path_casing():
    ...
    rewrite to lowercase ``.hermes`` (the real TeamHermes home), not the broken
    ``.TeamHermes``.
