...
本页涵盖所有与 [TeamHermes profiles](../user-guide/profiles.md) 相关的命令。通用 CLI 命令请参阅 [CLI 命令参考](./cli-commands.md)。
...
管理 profile 的顶级命令。不带子命令运行 `th profile` 将显示帮助信息。
...
将 `<name>` 设为活跃 profile。此后所有 `th` 命令（不带 `-p`）都将使用该 profile。
...
```bash
th profile use work
th profile use default
```

## `th profile create`

```bash
th profile create <name> [options]
```
...
| `--description "<text>"` | 一到两句话描述该 profile 的用途。供 kanban 编排器根据角色而非仅凭 profile 名称来路由任务。可跳过，稍后通过 `th profile describe` 添加。持久化保存在 `<profile_dir>/profile.yaml` 中。 |
| `--no-skills` | 创建一个**空** profile，不启用任何内置 skill。会在 profile 目录中写入 `.no-skills` 标记，使后续 `th update` 不再重新植入内置 skill 集，且拒绝与 `--clone` / `--clone-all` 组合使用（因为后者会复制 skill）。适用于不应继承完整 skill 目录的窄化编排器 profile 或沙箱 profile。 |
...
th profile create work2 --clone --clone-from work
```

## `th profile describe`

```bash
th profile describe [<name>] [options]
```
...
```bash
# 读取当前描述
th profile describe researcher

# 显式设置描述
th profile describe researcher --text "Reads source code and writes findings."

# 让 LLM 生成描述
th profile describe researcher --auto

# 为所有没有描述的 profile 填充描述
th profile describe --all --auto
```

## `th profile delete`

```bash
th profile delete <name> [options]
```
...
```bash
th profile delete mybot
th profile delete mybot --yes
```
...
```bash
th profile show <name>
```

显示 profile 的详细信息，包括其主目录、配置的模型、gateway 状态、skill 数量和配置文件状态。

此处显示的是 profile 的 TeamHermes 主目录，而非终端工作目录。终端命令从 `terminal.cwd` 启动（或在本地后端 `cwd: "."` 时从启动目录启动）。
...
```bash
th profile alias <name> [options]
```

重新生成位于 `~/.local/bin/<name>` 的 shell alias 脚本。适用于 alias 被意外删除，或移动 TeamHermes 安装目录后需要更新的情况。
...
```bash
th profile alias work
# 创建/更新 ~/.local/bin/work

th profile alias work --name mywork
# 创建 ~/.local/bin/mywork

th profile alias work --remove
# 移除 wrapper 脚本
```
...
```bash
th profile rename <old-name> <new-name>
```
...
```bash
th profile rename mybot assistant
...
```bash
th profile export <name> [options]
```
...
```bash
th profile export work
# 在当前目录创建 work.tar.gz

th profile export work -o ./work-2026-03-29.tar.gz
```

## `th profile import`

```bash
th profile import <archive> [options]
```
...
```bash
th profile import ./work-2026-03-29.tar.gz
# 从归档文件推断 profile 名称

th profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`th profile export` / `import` 仍是在**本机进行 profile 本地备份和恢复**的正确命令。发行版（`install` / `update` / `info`）是独立概念：通过 git 分发 profile，供他人安装。
:::

### `th profile install`

```bash
th profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | 同时创建 shell wrapper（例如 `telemetry` → `th -p telemetry`）。 |
...
```bash
# 从 GitHub 仓库安装（简写）
th profile install github.com/kyle/telemetry-distribution --alias

# 从完整 HTTPS git URL 安装
th profile install https://github.com/kyle/telemetry-distribution.git

# 从 SSH 安装
th profile install git@github.com:kyle/telemetry-distribution.git

# 开发时从本地目录安装
th profile install ./telemetry/
```

### `th profile update`

```bash
th profile update <name> [--force-config] [--yes]
```
...
```bash
th profile info <name>
```

打印 profile 的发行版 manifest — 名称、版本、所需 TeamHermes 版本、作者、环境变量要求、来源 URL/路径，以及发行版最后一次 `install` 或 `update` 时记录的 `Installed:` 时间戳。适用于安装前检查共享 profile 的需求，以及发现"该 profile 已安装 6 个月未更新"等情况。

`th profile list` 也会在 `Distribution` 列中显示发行版名称和版本，`th profile show <name>` / `delete <name>` 会显示来源 URL，让你一眼看出哪些 profile 来自 git 仓库，哪些是本地创建的。
...
th profile install git@github.com:your-org/internal-assistant.git

# 使用 git credential helper
th profile install https://github.com/your-org/internal-assistant.git
```
...
`hermes_requires` 支持 `>=`、`<=`、`==`、`!=`、`>`、`<`，或裸版本号（视为 `>=`）。若当前 TeamHermes 版本不满足规格，安装将失败并给出明确错误。
...
2. 初始化 git 仓库（或使用已有仓库），推送到 GitHub / GitLab / 任何 TeamHermes 可克隆的托管平台。
3. 告知接收方运行 `th profile install <your-repo-url>`。
...
```bash
th -p <name> <command> [options]
th --profile <name> <command> [options]
```
...
```bash
th -p work chat -q "Check the server status"
th --profile dev gateway start
th -p personal skills list
th -p work config edit
```
...
安装后，Tab 补全适用于：
- `th profile <TAB>` — 子命令（list、use、create 等）
- `th profile use <TAB>` — profile 名称
- `th -p <TAB>` — profile 名称
