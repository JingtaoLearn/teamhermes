...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 TeamHermes profile（配置文件）。覆盖 `hermes profile use` 设置的粘性默认值。 |
...
| `hermes mcp` | 管理 MCP 服务器配置，并将 TeamHermes 作为 MCP 服务器运行。 |
| `hermes plugins` | 管理 TeamHermes Agent plugin（安装、启用、禁用、删除）。 |
...
| `hermes profile` | 管理 profile——多个隔离的 TeamHermes 实例。 |
...
**`hermes model`**（从终端运行，在任何 TeamHermes 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 TeamHermes 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 TeamHermes 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `hermes model`。
:::
...
hermes auth spotify                                      # 通过 PKCE 将 TeamHermes 与 Spotify 认证
```
...
`hermes update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 TeamHermes 更新时，使用此选项。
...
- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 前对 `~/.teamhermes/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `hermes backup restore --state pre-update` 回滚。
