...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 TeamHermes profile（配置文件）。覆盖 `thm profile use` 设置的粘性默认值。 |
...
| 命令 | 用途 |
|---------|---------|
| `thm chat` | 与 agent 进行交互式或单次聊天。 |
| `thm model` | 交互式选择默认 provider 和模型。 |
| `thm fallback` | 管理主模型出错时依次尝试的 fallback provider。 |
| `thm gateway` | 运行或管理消息 gateway 服务。 |
| `thm proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](../user-guide/features/subscription-proxy.md)。 |
| `thm lsp` | 管理 Language Server Protocol 集成（为 write_file/patch 提供语义诊断）。 |
| `thm setup` | 全部或部分配置的交互式设置向导。 |
| `thm whatsapp` | 配置并配对 WhatsApp 桥接。 |
| `thm slack` | Slack 辅助工具（当前功能：生成将每条命令注册为原生斜杠命令的 app manifest）。 |
| `thm auth` | 管理凭据——添加、列出、删除、重置、设置策略。处理 Codex/Nous/Anthropic 的 OAuth 流程。 |
| `thm login` / `logout` | **已弃用** — 请改用 `thm auth`。 |
| `thm status` | 显示 agent、auth 和平台状态。 |
| `thm cron` | 检查并触发 cron 调度器。 |
| `thm kanban` | 多 profile 协作看板（任务、链接、调度器）。 |
| `thm webhook` | 管理用于事件驱动激活的动态 webhook 订阅。 |
| `thm hooks` | 检查、审批或删除 `config.yaml` 中声明的 shell 脚本 hook。 |
| `thm doctor` | 诊断配置和依赖问题。 |
| `thm security audit` | 对 venv、plugin 依赖和固定 MCP 服务器进行按需供应链审计（OSV.dev）。 |
| `thm dump` | 可直接复制粘贴的设置摘要，用于支持/调试。 |
| `thm debug` | 调试工具——上传日志和系统信息以获取支持。 |
| `thm backup` | 将 TeamHermes 主目录备份为 zip 文件。 |
| `thm checkpoints` | 检查/修剪/清除 `~/.teamhermes/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
| `thm import` | 从 zip 文件恢复 TeamHermes 备份。 |
| `thm logs` | 查看、跟踪和过滤 agent/gateway/错误日志文件。 |
| `thm config` | 显示、编辑、迁移和查询配置文件。 |
| `thm pairing` | 审批或撤销消息配对码。 |
| `thm skills` | 浏览、安装、发布、审计和配置 skill。 |
| `thm bundles` | 将多个 skill 归组到单个 `/<name>` 斜杠命令下。参见 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。 |
| `thm curator` | 后台 skill 维护——状态、运行、暂停、固定。参见 [Curator](../user-guide/features/curator.md)。 |
| `thm memory` | 配置外部 memory provider。当对应 provider 激活时，特定于 plugin 的子命令（如 `thm honcho`）会自动注册。 |
| `thm acp` | 将 TeamHermes 作为 ACP 服务器运行，用于编辑器集成。 |
| `thm mcp` | 管理 MCP 服务器配置，并将 TeamHermes 作为 MCP 服务器运行。 |
| `thm plugins` | 管理 TeamHermes Agent plugin（安装、启用、禁用、删除）。 |
| `thm portal` | Nous Portal 状态、订阅链接和 Tool Gateway 路由。参见 [Tool Gateway](../user-guide/features/tool-gateway.md)。 |
| `thm tools` | 按平台配置已启用的工具。 |
| `thm computer-use` | 安装或检查 cua-driver 后端（macOS Computer Use）。 |
| `thm sessions` | 浏览、导出、修剪、重命名和删除会话。 |
| `thm insights` | 显示 token/费用/活动分析。 |
| `thm claw` | OpenClaw 迁移辅助工具。 |
| `thm dashboard` | 启动用于管理配置、API 密钥和会话的 Web 控制台。 |
| `thm profile` | 管理 profile——多个隔离的 TeamHermes 实例。 |
| `thm completion` | 打印 shell 补全脚本（bash/zsh/fish）。 |
| `thm version` | 显示版本信息。 |
| `thm update` | 拉取最新代码并重新安装依赖（git 安装），或检查 PyPI 并执行 `pip install --upgrade`（pip 安装）。`--check` 预览而不安装；`--backup` 在拉取前对 `HERMES_HOME` 进行快照。 |
| `thm uninstall` | 从系统中删除 TeamHermes。 |
...
### `thm -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`thm -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
相同的 agent、相同的工具、相同的 skill——只是剥离了所有交互式/装饰性层。如果你还需要在记录中包含工具输出，请改用 `thm chat -q`；`-z` 专门用于"我只需要最终答案"的场景。
...
:::warning thm model 与 /model——了解区别
**`thm model`**（从终端运行，在任何 TeamHermes 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 TeamHermes 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 TeamHermes 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `thm model`。
:::
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `thm model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `HERMES_HOME`。当你并行运行多个 profile 并希望在 `thm update` 后全部重启时很有用。 |
...
:::tip WSL 用户
使用 `thm gateway run` 而非 `thm gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s thm 'thm gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容别名——在已有安装上裸运行 `thm setup` 现在默认执行此操作。 |
...
关于 gateway 本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一键设置路径，请参阅上方的 `thm setup --portal`。
...
```bash
thm slack manifest              # 将 manifest 打印到 stdout
thm slack manifest --write      # 写入 ~/.teamhermes/slack-manifest.json
thm slack manifest --slashes-only  # 仅输出 features.slash_commands 数组
```
...
`thm update` 后重新运行 `thm slack manifest --write` 以获取新增命令。


## `thm login` / `thm logout` *（已弃用）*

:::caution
`thm login` 已被移除。请使用 `thm auth` 管理 OAuth 凭据，使用 `thm model` 选择 provider，或使用 `thm setup` 进行完整的交互式设置。
:::
...
thm auth                                              # 交互式向导
thm auth list                                         # 显示所有池
thm auth list openrouter                              # 显示特定 provider
thm auth add openrouter --api-key sk-or-v1-xxx        # 添加 API 密钥
thm auth add anthropic --type oauth                   # 添加 OAuth 凭据
thm auth remove openrouter 2                          # 按索引删除
thm auth reset openrouter                             # 清除冷却时间
thm auth status anthropic                             # 显示某 provider 的认证状态
thm auth logout anthropic                             # 登出并清除已存储的认证状态
thm auth spotify                                      # 通过 PKCE 将 TeamHermes 与 Spotify 认证
```
...
| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `thm kanban boards switch`、`HERMES_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `thm kanban`。Worker 的环境中固定了 `HERMES_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
thm kanban list                  # 显示 atm10-server 的任务
...
:::tip
`thm dump` 专为分享而设计。交互式诊断请使用 `thm doctor`。可视化概览请使用 `thm status`。
:::
...
```bash
thm debug share              # 上传调试报告，打印 URL
thm debug share --lines 500  # 包含更多日志行
thm debug share --expire 30  # 粘贴保留 30 天
thm debug share --local      # 在终端打印报告（不上传）
```
...
创建 TeamHermes 配置、skill、会话和数据的 zip 归档。备份不包含 teamthm 代码库本身。
...
- `teamthm` 代码本身（这是用户数据备份，不是仓库快照）。

### 示例

```bash
thm backup                           # 完整备份到 ~/hermes-backup-*.zip
thm backup -o /tmp/hermes.zip        # 完整备份到指定路径
thm backup --quick                   # 仅状态快速快照
thm backup --quick --label "pre-upgrade"  # 带标签的快速快照
```
...
| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `thm checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
```bash
thm checkpoints                                  # 状态概览
thm checkpoints prune --retention-days 3         # 激进清理
thm checkpoints prune --max-size-mb 200          # 一次性收紧大小上限
thm checkpoints clear-legacy -f                  # 删除 v1 归档目录
thm checkpoints clear -f                         # 清除所有内容
```
...
### 示例
```bash
thm import ~/hermes-backup-20260423.zip           # 覆盖现有配置前提示确认
thm import ~/hermes-backup-20260423.zip --force   # 不提示直接覆盖
```
...
TeamHermes 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`thm logs list` 子命令显示所有日志文件，包括已轮转的。
...
thm skills install https://sharethis.chat/SKILL.md                     # 直接 URL（单文件 SKILL.md）
thm skills install https://example.com/SKILL.md --name my-skill        # frontmatter 无名称时覆盖名称
thm skills check
thm skills update
thm skills config
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `thm update` 后的第一次 tick 时立即执行 curator。使用 `thm curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `thm model` 相同的选择器）并追加到链末尾 |
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `thm <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `thm honcho`）。未激活的 provider 不暴露其子命令。运行 `thm --help` 查看当前已连接的命令。
:::
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `thm plugins` 会打开包含两个部分的复合交互界面：
...
`thm computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时 `thm tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`thm update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 TeamHermes 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 TeamHermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.teamthm`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。
...
| `--no-backup` | 跳过迁移前对 `~/.teamhermes/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.teamhermes/backups/pre-migration-*.zip`；可用 `thm import` 恢复）。 |
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `pip install teamthm[web]`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页需要 `--tui` 加上 `pty` extra。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--tui` | 关闭 | 通过 PTY/WebSocket 桥接在后台运行 `thm --tui`，启用浏览器内 Chat 标签页。需要 `pip install 'teamthm[web,pty]'` 以及 Linux、macOS 或 WSL2 等 POSIX PTY 环境。 |
| `--insecure` | 关闭 | 允许绑定到非 localhost 主机。会在网络上暴露控制台凭据；仅在受信任的网络控制下使用。 |
| `--stop` | — | 停止正在运行的 `thm dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `thm dashboard` 进程并退出。 |
...
拉取最新的 `teamthm` 代码并在 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。

**pip 安装：** `thm update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade teamthm`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。
...
- **配对数据快照。** 即使 `--backup` 关闭，`thm update` 也会在 `git pull` 前对 `~/.teamhermes/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `thm backup restore --state pre-update` 回滚。
- **旧版 `hermes.service` 警告。** 如果 TeamHermes 检测到预重命名的 `hermes.service` systemd 单元（而非当前的 `hermes-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
|---------|-------------|
| `thm version` | 打印版本信息。 |
| `thm update` | 拉取最新变更并重新安装依赖。 |
| `thm uninstall [--full] [--yes]` | 删除 TeamHermes，可选择删除所有 config/数据。 |
