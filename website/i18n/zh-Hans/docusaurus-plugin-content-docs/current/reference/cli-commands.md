...
| `hermes checkpoints` | 检查/修剪/清除 `~/.teamhermes/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
...
| `thm memory` | 配置外部 memory provider。当对应 provider 激活时，特定于 plugin 的子命令（如 `hermes honcho`）会自动注册。 |
| `hermes acp` | 将 TeamHermes 作为 ACP 服务器运行，用于编辑器集成。 |
...
| `hermes uninstall` | 从系统中删除 TeamHermes。 |
...
:::tip WSL 用户
使用 `thm gateway run` 而非 `thm gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s hermes 'thm gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
hermes slack manifest --write      # 写入 ~/.teamhermes/slack-manifest.json
...
`thm update` 后重新运行 `hermes slack manifest --write` 以获取新增命令。
...
:::tip
`hermes dump` 专为分享而设计。交互式诊断请使用 `thm doctor`。可视化概览请使用 `thm status`。
:::
...
创建 TeamHermes 配置、skill、会话和数据的 zip 归档。备份不包含 teamhermes 代码库本身。
...
- `teamhermes` 代码本身（这是用户数据备份，不是仓库快照）。
...
TeamHermes 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`hermes logs list` 子命令显示所有日志文件，包括已轮转的。
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `hermes <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `hermes honcho`）。未激活的 provider 不暴露其子命令。运行 `thm --help` 查看当前已连接的命令。
:::
...
`hermes computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时 `thm tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `pip install teamhermes[web]`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页需要 `--tui` 加上 `pty` extra。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
| `--host` | `127.0.0.1` | 绑定地址 |
| `--no-open` | — | 不自动打开浏览器 |
| `--tui` | 关闭 | 通过 PTY/WebSocket 桥接在后台运行 `hermes --tui`，启用浏览器内 Chat 标签页。需要 `pip install 'teamhermes[web,pty]'` 以及 Linux、macOS 或 WSL2 等 POSIX PTY 环境。 |
...
拉取最新的 `teamhermes` 代码并在 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。

**pip 安装：** `thm update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade teamhermes`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。
...
| `hermes uninstall [--full] [--yes]` | 删除 TeamHermes，可选择删除所有 config/数据。 |
