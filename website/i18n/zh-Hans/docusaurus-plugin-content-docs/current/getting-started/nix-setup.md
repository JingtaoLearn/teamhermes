...
执行 `nix profile install` 后，`thm`、`teamhermes` 和 `thm-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`thm setup` 引导你完成提供商选择，`thm gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.teamhermes/`。
...
设置 `container.hostUsers` 可创建 `~/.teamhermes` 到服务状态目录的符号链接，使主机 CLI 和容器共享会话、配置和记忆：
...
| 覆盖 hermes 包 | `package` | `inputs.teamhermes-agent.packages.${system}.default.override { ... }` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$HERMES_HOME/.env` 中。TeamHermes 在每次启动时读取此文件，因此更改在 `systemctl restart teamhermes` 后生效——无需重建容器。
...
2. **`.managed` 标记文件**，位于 `HERMES_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it teamhermes thm config set ...` 也会被屏蔽）
...
~/.teamhermes -> /var/lib/hermes/.teamhermes       （符号链接桥接，按 hostUsers）
...
|---|---|---|---|---|
| `systemctl restart teamhermes` | 否 | 保留 | 保留 | 保留 |
...
对于已在 teamhermes 的 `pyproject.toml` 中声明的可选 extras（例如 `hindsight` 或 `honcho` 等记忆提供商），使用 `extraDependencyGroups` 在构建时将其包含到封闭的 venv 中：
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `thm` 和 `teamhermes` 二进制文件存在且 `thm version` 可运行 |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `enable` | `bool` | `false` | 启用 teamhermes 服务 |
| `package` | `package` | `teamhermes` | 使用的 teamhermes 包 |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.teamhermes` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `thm` 组 |
...
| `thm version` 显示旧版本 | 容器未重启 | `systemctl restart teamhermes` |
...
| `no container with name or ID "teamhermes"`（Podman） | Podman rootful 容器对普通用户不可见 | 为 podman 添加免密 sudo（参见[容器模式](#container-mode)章节） |
| `unable to find user hermes` | 容器仍在启动中（入口点尚未创建用户） | 等待几秒后重试——CLI 会自动重试 |
| 通过 `extraPackages` 添加的工具在终端中找不到 | 需要 `nixos-rebuild switch` 更新每用户 profile | 重建并重启：`nixos-rebuild switch && systemctl restart teamhermes` |