...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`thm setup`、`hermes gateway install`、编辑配置）与标准安装完全相同。
...
执行 `nix profile install` 后，`thm`、`hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`thm setup` 引导你完成提供商选择，`hermes gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.teamhermes/`。
...
- 路由是透明的：`thm chat`、`hermes sessions list`、`thm version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
...
Agent 身份文件是独立的：Hermes 从 `$HERMES_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.teamhermes-agent.stateDir}/.teamhermes/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
