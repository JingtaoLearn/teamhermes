...
执行 `nix profile install` 后，`thm`、`hermes-agent` 和 `thm-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`thm setup` 引导你完成提供商选择，`thm gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.teamhermes/`。
...
2. **`.managed` 标记文件**，位于 `HERMES_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it hermes-agent thm config set ...` 也会被屏蔽）
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `thm` 和 `hermes-agent` 二进制文件存在且 `thm version` 可运行 |
...
| `thm version` 显示旧版本 | 容器未重启 | `systemctl restart hermes-agent` |
