...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`th setup`、`th gateway install`、编辑配置）与标准安装完全相同。
...
执行 `nix profile install` 后，`th`、`hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`th setup` 引导你完成提供商选择，`th gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.teamhermes/`。
...
就这些。`nixos-rebuild switch` 会创建 `th` 用户、生成 `config.yaml`、连接密钥并启动 gateway——这是一个长期运行的服务，将 Agent 连接到消息平台（Telegram、Discord 等）并监听传入消息。
...
设置 `addToSystemPackages = true` 有两个作用：将 `th` CLI 添加到系统 PATH，**并**在系统范围内设置 `HERMES_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `th` 会创建独立的 `~/.teamhermes/` 目录。
:::

### 容器感知 CLI

:::info
当 `container.enable = true` 且 `addToSystemPackages = true` 时，主机上的**所有** `th` 命令都会自动路由到托管容器中执行。这意味着你的交互式 CLI 会话在与 gateway 服务相同的环境中运行——可以访问所有容器内安装的包和工具。

- 路由是透明的：`th chat`、`th sessions list`、`th version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
...
`hostUsers` 中列出的用户会自动加入 `th` 组以获得文件权限访问。
...
CLI 会自动检测何时需要 sudo 并透明地使用它。没有此配置，你需要手动运行 `sudo th chat`。
:::
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$HERMES_HOME/.env` 中。TeamHermes 在每次启动时读取此文件，因此更改在 `systemctl restart hermes-agent` 后生效——无需重建容器。
...
| 被屏蔽的命令 | 原因 |
|---|---|
| `th setup` | 配置是声明式的——请在 Nix 配置中编辑 `settings` |
| `th config edit` | 配置由 `settings` 生成 |
| `th config set <key> <value>` | 配置由 `settings` 生成 |
| `th gateway install` | systemd 服务由 NixOS 管理 |
| `th gateway uninstall` | systemd 服务由 NixOS 管理 |

这可以防止 Nix 声明的内容与磁盘上实际内容之间产生漂移。检测使用两个信号：

1. **`HERMES_MANAGED=true`** 环境变量——由 systemd 服务设置，对 gateway 进程可见
2. **`.managed` 标记文件**，位于 `HERMES_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it hermes-agent th config set ...` 也会被屏蔽）
...
Nix 构建的二进制文件能在 Ubuntu 容器内运行，是因为 `/nix/store` 被绑定挂载——它携带自己的解释器和所有依赖，不依赖容器的系统库。容器入口点通过 `current-package` 符号链接解析：`/data/current-package/bin/th gateway run --replace`。执行 `nixos-rebuild switch` 时，只更新符号链接——容器继续运行。
...
NixOS 模块支持声明式插件安装——无需命令式的 `th plugins install`。
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `th` 和 `hermes-agent` 二进制文件存在且 `th version` 可运行 |
| `entry-points-sync` | `pyproject.toml` 中 `[project.scripts]` 的每个条目在 Nix 包中都有对应的封装二进制文件 |
| `cli-commands` | `th --help` 暴露 `gateway` 和 `config` 子命令 |
| `managed-guard` | `HERMES_MANAGED=true th config set ...` 打印 NixOS 错误 |
...
| `addToSystemPackages` | `bool` | `false` | 将 `th` CLI 添加到系统 PATH 并在系统范围内设置 `HERMES_HOME` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | `th gateway` 的额外参数 |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.teamhermes` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `th` 组 |
...
| `th version` 显示旧版本 | 容器未重启 | `systemctl restart hermes-agent` |
