...
| 目标 | 先做这步 | 再做这步 |
|---|---|---|
| 只想让 TeamHermes 在本机跑起来 | `thm setup` | 运行一次真实对话并验证有响应 |
| 已知道要用哪个 provider | `thm model` | 保存配置，然后开始聊天 |
| 想搭建机器人或长期运行的服务 | CLI 正常后运行 `thm gateway setup` | 接入 Telegram、Discord、Slack 或其他平台 |
| 想使用本地或自托管模型 | `thm model` → 自定义 endpoint | 验证 endpoint、模型名称和上下文长度 |
| 想要多 provider 故障转移 | 先运行 `thm model` | 基础对话正常后再添加路由和故障转移 |
...
这是最重要的配置步骤。使用 `thm model` 以交互方式完成选择：
...
| Provider | 说明 | 配置方式 |
|----------|-----------|---------------|
| **Nous Portal** | 订阅制，零配置 | 通过 `thm model` 进行 OAuth 登录 |
| **OpenAI Codex** | ChatGPT OAuth，使用 Codex 模型 | 通过 `thm model` 进行设备码认证 |
| **Anthropic** | 直接使用 Claude 模型——Max 计划 + 额外用量积分（OAuth），或按 token 付费的 API key | `thm model` → OAuth 登录（需要 Max + 额外积分），或 Anthropic API key |
...
| **MiniMax (OAuth)** | 通过浏览器 OAuth 使用 MiniMax-M2.7，无需 API key | `thm model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot 订阅（GPT-5.x、Claude、Gemini 等） | 通过 `thm model` 进行 OAuth，或设置 `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent 后端（在本地启动 `copilot` CLI） | `thm model`（需要 `copilot` CLI + `copilot login`） |
...
:::tip
你可以随时通过 `thm model` 切换 provider——没有锁定。所有支持的 provider 完整列表及配置详情，请参阅 [AI Providers](../integrations/providers.md)。
:::
...
- `thm tools` — 按平台调整工具访问权限
- `thm skills` — 浏览并安装可复用的工作流
...
```bash
thm config set terminal.backend docker    # Docker 隔离
thm config set terminal.backend ssh       # 远程服务器
```
...
|---------|-------------|
| `thm` | 开始聊天 |
| `thm model` | 选择 LLM provider 和模型 |
| `thm tools` | 配置每个平台启用的工具 |
| `thm setup` | 完整配置向导（一次性配置所有内容） |
| `thm doctor` | 诊断问题 |
| `thm update` | 更新到最新版本 |
