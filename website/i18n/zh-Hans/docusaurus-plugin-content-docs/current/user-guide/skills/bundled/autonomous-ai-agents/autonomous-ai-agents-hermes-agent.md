...
| 标签 | `thm`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `development` |
...
使用 `thm config edit` 或 `thm config set section.key value` 编辑。
...
支持 20+ 个提供商。通过 `thm model` 或 `thm setup` 设置。
...
通过 `thm tools`（交互式）或 `thm tools enable/disable NAME` 启用/禁用。
...
常见的"为什么 TeamHermes 对我的输出/工具调用/命令做了 X？"开关——以及更改它们的确切命令。其中大多数需要新会话（聊天中的 `/reset`，或启动新的 `thm` 调用），因为它们在启动时只读取一次。
...
```bash
thm config set security.redact_secrets true       # 全局启用
```

**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export HERMES_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `thm config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
```bash
thm config set privacy.redact_pii true    # 启用
thm config set privacy.redact_pii false   # 禁用（默认）
```
...
```bash
thm config set approvals.mode smart       # 推荐的折中方案
thm config set approvals.mode off         # 绕过一切（不推荐）
```
...
要完全阻止模型访问网络或媒体工具，打开 `thm tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
- **fire-and-forget 使用 `thm chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`thm cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
- **CLI：** `thm curator <verb>` — `status`、`run`、`pause`、`resume`、`pin`、`unpin`、`archive`、`restore`、`prune`、`backup`、`rollback`。
...
用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `thm kanban <verb>` 驱动；调度器生成的 worker 看到由 `HERMES_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`thm config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。
...
### 工具不可用
1. `thm tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `thm doctor` — 检查配置和依赖
2. `thm auth` — 重新认证 OAuth 提供商（或 `thm auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `thm model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `thm config edit` 或[配置文档](https://hermes-agent.nousresearch.com/docs/user-guide/configuration) |
| 可用工具 | `thm tools list` 或[工具参考](https://hermes-agent.nousresearch.com/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://hermes-agent.nousresearch.com/docs/reference/slash-commands) |
| Skill 目录 | `thm skills browse` 或[Skill 目录](https://hermes-agent.nousresearch.com/docs/reference/skills-catalog) |
| 提供商设置 | `thm model` 或[提供商指南](https://hermes-agent.nousresearch.com/docs/integrations/providers) |
| 平台设置 | `thm gateway setup` 或[消息文档](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/) |
| MCP 服务器 | `thm mcp list` 或[MCP 指南](https://hermes-agent.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `thm profile list` 或[Profiles 文档](https://hermes-agent.nousresearch.com/docs/user-guide/profiles) |
| Cron 任务 | `thm cron list` 或[Cron 文档](https://hermes-agent.nousresearch.com/docs/user-guide/features/cron) |
| 记忆 | `thm memory status` 或[记忆文档](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory) |
| 环境变量 | `thm config env-path` 或[环境变量参考](https://hermes-agent.nousresearch.com/docs/reference/environment-variables) |
| CLI 命令 | `thm --help` 或[CLI 参考](https://hermes-agent.nousresearch.com/docs/reference/cli-commands) |
| Gateway 日志 | `~/.teamhermes/logs/gateway.log` |
| 会话文件 | `~/.teamhermes/sessions/` 或 `thm sessions browse` |
