...
| 相关 skills | [`teamhermes`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-teamhermes) |
...
运行 `hermes honcho setup`。确保 `~/.teamhermes/config.yaml` 中包含 `memory.provider: honcho`。

### 记忆未跨会话持久化
检查 `hermes honcho status` -- 验证 `saveMessages: true` 且 `writeFrequency` 不是 `session`（该选项仅在退出时写入）。

### 配置文件未获得自己的 peer
创建时使用 `--clone`：`thm profile create <name> --clone`。对于现有配置文件：`hermes honcho sync`。
...
| `hermes honcho sync` | 为所有尚未拥有 host 块的 TeamHermes 配置文件创建 host 块 |
| `hermes honcho migrate` | 从 OpenClaw 原生记忆迁移到 TeamHermes + Honcho 的分步指南 |
