...
1. **已安装 TeamHermes Agent** — `pip install teamhermes`（参见 [安装](/getting-started/installation)）
2. **已配置 LLM 提供商** — 运行 `thm model` 或在 `~/.teamhermes/.env` 中设置首选提供商的凭据
3. **基础设置正常** — 运行 `thm` 验证 Agent 能够响应文字消息，再启用语音功能

:::tip
`~/.teamhermes/` 目录和默认的 `config.yaml` 会在首次运行 `thm` 时自动创建。只需手动创建 `~/.teamhermes/.env` 来存放 API 密钥。
:::

:::tip Nous Portal 同时覆盖两项
付费的 [Nous Portal](/user-guide/features/tool-gateway) 订阅通过 Tool Gateway 同时提供 LLM（第 2 步）**和** OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，`thm setup --portal` 可一次性完成两项配置。
:::
...
语音模式在**经典 CLI**（`thm chat`）和 **TUI**（`hermes --tui`）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至 `~/.teamhermes/logs/`，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
...
1. 使用 `thm` 启动 CLI，并通过 `/voice on` 启用语音模式
