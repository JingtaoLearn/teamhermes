...
```bash
th memory setup      # 交互式选择器 + 配置
th memory status     # 查看当前激活状态
th memory off        # 禁用外部提供者
```

也可以通过 `th plugins` → Provider Plugins → Memory Provider 选择激活的记忆提供者。
...
**安装向导：**
```bash
th memory setup        # 选择 "honcho" — 运行 Honcho 专属的安装后配置
```

旧版 `th honcho setup` 命令仍然有效（现在会重定向到 `th memory setup`），但只有在 Honcho 被选为激活记忆提供者后才会注册。
...
:::tip 从 `th honcho` 迁移
如果你之前使用过 `th honcho setup`，你的配置和所有服务端数据均完好无损。只需通过安装向导重新启用，或手动设置 `memory.provider: honcho`，即可通过新系统重新激活。
:::
...
| **AI peer**（`aiPeer`） | 每个 TeamHermes profile 一个。host key `th` → 默认；其他 profile 使用 `hermes.<profile>`。 |
...
扫描所有 TeamHermes profile，为没有 host 块的 profile 创建 host 块，从默认 `th` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
**本地模式 UI：** `hindsight-embed -p th ui start`
...
**安装：**
```bash
th memory setup    # 选择 "holographic"
# 或手动配置：
...
**安装：**
```bash
th memory setup    # 选择 "supermemory"
# 或手动配置：
...
| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `container_tag` | `th` | 用于搜索和写入的容器标签。支持 `{identity}` 模板用于 profile 范围隔离。 |
