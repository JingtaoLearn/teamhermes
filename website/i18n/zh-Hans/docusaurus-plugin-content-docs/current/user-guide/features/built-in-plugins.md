...
`plugins/memory/` 和 `plugins/context_engine/` 被刻意排除在内置扫描之外。这两个目录使用各自的发现路径，因为内存提供者和上下文引擎是通过 `thm memory setup` / 配置中的 `context.engine` 进行单选配置的提供者。
...
仓库在 `plugins/` 下附带了以下内置插件。所有插件均需手动启用——通过 `thm plugins enable <name>` 启用。
...
内存提供者（`plugins/memory/*`）和上下文引擎（`plugins/context_engine/*`）在 [内存提供者](./memory-providers.md) 中单独列出——它们分别通过 `thm memory` 和 `thm plugins` 管理。以下是两个长期运行的基于 hook 的插件的详细说明。
...
**启用：** `thm plugins enable disk-cleanup`（或在 `thm plugins` 中勾选复选框）。

**再次禁用：** `thm plugins disable disk-cleanup`。
...
会话分组基于 TeamHermes 会话 ID（或子 agent 的任务 ID），通过 `langfuse.propagate_attributes` 实现，因此单次 `thm chat` 会话中的所有内容都归属于同一个 Langfuse session。
...
**禁用：** `thm plugins disable observability/langfuse`。插件模块仍会被发现，但在你重新启用之前不会运行任何模块代码。
...
**禁用：** `thm plugins disable google_meet`。已缓存的转录和录音保留在 `~/.teamhermes/cache/google_meet/`，直到你手动删除。
...
**启用：** 无需启用——`hermes-achievements` 是一个仅限仪表盘的插件（无生命周期 hook，无模型可见工具）。它在 `thm dashboard` 首次启动时自动注册为标签页。`plugins.enabled` 配置仅控制生命周期/工具插件；仪表盘插件完全通过其 `dashboard/manifest.json` 发现。
...
- 在 `thm plugins list` 中，manifest 来源显示为 `bundled`
- 同名用户插件会覆盖内置版本
