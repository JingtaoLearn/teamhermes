...
```bash
th cron list
th cron pause <job_id>
th cron resume <job_id>
th cron run <job_id>
th cron remove <job_id>
th cron status
th cron tick
```
...
sudo th gateway install --system   # Linux：服务器开机启动的系统服务
th gateway             # 或在前台运行
...
Cron 在全新的 agent 会话中运行每个任务，不附加任何聊天平台。默认情况下，cron agent 获得**你在 `th tools` 中为 `cron` 平台配置的工具集**——不是 CLI 默认值，也不是所有工具。
...
当任务上设置了 `enabled_toolsets` 时，它优先生效；否则 `th tools` 的 cron 平台配置生效；否则 TeamHermes 回退到内置默认值。这对成本控制很重要：在每个小型"获取新闻"任务中携带 `moa`、`browser`、`delegation` 会在每次 LLM 调用时膨胀工具 schema prompt。
