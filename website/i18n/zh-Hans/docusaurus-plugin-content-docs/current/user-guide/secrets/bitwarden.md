...
3. 每次 `th`（或 gateway，或 cron 任务）启动时，在加载 `~/.teamhermes/.env` 之后，TeamHermes 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
...
此后，每次调用 `th` 都会在启动时拉取最新 secret。进程中首次应用 secret 时，stderr 会显示一行摘要信息。

## CLI

| 命令 | 功能 |
|---|---|
| `th secrets bitwarden setup` | 交互式向导（安装二进制文件、提示输入令牌、选择项目、测试拉取） |
| `th secrets bitwarden status` | 显示配置、二进制版本及令牌是否存在 |
| `th secrets bitwarden sync` | 演习模式：立即拉取 secret 并显示将应用的内容 |
| `th secrets bitwarden sync --apply` | 拉取并导出到当前 shell 的环境中 |
| `th secrets bitwarden install` | 仅下载固定版本的 `bws` 二进制文件（无需认证） |
| `th secrets bitwarden disable` | 将 `enabled` 设为 `false`；保留令牌和项目 ID |
...
| `cache_ttl_seconds` | `300` | 进程内拉取结果的复用时长。设为 `0` 可禁用缓存。缓存按进程隔离；新的 `th` 调用从头开始。 |
...
| 现象 | 原因 | 修复方法 |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | 配置中已启用，但令牌已从 `.env` 中清除 | 重新运行 `th secrets bitwarden setup` |
