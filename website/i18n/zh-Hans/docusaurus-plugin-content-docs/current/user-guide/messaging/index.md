...
```bash
thm gateway              # 在前台运行
thm gateway setup        # 交互式配置消息平台
thm gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo thm gateway install --system   # 仅 Linux：安装开机启动的系统服务
thm gateway start        # 启动默认服务
thm gateway stop         # 停止默认服务
thm gateway status       # 检查默认服务状态
thm gateway status --system         # 仅 Linux：显式检查系统服务
```
...
thm gateway start                 # 启动服务
thm gateway stop                  # 停止服务
thm gateway status                # 检查状态
...
```bash
thm gateway install               # 安装为 launchd agent
thm gateway start                 # 启动服务
thm gateway stop                  # 停止服务
thm gateway status                # 检查状态
...
:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `thm gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::
