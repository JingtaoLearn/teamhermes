...
```bash
th gateway              # 前台运行
th gateway install      # 安装为用户服务
sudo th gateway install --system   # 仅 Linux：开机启动系统服务
```
...
当 TeamHermes 添加新命令时（例如执行 `th update` 后），重新生成 manifest 并更新你的 Slack 应用：
