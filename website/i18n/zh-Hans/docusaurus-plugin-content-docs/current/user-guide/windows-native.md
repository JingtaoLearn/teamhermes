...
:::tip 想用 WSL？
如果你更倾向于真正的 POSIX 环境（用于 dashboard 内嵌终端、`fork` 语义、Linux 风格文件监视器等），请参阅 **[Windows（WSL2）指南](./windows-wsl-quickstart.md)**。两者可以干净共存：原生数据存放在 `%LOCALAPPDATA%\hermes`，WSL 数据存放在 `~/.teamthm`。
:::
...
| `-InstallDir` | `%LOCALAPPDATA%\hermes\teamthm` | 覆盖代码存放位置 |

安装程序会自动重试不稳定的 git 拉取，并剥离下载的 `install.ps1` 内容中的 BOM，因此 HTTP 传输中携带的 UTF-8 BOM 不再会破坏 `[scriptblock]::Create((irm ...))` 形式。

### 桌面安装程序（备选方案）

也提供了一个轻量 GUI 安装程序——如果你更倾向于双击 `.exe` 而非打开 PowerShell，可以使用它。下载 TeamHermes Desktop，运行安装程序，首次启动时 GUI 会在后台调用 `install.ps1` 来配置 Python（通过 `uv`）、Node、PortableGit 以及下文描述的其余依赖引导流程。首次运行后，桌面应用与 PowerShell 安装的 `thm` CLI 共享同一个 `%LOCALAPPDATA%\hermes\teamthm` 安装目录和 `%USERPROFILE%\.teamthm` 数据目录——可以在 GUI 和 CLI 之间自由切换。
...
5. **将仓库克隆**到 `%LOCALAPPDATA%\hermes\teamthm` 并在其中创建 virtualenv。
...
| 功能 | 原生 Windows | WSL2 |
|---|---|---|
| CLI（`thm chat`、`thm setup`、`thm gateway` 等） | ✓ | ✓ |
| 交互式 TUI（`thm --tui`） | ✓ | ✓ |
...
Windows 上的 `thm gateway install` 使用**计划任务**，并以 Startup 文件夹作为回退——无需管理员权限。
...
```powershell
thm gateway status      # 合并视图：schtasks + Startup 文件夹 + 运行中的 PID
thm gateway start       # 立即启动计划任务
thm gateway stop        # 等效于优雅的 SIGTERM（通过 psutil 调用 TerminateProcess）
thm gateway restart
thm gateway uninstall   # 移除 schtasks 条目、Startup 快捷方式、pid 文件
```

`thm gateway status` 是幂等的——调用一千次也不会意外杀死 gateway。（PR #21561 之前它会静默地这样做，原因是 `os.kill(pid, 0)` 在 C 层与 `CTRL_C_EVENT` 发生碰撞——如果你想了解来龙去脉，请参阅下方"进程管理内部机制"。）
...
| 路径 | 内容 |
|---|---|
| `%LOCALAPPDATA%\hermes\teamthm\` | Git 检出 + venv。可安全执行 `Remove-Item -Recurse` 后重新安装。 |
...
| `%USERPROFILE%\.teamthm\` | 你的配置、认证、技能、会话、日志。**重装后保留。** |

这种分离是有意为之：`%LOCALAPPDATA%\hermes` 是可丢弃的基础设施（可以删除后用一行命令恢复）。`%USERPROFILE%\.teamthm` 是你的数据——配置、记忆、技能、会话历史——其结构与 Linux 安装完全相同。在机器间同步它，你的 TeamHermes 就随之迁移。
...
```powershell
Get-Command thm        # 应输出 C:\Users\<you>\AppData\Local\hermes\bin\hermes.cmd
thm --version
```

### 环境变量

TeamHermes 同时支持 `$env:X`（进程作用域）和用户环境变量（永久，在系统属性 → 环境变量中设置）。将 API key 放在 `%USERPROFILE%\.teamthm\.env` 中是标准做法——与 Linux 相同：
...
这是干净的卸载路径——移除 schtasks 条目、Startup 文件夹快捷方式、`hermes.cmd` 垫片，删除 `%LOCALAPPDATA%\hermes\teamthm\`，并从用户 PATH 中移除相关条目。它会保留 `%USERPROFILE%\.teamthm\`（你的配置、认证、技能、会话、日志），以防你需要重新安装。
...
`thm uninstall` CLI 子命令还能处理 schtasks 条目以不同任务名注册的情况（旧版安装）——它通过安装路径而非硬编码任务名来搜索。
...
**重启后 gateway 无法持续运行。**
运行 `thm gateway status`——它会合并 schtasks 条目、Startup 文件夹快捷方式（如有）和运行中的 PID。如果 schtasks 已注册但未运行，组策略可能阻止了 `ONLOGON` 触发器。运行 `schtasks /Query /TN HermesGateway /V /FO LIST` 查看任务失败原因，或通过卸载后使用 `HERMES_GATEWAY_FORCE_STARTUP=1` 重新安装来回退到 Startup 文件夹路径。
