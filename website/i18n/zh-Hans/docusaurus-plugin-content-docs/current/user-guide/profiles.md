...
```bash
thm profile create coder       # 创建 profile + "coder" 命令别名
...
你也可以稍后通过 `thm profile describe` 设置或自动生成描述——完整路由模型请参阅 [Kanban 指南](./features/kanban#auto-vs-manual-orchestration)。
...
### 粘性默认值（`thm profile use`）

```bash
thm profile use coder
thm chat                   # 现在指向 coder
thm tools                  # 配置 coder 的工具
thm profile use default    # 切换回默认
```

设置默认值后，普通 `thm` 命令将指向该 profile。类似于 `kubectl config use-context`。
...
- **`thm profile`**：显示当前 profile 名称、路径、模型、gateway 状态
...
`thm update` 拉取一次代码（共享），并自动将新的内置技能同步到**所有** profile：
...
```bash
thm profile list           # 显示所有 profile 及其状态
thm profile show coder     # 显示某个 profile 的详细信息
thm profile rename coder dev-bot   # 重命名（同步更新别名和服务）
thm profile export coder   # 导出为 coder.tar.gz
thm profile import coder.tar.gz   # 从归档文件导入
```
