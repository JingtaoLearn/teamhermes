...
在同一台机器上运行多个独立的 TeamHermes agent——每个 agent 拥有各自的配置、API 密钥、记忆、会话、技能和 gateway 状态。

## 什么是 profile？

profile 是一个独立的 TeamHermes 主目录。每个 profile 拥有自己的目录，其中包含各自的 `config.yaml`、`.env`、`SOUL.md`、记忆、会话、技能、cron 任务和状态数据库。profile 让你可以为不同用途运行独立的 agent——编程助手、个人机器人、研究 agent——而不会混淆 TeamHermes 状态。
...
```bash
th profile create coder       # 创建 profile + "coder" 命令别名
coder setup                       # 配置 API 密钥和模型
coder chat                        # 开始对话
```

就这些。`coder` 现在是拥有独立配置、记忆和状态的 TeamHermes profile。
...
你也可以稍后通过 `th profile describe` 设置或自动生成描述——完整路由模型请参阅 [Kanban 指南](./features/kanban#auto-vs-manual-orchestration)。
...
将当前 profile 的 `config.yaml`、`.env` 和 `SOUL.md` 复制到新 profile。API 密钥和模型相同，但会话和记忆是全新的。编辑 `~/.teamhermes/profiles/work/.env` 可使用不同的 API 密钥，编辑 `~/.teamhermes/profiles/work/SOUL.md` 可设置不同的人格。
...
别名支持所有 hermes 子命令——底层实际上是 `th -p <name>`。
...
th chat -p coder -q "hello"    # 可在任意位置使用
```

### 粘性默认值（`th profile use`）

```bash
th profile use coder
th chat                   # 现在指向 coder
th tools                  # 配置 coder 的工具
th profile use default    # 切换回默认
```

设置默认值后，普通 `th` 命令将指向该 profile。类似于 `kubectl config use-context`。
...
- **`th profile`**：显示当前 profile 名称、路径、模型、gateway 状态

## Profile vs 工作区 vs 沙箱

profile 常与工作区或沙箱混淆，但它们是不同的概念：

- **profile** 为 TeamHermes 提供独立的状态目录：`config.yaml`、`.env`、`SOUL.md`、会话、记忆、日志、cron 任务和 gateway 状态。
...
在 local 后端使用 `cwd: "."` 表示"TeamHermes 启动时所在的目录"，而非"profile 目录"。
...
`th update` 拉取一次代码（共享），并自动将新的内置技能同步到**所有** profile：
...
```bash
th profile list           # 显示所有 profile 及其状态
th profile show coder     # 显示某个 profile 的详细信息
th profile rename coder dev-bot   # 重命名（同步更新别名和服务）
th profile export coder   # 导出为 coder.tar.gz
th profile import coder.tar.gz   # 从归档文件导入
```
...
使用 `--yes` 跳过确认：`th profile delete coder --yes`

:::note
你无法删除默认 profile（`~/.teamhermes`）。如需删除所有内容，请使用 `th uninstall`。
:::
