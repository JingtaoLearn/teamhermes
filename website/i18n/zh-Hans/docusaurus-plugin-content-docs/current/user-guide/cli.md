...
# 使用指定提供商
thm chat --provider nous        # 使用 Nous Portal
thm chat --provider openrouter  # 强制使用 OpenRouter
...
在聊天中使用 `/title My Session Name` 为当前会话命名，或从命令行使用 `thm sessions rename <id> <title>`。使用 `thm sessions list` 浏览历史会话。
