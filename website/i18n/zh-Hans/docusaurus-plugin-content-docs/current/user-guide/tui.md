...
- **Node.js** ≥ 20 — TUI 作为从 Python CLI 启动的子进程运行。`th doctor` 会验证此项。
...
默认情况下，`th --tui` 每次启动都会开启新会话。若要自动重新连接到最近的 TUI 会话（在终端或 SSH 连接意外断开时很有用），可选择启用：
...
- **`⚠ YOLO`** — 每当 YOLO 模式开启时（`th --yolo`、`/yolo` 或 `HERMES_YOLO_MODE=1`）显示的可见警告。同一徽章也出现在启动 banner 中，确保你不会在未注意到的情况下启动自动审批会话。
...
默认情况下，TUI 会在进程内启动自己的 gateway，因此每个 TUI 实例是自包含的。如果你已有一个长期运行的 gateway（例如在 tmux 中运行 `th gateway run`，或 systemd / launchd 服务），可以将 TUI 指向该 gateway——TUI 将成为一个瘦客户端，与连接到同一 gateway 的所有其他界面（消息平台、Web 仪表板、其他 TUI 会话）共享状态。
...
不带 `--tui` 启动 `th` 将继续使用 classic CLI。若要让某台机器默认使用 TUI，在 shell profile 中设置 `HERMES_TUI=1`。若要回退，取消设置即可。
