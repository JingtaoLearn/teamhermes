...
| `HERMES_DASHBOARD_TUI` | 设为 `1` 以启用浏览器内 Chat 标签页（通过 PTY/WebSocket 嵌入 `thm --tui`） | *（未设置）* |
...
:::warning 权限模型
除非你在命令链中保留 `/init`（或等效的旧版 `docker/entrypoint.sh` shim，它会转发到 stage2 hook），否则不要覆盖镜像入口点。s6-overlay 的 `/init` 以 root 运行，以便在首次启动时对卷执行 chown，然后通过 `s6-setuidgid` 为每个受监管的服务**以及**主程序降权至 `thm` 用户。在官方镜像内以 root 启动 `thm gateway run` 默认会被拒绝，因为这可能在 `/opt/data` 中留下 root 所有的文件，导致后续 dashboard 或 gateway 启动失败。仅在你有意接受该风险时才设置 `HERMES_ALLOW_ROOT_GATEWAY=1`。
:::
...
```sh
thm profile create coder            # 注册 gateway-coder s6 槽
thm -p coder gateway start          # s6-svc -u  → 受监管的 gateway
thm -p coder gateway stop           # s6-svc -d  → 服务停止
thm -p coder gateway restart        # s6-svc -t  → 向 supervisor 发送 SIGTERM
thm profile delete coder            # 拆除 s6 槽
```
...
如果某个工具可能对大多数 TeamHermes Agent 用户有用，考虑将其贡献到上游，而不是在私有派生镜像中维护。在 [teamthm 仓库](https://github.com/NousResearch/hermes-agent)提交 issue 或 pull request，描述该工具及其使用场景。被纳入官方镜像的工具惠及所有用户，并避免了维护下游 fork 的开销。
