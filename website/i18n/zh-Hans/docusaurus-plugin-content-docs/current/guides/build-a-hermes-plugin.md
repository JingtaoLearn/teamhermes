...
| **额外技能来源**（自定义 GitHub 仓库、私有技能索引） | [技能](/user-guide/features/skills)——`th skills tap add <repo>` · [发布 tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
- `ctx.register_cli_command()` 注册 CLI 子命令（例如 `th my-plugin <subcommand>`）
...
- **未在配置中启用**——插件需要手动启用。运行 `th plugins enable <name>`（名称来自 `plugins list` 输出，嵌套布局下可能是 `<category>/<plugin>`）。
...
**关键特性：**
- 插件技能是**只读**的——它们不会进入 `~/.teamhermes/skills/`，也无法通过 `skill_manage` 编辑。
- 插件技能**不会**列在系统提示词的 `<available_skills>` 索引中——需要显式加载。
- 裸技能名称不受影响——命名空间防止与内置技能冲突。
- 代理加载插件技能时，会在前面添加一个捆绑上下文横幅，列出同一插件的兄弟技能。

:::tip 旧版模式
旧的 `shutil.copy2` 模式（将技能复制到 `~/.teamhermes/skills/`）仍然有效，但存在与内置技能名称冲突的风险。新插件请优先使用 `ctx.register_skill()`。
:::
...
用户运行 `th plugins install` 时，会**交互式提示**输入任何缺失的 `requires_env` 变量。值会自动保存到 `.env`。
...
不加 `override=True` 时，注册表会拒绝任何会遮蔽来自不同 toolset 的已有工具的注册——这防止了意外覆盖。覆盖操作会以 INFO 级别记录日志，可在 `~/.teamhermes/logs/agent.log` 中审计。插件在内置工具之后加载，因此注册顺序是正确的：你的处理器会替换内置处理器。
...
注册后，用户可以运行 `th my-plugin status`、`th my-plugin config` 等命令。
...
def register(ctx):
    ctx.register_platform(
        ...
        # 在 `th gateway status` 中显示，无需 SDK 实例化。
...
将清单和处理器放入 `~/.teamhermes/hooks/<name>/`：
