...
>
> 已暂停。说"重新开启"或执行 `th cron resume abc123` 可恢复。
...
```bash
th cron create "every 5m"        # 间隔
th cron create "every 2h"
th cron create "0 9 * * *"       # 标准 cron：每天上午 9 点
th cron create "30m"             # 单次：30 分钟后运行一次
```
...
```bash
th cron list                                    # 查看所有任务
th cron pause <job_id>                          # 停止触发，保留定义
th cron resume <job_id>
th cron edit <job_id> --schedule "every 10m"    # 调整频率
th cron edit <job_id> --agent                   # 切换为 LLM 模式
th cron edit <job_id> --no-agent --script …     # 切换回无 agent 模式
th cron remove <job_id>                         # 删除任务
```
