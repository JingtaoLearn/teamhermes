...
def render_skill_page(
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `thm skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "TeamHermes also syncs bundled skills on `thm update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.teamhermes/skills/` tree, it is still shipped with TeamHermes; restore it with `thm skills reset <name> --restore`.",
        "",
