...
def render_skill_page(
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `th skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with TeamHermes Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "TeamHermes ships with a large built-in skill library copied into `~/.teamhermes/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "TeamHermes also syncs bundled skills on `th update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.teamhermes/skills/` tree, it is still shipped with TeamHermes; restore it with `th skills reset <name> --restore`.",
        "",
