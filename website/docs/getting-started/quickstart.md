...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with TeamHermes Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working TeamHermes setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [TeamHermes Agent Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="TeamHermes Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up TeamHermes for a team, bot, or always-on workflow
...
|---|---|---|
| I just want TeamHermes working on my machine | `thm setup` | Run a real chat and verify it responds |
| I already know my provider | `thm model` | Save the config, then start chatting |
| I want a bot or always-on setup | `thm  gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `thm model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `thm model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if TeamHermes cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install TeamHermes Agent
...
The single most important setup step. Use `thm model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `thm model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `thm model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `thm model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `thm model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `thm model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `thm model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `thm model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `thm model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `thm model` (requires `copilot` CLI + `copilot login`) |
...
TeamHermes Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `thm model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

TeamHermes separates secrets from normal config:

- **Secrets and tokens** → `~/.teamhermes/.env`
- **Non-secret settings** → `~/.teamhermes/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
thm  config set model anthropic/claude-opus-4.6
thm  config set terminal.backend docker
thm  config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
thm            # classic CLI
thm  --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
TeamHermes ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `thm` vs `thm  --tui`.
:::
...
- TeamHermes replies without error
...
```bash
thm  --continue    # Resume the most recent session
thm  -c            # Short form
```
...
```bash
thm  gateway setup    # Interactive platform configuration
```
...
- `thm tools` — tune tool access per platform
- `thm skills` — browse and install reusable workflows
...
```bash
thm  config set terminal.backend docker    # Docker isolation
thm  config set terminal.backend ssh       # Remote server
```
...
# ~/.teamhermes/hermes-agent on Linux/macOS or %LOCALAPPDATA%\hermes\hermes-agent on Windows):
...
(If you installed without `[all]`, run `cd ~/.teamhermes/hermes-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| TeamHermes opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `thm model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `thm  gateway setup` and check `thm  gateway status` |
| `thm  --continue` can't find old session | Switched profiles or session never saved | Check `thm  sessions list` and confirm you're in the right profile |
...
| `thm doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
| `thm tools` | Configure which tools are enabled per platform |
| `thm setup` | Full setup wizard (configures everything at once) |
| `thm doctor` | Diagnose issues |
| `thm update` | Update to latest version |
| `thm  gateway` | Start the messaging gateway |
| `thm  --continue` | Resume last session |
