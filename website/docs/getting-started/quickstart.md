...
|---|---|---|
| I just want TeamHermes working on my machine | `th setup` | Run a real chat and verify it responds |
| I already know my provider | `th model` | Save the config, then start chatting |
| I want a bot or always-on setup | `th gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `th model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `th model` first | Add routing and fallback only after the base chat works |
...
th postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
The single most important setup step. Use `th model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `th model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `th model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `th model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `th model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `th model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `th model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `th model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `th model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `th model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `th model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::
...
```bash
th config set model anthropic/claude-opus-4.6
th config set terminal.backend docker
th config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
hermes            # classic CLI
th --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
TeamHermes ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `th` vs `th --tui`.
:::
...
```bash
th --continue    # Resume the most recent session
th -c            # Short form
```
...
```bash
th gateway setup    # Interactive platform configuration
```
...
- `th tools` — tune tool access per platform
- `th skills` — browse and install reusable workflows
...
```bash
th config set terminal.backend docker    # Docker isolation
th config set terminal.backend ssh       # Remote server
```
...
|---|---|---|
| TeamHermes opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `th model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `th gateway setup` and check `th gateway status` |
| `th --continue` can't find old session | Switched profiles or session never saved | Check `th sessions list` and confirm you're in the right profile |
...
| `th doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
|---------|-------------|
| `th` | Start chatting |
| `th model` | Choose your LLM provider and model |
| `th tools` | Configure which tools are enabled per platform |
| `th setup` | Full setup wizard (configures everything at once) |
| `th doctor` | Diagnose issues |
| `th update` | Update to latest version |
| `th gateway` | Start the messaging gateway |
| `th --continue` | Resume last session |
