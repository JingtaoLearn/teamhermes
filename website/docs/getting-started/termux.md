...
- links `th` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `th` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/th" "$PREFIX/bin/th"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `th` command persist across new shells without re-activating the venv every time.
...
### `th doctor` says ripgrep or Node is missing
