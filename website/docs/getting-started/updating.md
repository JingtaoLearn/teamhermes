...
thm  update            # runs pip install --upgrade teamthm
```

Or manually:

```bash
pip install --upgrade teamthm    # or: uv pip install --upgrade teamthm
```
...
3. **Post-pull syntax validation + auto-rollback** — after the pull, TeamHermes compiles the eight critical files every `thm` invocation imports at startup. If any fails to parse (e.g. an orphan merge-conflict marker, an accidentally truncated file), TeamHermes runs `git reset --hard <pre-pull-sha>` to roll the install back so your shell stays bootable. Re-run `thm update` once the upstream fix lands.
...
```bash
thm  update --branch release-candidate
thm  update --check --branch experimental   # preview behindness only
```
...
On Windows, `thm update` will refuse to run if it detects another `hermes.exe` process holding the venv's entry-point executable open — most commonly the TeamHermes Desktop app's spawned backend, an open `thm` REPL in another terminal, or a running gateway:
...
✗ Another hermes.exe is running:
  ...
  Close TeamHermes Desktop, exit any open `thm` REPLs, and
  stop the gateway (`thm  gateway stop`) before retrying.
...
3. `thm  --version` — confirm the version bumped as expected
4. If you use the gateway: `thm  gateway status`
...
thm  config check
thm  config migrate   # Interactively add any missing options
```
...
nix flake update teamthm

# Or rebuild with the latest
nix profile upgrade teamthm
```
...
```bash
pip uninstall teamthm
rm -rf ~/.teamthm            # Optional — keep if you plan to reinstall
```
