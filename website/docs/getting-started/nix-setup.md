...
description: "Install and deploy TeamHermes Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

TeamHermes Agent ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`thm setup`, `thm  gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage thm the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `thm`, `teamthm`, and `thm-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `thm setup` walks you through provider selection, `thm  gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.teamhermes/`.
...
That's it. `nixos-rebuild switch` creates the `thm` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o thm /dev/stdin /var/lib/hermes/env
```
...
Setting `addToSystemPackages = true` does two things: puts the `thm` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `thm` in your shell creates a separate `~/.teamhermes/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `thm` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `thm chat`, `thm  sessions list`, `thm version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the thm  codebase, set `HERMES_DEV=1` to bypass container routing and run the local checkout directly

Set `container.hostUsers` to create a `~/.teamthm` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
Users listed in `hostUsers` are automatically added to the `thm` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo thm  chat` manually.
:::
...
systemctl status teamthm

# Watch logs (Ctrl+C to stop)
journalctl -u teamthm -f

# If addToSystemPackages is true, test the CLI
thm  version
thm  config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, TeamHermes defaults to OpenRouter.
:::
...
| Override the thm  package | `package` | `inputs.teamhermes-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). TeamHermes reads this file on every startup, so changes take effect with a `systemctl restart teamthm` — no container recreation needed.
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). TeamHermes looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: TeamHermes loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.teamhermes-agent.stateDir}/.teamhermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
Set `auth = "oauth"` for servers using OAuth 2.1. TeamHermes implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, TeamHermes prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u thm` (native):

```bash
# Container mode
docker exec -it teamthm \
  thm  mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u thm HERMES_HOME=/var/lib/hermes/.teamthm \
  thm  mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
# Ensure: chown thm:thm, chmod 0600
```
...
When thm  runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `thm setup` | Config is declarative — edit `settings` in your Nix config |
| `thm config edit` | Config is generated from `settings` |
| `thm config set <key> <value>` | Config is generated from `settings` |
| `thm  gateway install` | The systemd service is managed by NixOS |
| `thm  gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`HERMES_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it teamthm thm  config set ...` is also blocked)
...
When container mode is enabled, thm  runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-teamhermes-0.1.0  ──►  /nix/store/... (ro)
~/.teamthm -> /var/lib/hermes/.teamthm       (symlink bridge, per hostUsers)
...
|---|---|---|---|---|
| `systemctl restart teamthm` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the thm  package itself do **not** trigger recreation.
...
The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current thm  package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `thm plugins install` needed.
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. TeamHermes discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
The package's `site-packages` is added to PYTHONPATH in the thm  wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in teamthm's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.
...
:::note
A build-time collision check prevents plugin packages from shadowing core thm  dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
| Check | What it tests |
|---|---|
| `package-contents` | `thm` and `teamthm` binaries exist and `thm version` runs |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the teamthm service |
| `package` | `package` | `teamthm` | The teamthm package to use |
| `user` | `str` | `"thm"` | System user |
| `group` | `str` | `"thm"` | System group |
...
| `addToSystemPackages` | `bool` | `false` | Add `thm` CLI to system PATH and set `HERMES_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `thm  gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the thm  user's per-user profile so terminal commands, skills, and cron jobs all see them |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.teamthm` symlink to the service stateDir and are auto-added to the `thm` group |
...
```
/var/lib/hermes/                     # stateDir (owned by thm:thm, 0750)
├── .teamhermes/                         # HERMES_HOME
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | TeamHermes binary + all Nix deps |
...
cd /etc/nixos && nix flake update teamthm
...
```bash
systemctl status teamthm
docker ps -a --filter name=teamthm
docker inspect teamthm --format='{{.State.Status}}'
docker exec -it teamthm bash
docker exec teamthm readlink /data/current-package
docker exec teamthm cat /data/.container-identity
```
...
```bash
sudo systemctl stop teamthm
docker rm -f teamthm
sudo rm /var/lib/hermes/.container-identity
sudo systemctl start teamthm
```
...
```bash
# Native mode
sudo -u thm cat /var/lib/hermes/.teamhermes/.env

# Container mode
docker exec teamthm cat /data/.teamhermes/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec teamthm readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u teamthm` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `thm version` shows old version | Container not restarted | `systemctl restart teamthm` |
| Permission denied on `/var/lib/hermes` | State dir is `0750 thm:thm` | Use `docker exec` or `sudo -u thm` |
| `nix-collect-garbage` removed thm | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "teamthm"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user thm` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart teamthm` |
