...
description: "How TeamHermes builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

TeamHermes deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.teamhermes.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.teamhermes/SOUL.md)
You are TeamHermes, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.teamhermes/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are TeamHermes Agent, an intelligent AI assistant created by Nous Research.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .teamhermes.md / HERMES.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `.teamhermes.md`, `HERMES.md` | CWD up to git root | TeamHermes-native project config |
...
- **YAML frontmatter stripped** — `.teamhermes.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, TeamHermes concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.teamhermes.md` / `HERMES.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs TeamHermes already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.teamhermes/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.teamhermes/MEMORY.md` and `~/.teamhermes/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.teamhermes.md`, `HERMES.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking TeamHermes.
...
- if you want to change how TeamHermes assembles prompts for everyone, change Python and treat it as a code contribution
