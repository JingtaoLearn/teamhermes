...
TeamHermes Agent uses a SQLite database (`~/.teamhermes/state.db`) to persist session
...
Multiple thm  processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.teamhermes/state.db
...
Default path: `~/.teamhermes/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.teamhermes/` by default, or the value of `HERMES_HOME` environment variable.
