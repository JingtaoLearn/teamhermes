...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`th send` CLI](/guides/pipe-script-output) wrapping the same tool for shell scripts
...
- `th gateway start` / `th gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.teamhermes/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `th gateway stop` stops only the current profile's gateway. `th gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
