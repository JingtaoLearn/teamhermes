...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `th config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
| `th config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `th status` | Shows plugin platforms with `(plugin)` tag |
| `th gateway setup` | Plugin platforms appear in setup menu |
| `th tools` / `th skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.teamhermes/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `th gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `th cron run` separate from `th gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `th config`
