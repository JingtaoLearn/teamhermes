...
:::tip
`hermes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `th setup`.
:::
...
7. `th model` menu includes the provider
8. `th setup` wizard delegates to `main.py` automatically
...
- Provider-specific `th model` menu entries with bespoke auth flows
...
A provider is not discoverable until it shows up in the interactive `th model` flow.
...
:::tip
`hermes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `th model` and `th setup` automatically.
:::
...
### 7. Updating `th model` but not `th setup`
