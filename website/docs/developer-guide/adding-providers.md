...
description: "How to add a new inference provider to TeamHermes Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

TeamHermes can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `thm model` menu entries
...
:::tip
`hermes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `thm setup`.
:::
...
7. `thm model` menu includes the provider
8. `thm setup` wizard delegates to `main.py` automatically
...
- Provider-specific `thm model` menu entries with bespoke auth flows
...
- What env vars should TeamHermes check, and in what priority order?
...
Be careful with API-key precedence. TeamHermes already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `hermes_cli/main.py`

A provider is not discoverable until it shows up in the interactive `thm model` flow.
...
:::tip
`hermes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `thm model` and `thm setup` automatically.
:::
...
When you add a native provider, double-check that TeamHermes is only sending fields that provider actually understands.
...
### 7. Updating `thm model` but not `thm setup`
