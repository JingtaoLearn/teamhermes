...
The ACP adapter wraps TeamHermes' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
thm  acp / thm-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'hermes-agent[acp]==<version>' thm-acp`, pointed at the `hermes-agent` PyPI release.
...
- `allow_once` -> TeamHermes `once`
- `allow_always` -> TeamHermes `always`
- reject options -> TeamHermes `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps TeamHermes tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["thm-acp"])
...
Instead it reuses TeamHermes' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured TeamHermes provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run registry clients can open TeamHermes' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.teamhermes/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `thm-acp` toolset definition
- `hermes_cli/main.py` — `thm  acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `thm-acp` script
