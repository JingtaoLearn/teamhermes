...
description: "How TeamHermes detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in TeamHermes' browser tooling:
...
One `asyncio.Task` running in a background daemon thread per TeamHermes `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `thm-acp`, `hermes-api-server`, and core toolsets (gated on CDP reachability)
