...
| `hermes_cli/cron.py` | CLI `th cron` subcommands |
...
Jobs are stored in `~/.teamhermes/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `th cron` commands are run or during active CLI sessions.
...
```python
# ~/.teamhermes/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.teamhermes/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `th cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `th cron` CLI provides direct job management:

```bash
th cron list                    # Show all jobs
th cron create                  # Interactive job creation (alias: add)
th cron edit <job_id>           # Edit job configuration
th cron pause <job_id>          # Pause a running job
th cron resume <job_id>         # Resume a paused job
th cron run <job_id>            # Trigger immediate execution
th cron remove <job_id>         # Delete a job
```
