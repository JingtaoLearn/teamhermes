...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept TeamHermes already has applies. The plugin never sees a
...
teamhermes itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — TeamHermes provider id (e.g. "openrouter")
  ...
  entries:
    my-plugin:
      llm:
        ...
        # `thm model` and config.yaml model.provider).
        ...
        # plugin sends — TeamHermes does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.teamhermes/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through TeamHermes' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing TeamHermes subsystem:
