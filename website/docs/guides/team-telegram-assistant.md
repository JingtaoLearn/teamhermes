...
This tutorial walks you through setting up a Telegram bot powered by TeamHermes Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **TeamHermes Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.teamhermes/.env`

:::tip
A $5/month VPS is plenty for running the gateway. TeamHermes itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team TeamHermes Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by TeamHermes Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.teamhermes/.env`:
...
```
[Gateway] Starting TeamHermes Gateway...
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `thm  gateway install` to update the plist.
:::
...
```bash
thm  gateway stop && thm  gateway start
```
...
**Option 2:** Set it manually in `~/.teamhermes/.env`:
...
Control how much detail the bot shows when using tools. In `~/.teamhermes/config.yaml`:
...
Customize how the bot communicates by editing `~/.teamhermes/SOUL.md`:

For a full guide, see [Use SOUL.md with TeamHermes](/guides/use-soul-with-hermes).
...
```bash
# From the CLI
thm  cron list          # View all scheduled jobs
thm  cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hermes-gateway` (Linux) or `~/.teamhermes/logs/gateway.log` (macOS) |
| Cron job output | `~/.teamhermes/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.teamhermes/cron/jobs.json` |
| Pairing data | `~/.teamhermes/pairing/` |
| Session history | `~/.teamhermes/sessions/` |
