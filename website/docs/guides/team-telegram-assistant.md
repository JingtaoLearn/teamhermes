...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `th gateway install` to update the plist.
:::
...
```bash
th gateway stop && th gateway start
```
...
th pairing revoke telegram 987654321
...
```bash
# From the CLI
th cron list          # View all scheduled jobs
th cron status        # Check if scheduler is running
