...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `th send`."
---

# Pipe Script Output to Messaging Platforms

`th send` is a small, scriptable CLI that pushes a message to any
messaging platform TeamHermes is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | th send --to slack:#builds`)
...
th send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
th send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
th send --list

# Filter by platform
th send --list telegram
```
...
Any platform TeamHermes ships adapters for works as a target:
...
`th send` resolves the message body in this order:

1. **Positional argument** — `th send --to telegram "hi"`
2. **`--file PATH`** — `th send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | th send --to telegram`

When stdin is a TTY (no pipe), TeamHermes does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  th send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `th send` reuses your TeamHermes config, the same script works on
any host where TeamHermes is installed — no need to export bot tokens into
...
`th send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  th send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | th send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.teamhermes/bin/th send \
...
```bash
./train.py --epochs 200 && \
  th send --to telegram "training done" || \
  th send --to telegram "training failed (exit $?)"
```
...
th send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(th send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `th send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `th send` calls
the platform's REST endpoint directly using credentials from
`~/.teamhermes/.env` and `~/.teamhermes/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `th gateway start` and retry.
...
The listing is built from `~/.teamhermes/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`th gateway
...
| Approach | Multi-platform | Reuses TeamHermes creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `th send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`th send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `th send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `th send` shares with cron delivery.
