...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `thm  send`."
---

# Pipe Script Output to Messaging Platforms

`thm  send` is a small, scriptable CLI that pushes a message to any
...
- Piping any tool's output anywhere (`make | thm  send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `thm 
...
thm  send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
thm  send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
thm  send --list

# Filter by platform
thm  send --list telegram
```
...
`thm  send` resolves the message body in this order:

1. **Positional argument** — `thm  send --to telegram "hi"`
2. **`--file PATH`** — `thm  send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | thm  send --to telegram`
...
if [ "$ram_pct" -ge 85 ]; then
  thm  send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `thm  send` reuses your TeamHermes config, the same script works on
...
`thm  send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  thm  send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | thm  send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.teamhermes/bin/hermes send \
...
```bash
./train.py --epochs 200 && \
  thm  send --to telegram "training done" || \
  thm  send --to telegram "training failed (exit $?)"
```
...
thm  send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(thm  send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `thm  send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `thm  send` calls
...
pointing at the gateway; start it with `thm  gateway start` and retry.
...
"no channels discovered yet", start the gateway once (`thm  gateway
...
|----------|----------------|---------------------|---------------|----------|
| `thm  send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`thm  send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `thm  send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `thm  send` shares with cron delivery.
