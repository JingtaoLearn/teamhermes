---
sidebar_position: 6
title: "Use MCP with TeamHermes"
description: "A practical guide to connecting MCP servers to TeamHermes Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with TeamHermes

This guide shows how to actually use MCP with TeamHermes Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native TeamHermes tool
- you want TeamHermes to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect TeamHermes to internal APIs, databases, or company systems without modifying TeamHermes core

Do not use MCP when:
- a built-in TeamHermes tool already solves the job well
...
- TeamHermes remains the agent
- MCP servers contribute tools
- TeamHermes discovers those tools at startup or reload time
...
If you installed TeamHermes with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- TeamHermes banner/status should show MCP integration when configured
- ask TeamHermes what tools it has available
...
## WSL2: bridge TeamHermes in WSL to Windows Chrome

This is the practical setup when:

- TeamHermes runs inside WSL2
...
In this setup, TeamHermes does **not** connect to Chrome directly. Instead:

- TeamHermes runs in WSL
- TeamHermes starts a local stdio MCP server
...
```text
TeamHermes (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- TeamHermes stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on TeamHermes core browser transport
...
```bash
thm  mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
thm  mcp test chrome-devtools-win
```

Then start a fresh TeamHermes session or run:
...
Once loaded, TeamHermes can use the MCP-prefixed browser tools directly. For example:
...
If TeamHermes runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start TeamHermes from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start TeamHermes from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in TeamHermes:
...
2. TeamHermes-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So TeamHermes will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want TeamHermes to reason over a bounded workspace.
...
Now TeamHermes can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing TeamHermes core.
...
Because TeamHermes now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
