...
3. xAI redirects back to Hermes and the tokens are saved to `~/.teamhermes/auth.json`.
4. From then on, Hermes refreshes the access token in the background — you stay signed in until you `thm auth remove xai-oauth` or revoke access from your xAI account settings.
