...
Copy-paste recipes for common automation patterns. Each template uses TeamHermes's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`thm  webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `thm  webhook subscribe`) |
...
```bash
thm  webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
thm  cron create "0 6 * * *" \
  "Run a dependency security audit on the teamthm project.
...
```bash
thm  webhook subscribe deploy-verify \
  --events "deployment" \
...
```python title="~/.teamhermes/scripts/check-uptime.py"
...
```bash
thm  webhook subscribe auto-port \
  --events "pull_request" \
...
thm  cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the teamthm codebase.
