...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`th webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `th webhook subscribe`) |
...
```bash
th webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
th webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
th webhook subscribe auto-port \
  --events "pull_request" \
