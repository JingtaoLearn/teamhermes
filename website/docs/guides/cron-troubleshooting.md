...
If you're expecting jobs to fire automatically, you need a running gateway (`th gateway` for foreground, or `th gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `th cron tick`.
...
```bash
date
th cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.teamhermes/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.teamhermes/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.teamhermes/.env` |
...
| `local` | Write access to `~/.teamhermes/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `th cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `th skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `th skills list`.
...
2. `~/.teamhermes/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `th cron list`
...
th cron edit <job_id> --script ~/.teamhermes/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `th skills install <skill-name>`.
...
Jobs are stored in `~/.teamhermes/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.teamhermes/cron/jobs.json
chmod 600 ~/.teamhermes/cron/jobs.json   # Your user should own it
```
...
```bash
th cron list                    # Show all jobs, states, next_run times
th cron run <job_id>            # Schedule for next tick (for testing)
th cron edit <job_id>           # Fix configuration issues
th logs                         # View recent TeamHermes logs
th skills list                  # Verify installed skills
```
...
1. Run the job with `th cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.teamhermes/logs/agent.log` for scheduler messages and `~/.teamhermes/logs/errors.log` for warnings
