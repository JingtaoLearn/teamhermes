...
# 3. Verify
thm  cron list
thm  cron run <job_id>    # fire it once to test
```
...
```bash
thm  cron create "every 5m"        # interval
thm  cron create "every 2h"
thm  cron create "0 9 * * *"       # standard cron: 9am daily
thm  cron create "30m"             # one-shot: run once in 30 minutes
```
...
```bash
thm  cron list                                    # see all jobs
thm  cron pause <job_id>                          # stop firing, keep definition
thm  cron resume <job_id>
thm  cron edit <job_id> --schedule "every 10m"    # adjust cadence
thm  cron edit <job_id> --agent                   # flip to LLM mode
thm  cron edit <job_id> --no-agent --script …     # flip back
thm  cron remove <job_id>                         # delete it
```
