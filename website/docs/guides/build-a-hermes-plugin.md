...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `thm skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `thm  my-plugin <subcommand>`)
...
- **Not enabled in config** — plugins are opt-in. Run `thm plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
...
When users run `thm plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
Plugins can add their own `thm  <plugin>` subcommand tree:
...
After registration, users can run `thm  my-plugin status`, `thm  my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `thm  name` in a terminal |
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `thm  gateway status` without SDK instantiation.
...
# Plugin auto-discovered on next thm  startup
```
