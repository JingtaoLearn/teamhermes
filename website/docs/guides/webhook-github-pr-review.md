...
- TeamHermes Agent installed and running (`th gateway`)
...
:::note
`th webhook test <name>` only works for **dynamic subscriptions** created with `th webhook subscribe`. It does not read routes from `config.yaml`.
:::
