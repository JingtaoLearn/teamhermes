...
One command — `thm setup --portal` — and you're ready to `thm chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
TeamHermes also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `thm  --tui` — see the [TUI](tui.md) guide.
:::
...
thm  chat --model "anthropic/claude-sonnet-4"

# With a specific provider
thm  chat --provider nous        # Use Nous Portal
thm  chat --provider openrouter  # Force OpenRouter
...
thm  --continue             # Resume the most recent CLI session (-c)
thm  --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
thm  chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
thm  -w                         # Interactive mode in worktree
thm  -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the TeamHermes CLI layout showing the banner, conversation area, and fixed input prompt." />
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `thm  --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`thm  -c` or `thm  --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
```bash
thm  --continue                          # Resume the most recent CLI session
thm  -c                                  # Short form
thm  -c "my project"                     # Resume a named session (latest in lineage)
thm  --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
thm  --resume "refactoring auth"         # Resume by title
thm  -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `thm  sessions rename <id> <title>` from the command line. Use `thm  sessions list` to browse past sessions.
