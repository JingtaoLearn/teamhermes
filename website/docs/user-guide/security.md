...
TeamHermes Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, TeamHermes checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.teamhermes/config.yaml`:
...
| `timeout` | `60` | Seconds TeamHermes waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `th --yolo` or `th chat --yolo`
...
When YOLO is active, TeamHermes shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that TeamHermes refuses to run them **regardless** of:
...
Configure the timeout in `~/.teamhermes/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.teamhermes/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.teamhermes/.env` | Overwrite sensitive file via redirection |
...
Commands approved with "always" are saved to `~/.teamhermes/config.yaml`:
...
:::tip
Use `th config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, TeamHermes controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.teamhermes/.env`:
...
For more flexible authorization, TeamHermes includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `th pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.teamhermes/config.yaml`:
...
th pairing approve telegram ABC12DEF

# Revoke a user's access
th pairing revoke telegram 123456789

# Clear all pending codes
th pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.teamhermes/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, TeamHermes applies strict security hardening to every container.
...
Container resources are configurable in `~/.teamhermes/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.teamhermes/sandboxes/docker/<task_id>/`
...
When loaded, TeamHermes checks if these files exist in the active profile's `HERMES_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit TeamHermes infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- TeamHermes infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
TeamHermes integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run TeamHermes under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.teamhermes/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.teamhermes/logs/` for unauthorized access attempts
10. **Keep updated** — run `th update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.teamhermes/.env
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.teamhermes/.env`:
...
TeamHermes ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hermes_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `th doctor` for the full remediation.
- **`th doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. TeamHermes installs these **lazily** on first use rather than eagerly under `hermes-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `th tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `th tools`.
