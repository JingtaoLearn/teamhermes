...
|---|---|---|
| CLI (`thm chat`, `thm setup`, `thm  gateway`, …) | ✓ | ✓ |
| Interactive TUI (`thm  --tui`) | ✓ | ✓ |
...
`thm  gateway install` on Windows uses **Scheduled Tasks** with a Startup-folder fallback — no admin required.
...
```powershell
thm  gateway status      # Merged view: schtasks + Startup folder + running PID
thm  gateway start       # Starts the scheduled task now
thm  gateway stop        # Graceful SIGTERM equivalent (TerminateProcess via psutil)
thm  gateway restart
thm  gateway uninstall   # Removes schtasks entry, Startup shortcut, pid file
```

`thm  gateway status` is idempotent — call it a thousand times in a row and it will never accidentally kill the gateway. (Pre-PR #21561 it silently did, via `os.kill(pid, 0)` colliding with `CTRL_C_EVENT` at the C level — see "process management internals" below if you care about the story.)
...
```powershell
Get-Command thm        # should print C:\Users\<you>\AppData\Local\hermes\bin\hermes.cmd
thm  --version
```
...
The `thm  uninstall` CLI subcommand also handles the case where the schtasks entry was registered under a different task name (older installs) — it searches by install path rather than by hardcoded task name.
