...
| `--tui` | off | Expose the in-browser Chat tab (embedded `thm  --tui` via PTY/WebSocket). Alternatively set `HERMES_DASHBOARD_TUI=1`. |

```bash
# Custom port
thm  dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
thm  dashboard --host 0.0.0.0

# Start without opening browser
thm  dashboard --no-open
...
The default `teamhermes` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'teamhermes[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install teamhermes[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- Node.js (same requirement as `thm  --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'teamhermes[web,pty]'`, or `[all]` covers both)
