...
**Tool Search** is TeamHermes' opt-in progressive-disclosure layer for that
...
:::info Built-in TeamHermes tools never defer
The tools that make up TeamHermes' core capability set (`terminal`,
...
When the model invokes `tool_call`, TeamHermes **unwraps the bridge** and
...
- **No JS sandbox.** TeamHermes uses the simpler "structured tools" mode
