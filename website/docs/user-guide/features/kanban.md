...
The board has two front doors, both backed by the same `~/.teamhermes/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `th kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `th kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
called `default` (DB at `~/.teamhermes/kanban.db` for back-compat). Users who
only want one stream of work never need to know about boards; the feature
is opt-in.

Per-board isolation is absolute:

- Separate SQLite DB per board (`~/.teamhermes/kanban/boards/<slug>/kanban.db`).
...
th kanban boards list

# Create a new board.
th kanban boards create atm10-server \
    --name "ATM10 Server" \
...
th kanban --board atm10-server list
th kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
th kanban boards switch atm10-server
th kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
th kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.teamhermes/kanban/current` — the slug persisted by `th kanban
   boards switch`.
4. `default`.
...
`th dashboard` → Kanban tab shows a board switcher at the top as soon
...
th kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `th kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `th gateway
...
until one comes up — `th kanban create` warns about this at creation
time.

Running `th kanban daemon` as a separate process is **deprecated**;
...
th kanban create "nightly ops review" \
    --assignee ops \
...
```bash
th kanban complete t_abc t_def t_hij --result "batch wrap"
th kanban archive  t_abc t_def t_hij
th kanban unblock  t_abc t_def
th kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `th kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `th kanban` CLI.
...
### Why tools instead of shelling to `th kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `th kanban complete` *inside* the container, where `th` isn't installed and `~/.teamhermes/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.teamhermes/kanban.db` regardless of terminal backend.
...
```bash
th -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
th -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
th kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

th kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
th -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
th -p orchestrator skills reset kanban-orchestrator --restore
```
...
```bash
th kanban init      # one-time: create kanban.db if not already present
th dashboard        # "Kanban" tab appears in the nav, after "Skills"
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `th kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `th kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `th profile create --description "..."`, `th profile describe <name> --text "..."`, `th profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).

Config knobs (all under `kanban:` in `~/.teamhermes/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `th profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.teamhermes/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
```
th kanban init                                     # create kanban.db + print daemon hint
th kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
th kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
th kanban show <id> [--json]
th kanban assign <id> <profile>                    # or 'none' to unassign
th kanban link <parent_id> <child_id>
th kanban unlink <parent_id> <child_id>
th kanban claim <id> [--ttl SECONDS]
th kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
th kanban complete <id>... [--result "..."]
th kanban block <id> "<reason>" [--ids <id>...]
th kanban unblock <id>...
th kanban archive <id>...

th kanban tail <id>                                # follow a single task's event stream
th kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
th kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
th kanban runs <id> [--json]                       # attempt history (one row per run)
th kanban assignees [--json]                       # profiles on disk + per-assignee task counts
th kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
th kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `th gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
th kanban stats [--json]                           # per-status + per-assignee counts
th kanban log <id> [--tail BYTES]                  # worker log from ~/.teamhermes/kanban/logs/
th kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
th kanban notify-list [<id>] [--json]
th kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
th kanban context <id>                             # what a worker sees
th kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
th kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
th kanban create "nightly backup audit" \
...
Every `th kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `th chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `th kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `th kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.teamhermes/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`th kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `th kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
```bash
th kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
th kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
th kanban notify-list
th kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
th kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
th kanban runs t_abcd
...
**Bulk close caveat.** `th kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`th kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `th kanban heartbeat $TASK` to signal liveness during long operations. |
...
`th kanban tail <id>` shows these for a single task. `th kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.teamhermes/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.
