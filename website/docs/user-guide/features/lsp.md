...
TeamHermes runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. TeamHermes
...
1. TeamHermes captures a baseline of current diagnostics for the file.
...
…). TeamHermes auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — TeamHermes installs both packages together when you
run `thm  lsp install typescript` or auto-install fires on first
use.

## CLI

```
thm  lsp status          # service state + per-server install status
thm  lsp list            # registry, optionally --installed-only
thm  lsp install <id>    # eagerly install one server
thm  lsp install-all     # try every server with a known recipe
thm  lsp restart         # tear down running clients
thm  lsp which <id>      # print resolved binary path
```

`thm  lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, TeamHermes installs binaries into
...
shared location — the staging dir is fully TeamHermes-owned and is
...
Servers are kept alive for the life of the TeamHermes process. There's
...
**`thm  lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERMES_HOME>/lsp/bin/`. Run
`thm  lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `thm  lsp status`**
...
When `thm  lsp status` shows a `Backend warnings` section, install
...
`~/.teamhermes/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.teamhermes/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `thm  lsp restart` to clear the set;
