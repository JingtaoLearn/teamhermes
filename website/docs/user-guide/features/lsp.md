...
run `th lsp install typescript` or auto-install fires on first
use.

## CLI

```
th lsp status          # service state + per-server install status
th lsp list            # registry, optionally --installed-only
th lsp install <id>    # eagerly install one server
th lsp install-all     # try every server with a known recipe
th lsp restart         # tear down running clients
th lsp which <id>      # print resolved binary path
```

`th lsp status` is the best starting point — it shows which
...
**`th lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERMES_HOME>/lsp/bin/`. Run
`th lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `th lsp status`**
...
When `th lsp status` shows a `Backend warnings` section, install
...
the rest of the session. Run `th lsp restart` to clear the set;
