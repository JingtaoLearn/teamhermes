...
```bash
thm  chat --toolsets skills -q "What skills do you have?"
thm  chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, TeamHermes asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `thm setup` or `~/.teamhermes/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `thm config migrate` prompts for unconfigured settings, and `thm config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
- `description` (optional) — short text shown in `/bundles` and `thm  bundles list`.
...
thm  bundles create research

# Overwrite an existing bundle
thm  bundles create backend-dev --skill ... --force
...
```bash
thm  skills browse                              # Browse all hub skills (official first)
thm  skills browse --source official            # Browse only official optional skills
thm  skills search kubernetes                   # Search all sources
thm  skills search react --source skills-sh     # Search the skills.sh directory
thm  skills search https://mintlify.com/docs --source well-known
thm  skills inspect openai/skills/k8s           # Preview before installing
thm  skills install openai/skills/k8s           # Install with security scan
thm  skills install official/security/1password
thm  skills install skills-sh/vercel-labs/json-render/json-render-react --force
thm  skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
thm  skills list --source hub                   # List hub-installed skills
thm  skills check                               # Check installed hub skills for upstream updates
thm  skills update                              # Reinstall hub skills with upstream changes when needed
thm  skills audit                               # Re-scan all hub skills for security
thm  skills uninstall k8s                       # Remove a hub skill
thm  skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
thm  skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
thm  skills publish skills/my-skill --to github --repo owner/repo
thm  skills snapshot export setup.json          # Export skill config
thm  skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `thm skills search <query> --source skills-sh`. TeamHermes resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
These are maintained in the TeamHermes repository itself and install with builtin trust.
...
```bash
thm  skills search https://mintlify.com/docs --source well-known
thm  skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
thm  skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```
...
```bash
thm  skills install openai/skills/k8s
...
```bash
thm  skills search airbnb --source browse-sh
thm  skills inspect browse-sh/airbnb.com/search-listings-ddgioa
thm  skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
```bash
thm  skills install https://sharethis.chat/SKILL.md
thm  skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
thm  skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `thm skills update` re-fetches from the same URL automatically when you want to refresh.
...
`thm skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
```bash
thm  skills check          # Report which installed hub skills changed upstream
thm  skills update         # Reinstall only the skills with updates available
thm  skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other TeamHermes users add with `thm skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
```bash
thm  skills tap add my-org/hermes-skills
thm  skills search deploy
thm  skills install my-org/hermes-skills/deploy-runbook
```
...
The `thm skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `thm skills tap list` shows the effective path per tap.
...
```bash
thm  skills install owner/repo/skills/my-workflow
```
...
```bash
thm  skills tap list                                # show all configured taps
thm  skills tap add myorg/skills-repo               # add (default path: skills/)
thm  skills tap remove myorg/skills-repo            # remove
```
...
## Bundled skill updates (`thm skills reset`)

TeamHermes ships with a set of bundled skills in `skills/` inside the repo. On install and on every `thm update`, a sync pass copies those into `~/.teamhermes/skills/` and records a manifest at `~/.teamhermes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).
...
`thm skills reset` is the escape hatch:
...
thm  skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `thm  -p coder skills reset <name>` only affects that profile.
:::
