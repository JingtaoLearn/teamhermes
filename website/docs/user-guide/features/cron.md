...
Schedule tasks to run automatically with natural language or cron expressions. TeamHermes exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to TeamHermes itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
Cron jobs use whatever provider `thm model` selected. `thm setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. TeamHermes disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
thm  cron create "every 2h" "Check server status"
thm  cron create "every 1h" "Summarize new feed items" --skill blogwatcher
thm  cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask TeamHermes normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

TeamHermes will use the unified `cronjob` tool internally.
...
thm  cron create "every 1d at 09:00" \
...
By default a cron job inherits whichever TeamHermes profile owned the gateway / CLI that created it. Pass `--profile <name>` (CLI) or `profile=` (cronjob tool) to re-target the job at a different profile — the scheduler resolves that profile's `HERMES_HOME`, temporarily switches into it for the duration of the run, loads its `.env` + `config.yaml`, and executes the job there:

```bash
# Pin a job to the `night-ops` profile regardless of where it was scheduled
thm  cron create "every 1d at 03:00" \
...
Use `--profile default` to explicitly pin to the root TeamHermes profile. The named profile must already exist; the scheduler refuses to create profiles on the fly. To clear a profile pin during `cron edit`, pass an empty string (`--profile ""` or `profile=""`) — the job reverts to running in whatever profile the scheduler itself is in.
...
```bash
thm  cron edit <job_id> --schedule "every 4h"
thm  cron edit <job_id> --prompt "Use the revised task"
thm  cron edit <job_id> --skill blogwatcher --skill maps
thm  cron edit <job_id> --add-skill maps
thm  cron edit <job_id> --remove-skill blogwatcher
thm  cron edit <job_id> --clear-skills
```
...
```bash
thm  cron list
thm  cron pause <job_id_or_name>
thm  cron resume <job_id_or_name>
thm  cron run <job_id_or_name>
thm  cron remove <job_id_or_name>
thm  cron edit <job_id_or_name> [...flags]
thm  cron status
thm  cron tick
```
...
```bash
thm  gateway install     # Install as a user service
sudo thm  gateway install --system   # Linux: boot-time system service for servers
thm  gateway             # Or run in foreground
...
The `cronjob` tool's schema exposes `no_agent` to TeamHermes directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

TeamHermes will write the check script to `~/.teamhermes/scripts/` via `write_file`, then call:
...
- When Job 2 fires, TeamHermes reads Job 1's most recent output from `~/.teamhermes/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, TeamHermes skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `thm tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `thm tools` cron-platform config wins; otherwise TeamHermes falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether TeamHermes should even invoke the agent. Emit a final stdout line of the form:
...
:::tip
TeamHermes's own `~/.teamhermes/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::
...
Jobs may store `model` and `provider` as `null`. When those fields are omitted, TeamHermes resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
