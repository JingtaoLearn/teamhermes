...
1. `--accept-hooks` flag on the CLI (e.g. `thm  --accept-hooks chat`)
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `thm  hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

### The `thm  hooks` CLI

| Command | What it does |
|---------|--------------|
| `thm  hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `thm  hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `thm  hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `thm  hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Re-run `thm  hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
