...
```bash
thm  memory setup    # select "honcho" from the provider list
```
...
The `thm  honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `thm  memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
thm  honcho status          # Connection status, config, and key settings
thm  honcho setup           # Redirects to `thm  memory setup`
thm  honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
thm  honcho peer            # Show or update peer names + dialectic reasoning level
thm  honcho mode            # Show or set recall mode (hybrid/context/tools)
thm  honcho tokens          # Show or set token budget for context and dialectic
thm  honcho identity        # Seed or show the AI peer's Honcho identity
thm  honcho sync            # Sync Honcho config to all existing profiles
thm  honcho peers           # Show peer identities across all profiles
thm  honcho sessions        # List known Honcho session mappings
thm  honcho map             # Map current directory to a Honcho session name
thm  honcho enable          # Enable Honcho for the active profile
thm  honcho disable         # Disable Honcho for the active profile
thm  honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `thm  honcho`

If you previously used the standalone `thm  honcho setup`:
...
No re-login or re-setup needed. Run `thm  memory setup` and select "honcho" — the wizard detects your existing config.
