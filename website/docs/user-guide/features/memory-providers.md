...
TeamHermes Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
thm  memory setup      # interactive picker + configuration
thm  memory status     # check what's active
thm  memory off        # disable external provider
```
...
When a memory provider is active, TeamHermes automatically:
...
**Setup Wizard:**
```bash
thm  memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `thm  honcho setup` command still works (it now redirects to `thm  memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
:::tip Migrating from `thm  honcho`
If you previously used `thm  honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per TeamHermes profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.
...
| **AI peer** (`aiPeer`) | One per TeamHermes profile. Host key `thm` → default; `hermes.<profile>` for others. |
...
Scans every TeamHermes profile, creates host blocks for any profile without one, inherits settings from the default `thm` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
# Or manually:
thm  config set memory.provider openviking
...
**Setup:**
```bash
thm  memory setup    # select "mem0"
# Or manually:
thm  config set memory.provider mem0
...
**Setup:**
```bash
thm  memory setup    # select "hindsight"
# Or manually:
thm  config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.teamhermes/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p thm  ui start`
...
| `retain_context` | `conversation between TeamHermes Agent and the User` | Context label for retained memories |
...
**Setup:**
```bash
thm  memory setup    # select "holographic"
# Or manually:
...
**Setup:**
```bash
thm  memory setup    # select "retaindb"
# Or manually:
thm  config set memory.provider retaindb
...
# Then configure TeamHermes
thm  memory setup    # select "byterover"
# Or manually:
...
# Or manually:
thm  config set memory.provider supermemory
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per TeamHermes profile
...
thm  config set memory.provider memori
thm  memory setup
```
