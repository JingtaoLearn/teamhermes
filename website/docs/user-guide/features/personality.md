...
description: "Customize TeamHermes Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

TeamHermes Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who TeamHermes is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

TeamHermes now seeds a default `SOUL.md` automatically in:

```text
~/.teamhermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run TeamHermes with a custom home directory, it will use:
...
- TeamHermes creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- TeamHermes loads `SOUL.md` only from `HERMES_HOME`
- TeamHermes does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, TeamHermes falls back to a built-in default identity
...
If TeamHermes loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the TeamHermes instance itself.

That also makes it easier to teach users:
- "Edit `~/.teamhermes/SOUL.md` to change TeamHermes' default personality."
...
- how TeamHermes should handle uncertainty, disagreement, or ambiguity
...
## What TeamHermes injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, TeamHermes falls back to a built-in default identity ("You are TeamHermes Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
TeamHermes ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain TeamHermes, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives TeamHermes its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.teamhermes/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.teamhermes/SOUL.md`
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how TeamHermes speaks
- `display.skin` and `/skin` affect how TeamHermes looks in the terminal
