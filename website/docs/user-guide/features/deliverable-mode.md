...
When TeamHermes Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.teamhermes/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.teamhermes/config.yaml` (switchable per session
via `/personality`).
...
If you use TeamHermes' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.teamhermes/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. TeamHermes Agent's deliverable
