...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `th setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the browser provider via `th model` or `th tools`.
:::
...
mode, so you need it installed (`th setup tools → Browser Automation`
...
Or configure via `th tools` → Browser Automation → Camofox.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `th config set toolsets '["hermes-cli", "browser"]'`.
:::
...
```
Type "th agent" into the search field @e3
```
