...
Use `th computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `th tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
- **`th update`** — when you update TeamHermes itself, if `cua-driver` is
...
- **`th computer-use install --upgrade`** — manual force-refresh.
...
`th computer-use status` shows the installed version next to the
binary path.
...
Pair with `approvals.mode: manual` in `~/.teamhermes/config.yaml` if you want every action confirmed.
...
`th computer-use install` to fetch the cua-driver binary, or run
`th tools` and enable the Computer Use toolset.
