...
description: "Plugins shipped with TeamHermes Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

TeamHermes ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.teamhermes/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a TeamHermes Plugin](/guides/build-a-hermes-plugin) to write your own.
...
2. **User** — `~/.teamhermes/plugins/<name>/`
3. **Project** — `./.teamhermes/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `th memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `th plugins list` and the interactive `th plugins` UI), but none load until you explicitly enable them:

```bash
th plugins enable disk-cleanup
```

Or via `~/.teamhermes/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer TeamHermes. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
th plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `th plugins enable <name>`.
...
| `hermes-achievements` | dashboard tab | Steam-style collectible badges generated from your real TeamHermes session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `th memory` and `th plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
**Enabling:** `th plugins enable disk-cleanup` (or check the box in `th plugins`).

**Disabling again:** `th plugins disable disk-cleanup`.
...
**Enabling:** `th plugins enable security-guidance` (or check the box in `th plugins`).

**Disabling again:** `th plugins disable security-guidance`.
...
Traces TeamHermes turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of TeamHermes' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `th logs`.
...
Or check the box in the interactive `th plugins` UI. Then put the credentials in `~/.teamhermes/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "TeamHermes turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the TeamHermes session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `th chat` session lives under one Langfuse session.

**Verify:**

```bash
th plugins list                 # observability/langfuse should show "enabled"
th chat -q "hello"              # check the Langfuse UI for a "TeamHermes turn" trace
```
...
TeamHermes-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — TeamHermes-prefixed wins when both are set.
...
**Disabling:** `th plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.teamhermes/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
th plugins enable google_meet
...
**Disabling:** `th plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.teamhermes/cache/google_meet/` until you remove them.

### hermes-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real TeamHermes session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with TeamHermes feature changes.

**How it works:**

- Scans your entire `~/.teamhermes/state.db` session history on the dashboard backend
...
| Secret | Hidden until TeamHermes detects the first related signal in your history |
...
| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across TeamHermes updates. |
...
**Enabling:** Nothing to enable — `hermes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `th dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.
...
Bundled plugins are written exactly like any other TeamHermes plugin — see [Build a TeamHermes Plugin](/guides/build-a-hermes-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.teamhermes/plugins/<name>/`
- Manifest source is reported as `bundled` in `th plugins list`
