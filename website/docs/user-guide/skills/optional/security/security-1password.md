...
| | |
|---|---|
| Source | Optional — install with `thm skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by TeamHermes Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during TeamHermes terminal calls (desktop app flow only)
...
### Service Account (recommended for TeamHermes)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.teamhermes/.env` (the skill will prompt for this on first load).
...
## TeamHermes Execution Pattern (desktop app flow)

TeamHermes terminal commands are non-interactive by default and can lose auth context between calls.
