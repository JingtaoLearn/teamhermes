...
| | |
|---|---|
| Source | Optional — install with `thm skills install official/software-development/code-wiki` |
| Path | `optional-skills/software-development/code-wiki` |
| Version | `0.1.0` |
| Author | Teknium (teknium1), TeamHermes Agent |
| License | MIT |
...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.teamhermes/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.teamhermes/wikis/$REPO_NAME"
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "teamhermes code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.teamhermes/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.teamhermes/wikis/`. Only write into the repo when the user explicitly requests it.
