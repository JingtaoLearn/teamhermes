---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into TeamHermes Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into TeamHermes Agent"
---
...
Migrate a user's OpenClaw customization footprint into TeamHermes Agent. Imports TeamHermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `th skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | TeamHermes Agent (Nous Research) |
| License | MIT |
...
# OpenClaw -> TeamHermes Migration

Use this skill when a user wants to move their OpenClaw setup into TeamHermes Agent with minimal manual cleanup.
...
```bash
th claw migrate              # Full interactive migration
th claw migrate --dry-run    # Preview what would be migrated
th claw migrate --preset user-data   # Migrate without secrets
th claw migrate --overwrite  # Overwrite existing conflicts
th claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `th setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the TeamHermes home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into TeamHermes memory entries
- merge OpenClaw command approval patterns into TeamHermes `command_allowlist`
- migrate TeamHermes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to TeamHermes working-directory configuration
- copy OpenClaw skills into `~/.teamhermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen TeamHermes workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.teamhermes/tts/`
- archive non-secret docs that do not have a direct TeamHermes destination
...
- `~/.teamhermes/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.teamhermes/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.teamhermes/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of TeamHermes-compatible secrets, currently:
...
TeamHermes CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in TeamHermes. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a TeamHermes workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing TeamHermes skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.teamhermes/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.teamhermes/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.teamhermes/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.teamhermes/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.teamhermes/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.teamhermes/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of TeamHermes unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty TeamHermes targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean TeamHermes destination. Unsupported auth blobs must still be reported as skipped.
...
- TeamHermes persona state imported
- TeamHermes memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.teamhermes/skills/openclaw-imports/`
