...
| | |
|---|---|
| Source | Optional — install with `thm skills install official/devops/watchers` |
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.teamhermes/.env` to avoid the 60 req/hr anonymous rate limit):
