...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. teamhermes@agentmail.to).

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `thm skills install official/email/agentmail` |
...
Add to `~/.teamhermes/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
