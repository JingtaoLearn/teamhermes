...
| | |
|---|---|
| Source | Optional — install with `th skills install official/creative/kanban-video-orchestrator` |
...
3. Creates each TeamHermes profile via `th profile create --clone`
...
7. Fires the initial `th kanban create` task assigned to the director
...
```bash
th kanban watch --tenant <project-tenant>     # live events
th kanban list  --tenant <project-tenant>     # board snapshot
th dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `th kanban create` call to a specialist profile. The
