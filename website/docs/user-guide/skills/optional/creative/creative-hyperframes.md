...
| | |
|---|---|
| Source | Optional — install with `thm skills install official/creative/hyperframes` |
...
```bash
bash "$(dirname "$(find ~/.teamhermes/skills -path '*/hyperframes/SKILL.md' 2>/dev/null | head -1)")/scripts/setup.sh"
```
