...
| | |
|---|---|
| Source | Optional — install with `thm skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into TeamHermes or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to TeamHermes. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.teamhermes/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.teamhermes/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is TeamHermes integration, either:

- configure the server in `~/.teamhermes/config.yaml` using the `native-mcp` skill, or
...
### TeamHermes cannot see the deployed server

The server-building part may be correct while the TeamHermes config is not. Load the `native-mcp` skill and configure the server in `~/.teamhermes/config.yaml`, then restart TeamHermes.
