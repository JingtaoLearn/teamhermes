...
| | |
|---|---|
| Source | Optional — install with `th skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents/grok` |
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), TeamHermes Agent |
| License | MIT |
...
# Grok Build CLI — TeamHermes Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the TeamHermes terminal. Grok
...
1. Prepare stable input files first with TeamHermes tools (`read_file`,
...
2. **Don't conflate TeamHermes' xAI auth with the `grok` CLI's auth.** TeamHermes'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume TeamHermes' xAI auth carries
   over.
