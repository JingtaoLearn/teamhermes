...
description: "Configure and use Honcho memory with TeamHermes -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
# Honcho Memory for TeamHermes

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every TeamHermes profile its own peer identity while sharing a unified view of the user.
...
```bash
thm  honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this TeamHermes profile's AI peer
...
Honcho models conversations as interactions between **peers**. TeamHermes creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this TeamHermes instance. Each profile gets its own AI peer so agents develop independent views.
...
Manual override: `thm  honcho map my-project-name`
...
Each TeamHermes profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
thm  profile create coder --clone
...
```bash
thm  honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for TeamHermes when Honcho memory is active.
...
Run `thm  honcho setup`. Ensure `memory.provider: honcho` is in `~/.teamhermes/config.yaml`.

### Memory not persisting across sessions
Check `thm  honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `thm profile create <name> --clone`. For existing profiles: `thm  honcho sync`.
...
| Command | Description |
|---------|-------------|
| `thm  honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `thm  honcho status` | Show resolved config, connection test, peer info for active profile |
| `thm  honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `thm  honcho disable` | Disable Honcho for the active profile |
| `thm  honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `thm  honcho peers` | Show peer identities across all profiles |
| `thm  honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `thm  honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `thm  honcho sessions` | List known directory-to-session-name mappings |
| `thm  honcho map <name>` | Map current working directory to a Honcho session name |
| `thm  honcho identity` | Seed AI peer identity or show both peer representations |
| `thm  honcho sync` | Create host blocks for all TeamHermes profiles that don't have one yet |
| `thm  honcho migrate` | Step-by-step migration guide from OpenClaw native memory to TeamHermes + Honcho |
| `thm  memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `thm  memory status` | Show active memory provider and config |
| `thm  memory off` | Disable external memory provider |
