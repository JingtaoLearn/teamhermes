...
| | |
|---|---|
| Source | Optional — install with `th skills install official/autonomous-ai-agents/antigravity-cli` |
| Path | `optional-skills/autonomous-ai-agents/antigravity-cli` |
| Version | `0.1.0` |
| Author | Tony Simons (asimons81), TeamHermes Agent |
| License | MIT |
...
commands through the TeamHermes `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure — it does not wrap a network
API, so there is nothing to authenticate from TeamHermes itself.
