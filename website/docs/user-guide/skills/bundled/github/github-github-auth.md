...
- Give it a name like "teamhermes"
- Select scopes:
...
- Give it a title like "teamhermes-&lt;machine-name>"
...
elif [ -f ~/.teamhermes/.env ] && grep -q "^GITHUB_TOKEN=" ~/.teamhermes/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.teamhermes/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
