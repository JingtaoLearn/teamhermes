...
Everything operator-facing is a `th teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
```bash
th teams-pipeline validate              # config snapshot — run first after any change
th teams-pipeline token-health          # Graph token status
th teams-pipeline token-health --force-refresh   # force a fresh token acquisition
th teams-pipeline list                  # recent meeting jobs
th teams-pipeline list --status failed  # only failed jobs
th teams-pipeline show <job-id>         # full detail of one job
th teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
th teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
th teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
th teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
th teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

th teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
th teams-pipeline delete-subscription <sub-id>
th teams-pipeline maintain-subscriptions            # renew near-expiry ones
th teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `th teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `th cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
