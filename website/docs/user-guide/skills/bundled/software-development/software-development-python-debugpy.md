...
- A long-running process (thm  gateway, tui_gateway) misbehaves and you can't restart it
...
The thm  test runner and pytest both support this:
...
### `tui_gateway` subprocess (spawned by `thm  --tui`)
...
debugpy.wait_for_client()
```
Start `thm  --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
