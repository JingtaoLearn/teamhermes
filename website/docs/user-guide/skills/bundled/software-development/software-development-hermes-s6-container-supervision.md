---
title: "TeamHermes S6 Container Supervision"
sidebar_label: "TeamHermes S6 Container Supervision"
description: "Modify, debug, or extend the s6-overlay supervision tree inside the TeamHermes Agent Docker image — adding new services, debugging profile gateways, understandin..."
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# TeamHermes S6 Container Supervision

Modify, debug, or extend the s6-overlay supervision tree inside the TeamHermes Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
...
| Related skills | [`teamthm`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-teamthm), `teamhermes-dev` |
...
# TeamHermes s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the TeamHermes Docker image (something that should be supervised at every container start, like the dashboard)
...
If you're just running the TeamHermes Agent and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid thm  thm -p coder gateway run")
...
└── CMD ("main program")               ← /opt/hermes/docker/main-wrapper.sh
    └── routes user args: bare exec | thm  subcommand | thm (no args)
...
| `docker/main-wrapper.sh` | The container's CMD. Routes user args, drops to thm via `s6-setuidgid`, exec's the chosen program. |
| `docker/s6-rc.d/main-hermes/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main thm  runs as the CMD, not as a supervised service. |
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `thm  gateway start/stop/restart` and routes to s6 when running in a container. |

## Why Architecture B (CMD as main program, not s6-supervised)

The original plan (v1–v3) called for main thm to run as a supervised s6-rc service. Two real s6-overlay v3 mechanics blocked that:
...
So we use the s6-overlay-native CMD pattern: `ENTRYPOINT ["/init", "/opt/hermes/docker/main-wrapper.sh"]`. /init prepends the wrapper to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to thm via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract.

Trade-off: main thm is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). Dashboard supervision is the only **new** guarantee — and per-profile gateways under `/run/service/` get full supervision.
...
2. Drop to thm via `s6-setuidgid thm` at the top of run (unless you specifically need root).
...
The cont-init reconciler runs as thm (`s6-setuidgid thm` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> thm  profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$HERMES_HOME/profiles` to thm on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user thm` or rely on the stage2 chown sweep next reboot. Don't write files under `$HERMES_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `thm  -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).
...
- `teamhermes-dev`: General teamthm codebase navigation
