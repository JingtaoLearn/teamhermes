...
All management is via the `thm  webhook` CLI command:

### Create a subscription
```bash
thm  webhook subscribe <name> \
...
```bash
thm  webhook remove <name>
```

### Test a subscription
```bash
thm  webhook test <name>
thm  webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
thm  webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
thm  webhook subscribe stripe-payments \
...
```bash
thm  webhook subscribe ci-builds \
  --events "pipeline" \
...
- Subscriptions persist to `~/.teamhermes/webhook_subscriptions.json`

## How It Works

1. `thm  webhook subscribe` writes to `~/.teamhermes/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.teamhermes/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `thm  webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `thm  webhook test <name>` to verify the route works.
