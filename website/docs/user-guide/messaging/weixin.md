...
- **QR code login** — scan-to-connect setup via `thm  gateway setup`
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `thm  gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `thm  gateway setup` |
...
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `thm  gateway setup` to scan a new QR code |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `pip install teamhermes[messaging]`. Alternatively, open the URL printed above the QR |
