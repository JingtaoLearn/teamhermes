...
sidebar_position: 9
title: "Matrix"
description: "Set up TeamHermes Agent as a Matrix bot"
---

# Matrix Setup

TeamHermes Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the TeamHermes Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

TeamHermes works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how TeamHermes behaves once it's connected.

## How TeamHermes Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | TeamHermes responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, TeamHermes requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | TeamHermes supports Matrix threads (MSC3440). If you reply in a thread, TeamHermes keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, TeamHermes auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, TeamHermes isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
You can also run TeamHermes as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

TeamHermes needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give TeamHermes the bot's user ID and password. TeamHermes will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
TeamHermes Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure TeamHermes Agent
...
Optional behavior settings in `~/.teamhermes/config.yaml`:
...
TeamHermes supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
Add to your `~/.teamhermes/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, TeamHermes:

- Stores encryption keys in `~/.teamhermes/platforms/matrix/store/` (legacy installs: `~/.teamhermes/matrix/store/`)
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, TeamHermes imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

:::warning[Deleting the crypto store]
If you delete `~/.teamhermes/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

TeamHermes detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
Add this to your `~/.teamhermes/.env`:
...
If you previously used TeamHermes with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       ...
       "password": "***",
       "initial_device_display_name": "TeamHermes Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.teamhermes/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.teamhermes/platforms/matrix/store/crypto.db
   rm -f ~/.teamhermes/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.teamhermes/.env`:
...
a working E2EE setup with a previous version of TeamHermes and are upgrading.
...
services:
  hermes-matrix:
    ...
    volumes:
      - ./matrix-store:/root/.teamhermes/platforms/matrix/store
```
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote TeamHermes API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-TeamHermes-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the TeamHermes logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The TeamHermes gateway isn't running, or it failed to connect.

**Fix**: Check that `thm  gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.teamhermes/.env` and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: TeamHermes can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
