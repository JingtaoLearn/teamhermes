...
sidebar_position: 12
title: "Google Chat"
description: "Set up TeamHermes Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect TeamHermes Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your TeamHermes
...
> Run `thm  gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only TeamHermes can read (e.g.,
`~/.teamhermes/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a thm  restart)
...
Also grant `Pub/Sub Viewer` on the same subscription — TeamHermes calls
...
- **App name**: whatever you want users to see ("TeamHermes" is reasonable).
...
`ADDED_TO_SPACE` event that TeamHermes uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure TeamHermes

Add the Google Chat section to `~/.teamhermes/.env`:
...
Install the dependencies the Google Chat adapter needs (no TeamHermes extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "TeamHermes is thinking…" marker, then
...
Thread support: when a user replies inside a thread, TeamHermes detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate TeamHermes session.
...
3. Download the JSON. Move it onto the host that runs TeamHermes.
4. On the host, register the client with TeamHermes:
...
That writes `~/.teamhermes/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.teamhermes/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.teamhermes/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, TeamHermes isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `thm  gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: TeamHermes will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.teamhermes/google_chat_user_tokens/<sanitized_email>.json` (filesystem
