...
sidebar_position: 16
title: "Yuanbao"
description: "Connect TeamHermes Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect TeamHermes to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.teamhermes/.env`:
...
All standard TeamHermes commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.teamhermes/logs/gateway.log`
...
Yuanbao has a maximum message size. TeamHermes automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
HERMES_LOG_LEVEL=debug thm  gateway
```
...
```bash
thm  chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
