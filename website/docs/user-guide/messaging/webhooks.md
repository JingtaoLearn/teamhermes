...
1. Enable via `th gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `th webhook subscribe`
...
```bash
th webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `th webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.

### Create a subscription

```bash
th webhook subscribe github-issues \
  --events "issues" \
...
```bash
th webhook remove github-issues
```

### Test a subscription

```bash
th webhook test github-issues
th webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `th webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `th gateway run`
