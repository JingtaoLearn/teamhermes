...
> Run `thm  gateway setup` and pick **WhatsApp** for a guided walk-through.
...
3. Run `thm  whatsapp` and scan the QR code from that WhatsApp account
...
The Baileys bridge saves its session under `~/.teamhermes/platforms/whatsapp/session`. This means:
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `thm  whatsapp`. |
...
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `thm  whatsapp` if needed. |
...
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `thm  gateway install` to re-snapshot your current PATH into the plist, then `thm  gateway start`. See the [Gateway Service docs](./index.md#macos-launchd) for details. |
...
- The `~/.teamhermes/platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.teamhermes/platforms/whatsapp/session`
