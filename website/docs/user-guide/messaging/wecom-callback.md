...
Connect TeamHermes to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
TeamHermes supports two WeCom integration modes:
...
> Run `thm  gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. TeamHermes decrypts the message, queues it for the agent
...
(Use `thm  gateway start` only after `thm  gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in TeamHermes and the token the
...
in `~/.teamhermes/.env` values around `=` will also break signature checks. After
fixing, restart `thm  gateway run`.
...
Check `thm  gateway run` logs for the bound host/port. If the adapter bound to
