...
> Run `th gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
(Use `th gateway start` only after `th gateway install` has registered the systemd/launchd service.)
...
Check `th gateway run` logs for the bound host/port. If the adapter bound to
