...
sidebar_position: 4
title: "Slack"
description: "Set up TeamHermes Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect TeamHermes Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your TeamHermes instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. TeamHermes uses the modern
...
The fastest path is to paste a manifest TeamHermes generates for you. It
...
### Option A: From a TeamHermes-generated manifest (recommended)
...
4. Enter an app name (e.g., "TeamHermes Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, TeamHermes can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a TeamHermes configuration issue.
:::
...
TeamHermes uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
thm  gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
thm  gateway              # Foreground
thm  gateway install      # Install as a user service
sudo thm  gateway install --system   # Linux only: boot-time system service
```
...
Every TeamHermes command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
TeamHermes command with its description.

Under the hood: TeamHermes ships with a generated Slack app manifest (see
...
When TeamHermes adds new commands (e.g. after `thm update`), regenerate
...
3. Paste the new contents of `~/.teamhermes/slack-manifest.json`
...
`/hermes btw run the tests` — TeamHermes routes it the same way as `/btw
...
Slack never delivers them to TeamHermes.

As a workaround, TeamHermes recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
TeamHermes treats it identically to the slash form and replies in the same
thread.
...
```bash
thm  slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how TeamHermes behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@TeamHermes Agent what time is it?`). In channels, TeamHermes replies in a thread attached to that message. |
| **Threads** | If you @mention TeamHermes inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.teamhermes/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to TeamHermes in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before TeamHermes will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where TeamHermes will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @TeamHermes Agent`).

---

## Multi-Workspace Support

TeamHermes can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, TeamHermes also loads tokens from an **OAuth token file** at:

```
~/.teamhermes/slack_tokens.json
```
...
- When a message arrives, TeamHermes uses the correct workspace-specific client to respond.
...
TeamHermes supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @TeamHermes Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @TeamHermes Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. TeamHermes now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @TeamHermes Agent`)
...
- Tokens should be stored in `~/.teamhermes/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your TeamHermes config directory
