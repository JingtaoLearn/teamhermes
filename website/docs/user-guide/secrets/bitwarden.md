...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.teamhermes/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. TeamHermes stores that single token in `~/.teamhermes/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `th` (or the gateway, or a cron job) starts, after `~/.teamhermes/.env` has loaded, TeamHermes calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default TeamHermes **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every TeamHermes process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.
...
2. Create or pick a **Project** (e.g. "TeamHermes keys").
...
4. **Machine accounts → New machine account → My TeamHermes machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.teamhermes/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.teamhermes/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
th secrets bitwarden status
```

From now on, every `th` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `th secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `th secrets bitwarden status` | Show config + binary version + token presence |
| `th secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `th secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `th secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `th secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |

## Configuration

Defaults in `~/.teamhermes/config.yaml`:
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `th` invocations start fresh. |
...
Bitwarden never blocks TeamHermes startup. If anything goes wrong, you'll see a one-line warning in stderr and TeamHermes continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `th secrets bitwarden setup` |
...
- TeamHermes will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — TeamHermes does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.teamhermes/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple TeamHermes installations.
