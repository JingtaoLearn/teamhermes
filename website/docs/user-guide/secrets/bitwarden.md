...
3. Every time `thm` (or the gateway, or a cron job) starts, after `~/.teamhermes/.env` has loaded, TeamHermes calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
...
|---|---|
| `thm  secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `thm  secrets bitwarden status` | Show config + binary version + token presence |
| `thm  secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `thm  secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `thm  secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `thm  secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `thm  secrets bitwarden setup` |
