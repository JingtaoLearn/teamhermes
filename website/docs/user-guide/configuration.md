...
Run `thm setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
thm  config              # View current configuration
thm  config edit         # Open config.yaml in your editor
thm  config set KEY VAL  # Set a specific value
thm  config check        # Check for missing options (after updates)
thm  config migrate      # Interactively add missing options

# Examples:
thm  config set model anthropic/claude-opus-4
thm  config set terminal.backend docker
thm  config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `thm config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `thm chat --model anthropic/claude-sonnet-4` (per-invocation override)
...
:::warning
The agent has the same filesystem access as your user account. Use `thm tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Single persistent container, not per-command.** TeamHermes starts ONE long-lived container on first use and routes every terminal, file, and `execute_code` call through `docker exec` into that same container — across sessions, `/new`, `/reset`, and `delegate_task` subagents — for the lifetime of the TeamHermes process. Working-directory changes, installed packages, and files in `/workspace` carry over from one tool call to the next, just like a local shell. The container is stopped and removed on shutdown. See **Container lifecycle** below for details.
...
**Container lifecycle:** TeamHermes reuses a single long-lived container (`docker run -d ... sleep 2h`) for every terminal and file-tool call, across sessions, `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the TeamHermes process. Commands run via `docker exec` with a login shell, so working-directory changes, installed packages, and files in `/workspace` all persist from one tool call to the next. The container is stopped and removed on TeamHermes shutdown (or when the idle-sweep reclaims it).
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `thm config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. TeamHermes logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `thm doctor` to check.
...
TeamHermes resolves each listed variable from your current shell first, then falls back to `~/.teamhermes/.env` if it was saved with `thm config set`.
...
```bash
thm  config set terminal.persistent_shell false
```
...
- `thm config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `thm config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
thm  config set skills.config.myplugin.path ~/myplugin-data
```
...
`thm tools`), so a toolset listed here is always removed — even if a
...
```yaml
worktree: true    # Always create a worktree (same as thm  -w)
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `thm plugins` → Provider Plugins → Context Engine.
...
Instead of hand-editing YAML, run `thm model` and pick **"Configure auxiliary models"** from the menu. You'll get an interactive per-task picker:
...
:::tip MiniMax OAuth
`minimax-oauth` logs in via browser OAuth (no API key needed). Run `thm model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](../guides/minimax-oauth.md).
:::

:::tip xAI Grok OAuth
`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `thm model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md), and if TeamHermes is on a remote host see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md).
:::
...
auxiliary:
  ...
  # Kanban triage specifier — `thm kanban specify <id>` (or the
...
| `"nous"` | Force Nous Portal | `thm auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `thm model` → Codex |
| `"minimax-oauth"` | Force MiniMax OAuth (browser login, no API key). Uses MiniMax-M2.7-highspeed for auxiliary tasks. | `thm model` → MiniMax (OAuth) |
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok or X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `thm model` → xAI Grok OAuth (SuperGrok / Premium+) |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `thm model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
```
Run `thm model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
:::tip
Run `thm config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `thm tools`:
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: thm  chat --checkpoints). Default: false (opt-in).
...
| Context | Default |
|---------|---------|
| **CLI (`thm`)** | Current directory where you run the command |
