...
title: "Git Worktrees"
description: "Run multiple TeamHermes agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

TeamHermes Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with TeamHermes so each session has a clean, isolated working directory.

## Why Use Worktrees with TeamHermes?

TeamHermes treats the **current working directory** as the project root:

- CLI: the directory where you run `thm` or `thm chat`
...
Now you can `cd` into the new worktree and run TeamHermes there:

```bash
cd ../repo-feature

# Start TeamHermes in the worktree
thm
```
...
- TeamHermes checkpoint data under `~/.teamhermes/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per TeamHermes experiment**
...
## Using `thm  -w` (Automatic Worktree Mode)
...
For parallel agents, open multiple terminals and run `thm  -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each TeamHermes session its own clean checkout.
