...
TeamHermes Agent can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.
...
Or enable globally in `~/.teamhermes/config.yaml`:
...
| Command | Description |
|---------|-------------|
| `th checkpoints` | Show total size, project count, per-project breakdown |
| `th checkpoints status` | Same as bare `checkpoints` |
| `th checkpoints list` | Alias for `status` |
| `th checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `th checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `th checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- TeamHermes detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.teamhermes/checkpoints/store/`.
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.teamhermes/checkpoints/ at startup
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `th checkpoints prune` manually.
...
TeamHermes responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.teamhermes/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
...
Clear with: th checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
th checkpoints prune --retention-days 3 --max-size-mb 200
```
...
- **Directory scope** — TeamHermes skips overly broad directories (root `/`, home `$HOME`).
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `th checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.teamhermes/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.teamhermes/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
th checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `th chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `th checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each TeamHermes session in its own worktree/branch, with checkpoints as an extra layer.
