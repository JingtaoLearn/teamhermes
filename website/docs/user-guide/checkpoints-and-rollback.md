...
| Command | Description |
|---------|-------------|
| `thm  checkpoints` | Show total size, project count, per-project breakdown |
| `thm  checkpoints status` | Same as bare `checkpoints` |
| `thm  checkpoints list` | Alias for `status` |
| `thm  checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `thm  checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `thm  checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `thm  checkpoints prune` manually.
...
Clear with: thm  checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
thm  checkpoints prune --retention-days 3 --max-size-mb 200
```
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `thm  checkpoints status` / `prune` / `clear` instead.
...
- **Check `thm  checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
