...
TeamHermes supports multiple AI inference providers out of the box. Use `th model` to configure interactively, or set them in `config.yaml`.
...
The `web_search` and `web_extract` tools support four backend providers, configured via `config.yaml` or `th tools`:
