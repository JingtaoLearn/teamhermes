...
You need at least one way to connect to an LLM. Use `th model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `th model` (OAuth, subscription-based) |
| **OpenAI Codex** | `th model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `th model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `th model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `th model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
...
| **xAI Grok OAuth (SuperGrok)** | `th model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
...
| **Google Gemini (OAuth)** | `th model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.teamhermes/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `th model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `th model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.teamhermes/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `th model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `th model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `th model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.teamhermes/.env` (provider: `stepfun`) |
| **LM Studio** | `th model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `th model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
```bash
th setup --portal     # fresh install — OAuth + provider + gateway in one command
th model              # existing install — pick "Nous Portal" from the list
th portal status      # inspect login + routing at any time
```
...
|---------|-------------|--------------|
| **`th model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a TeamHermes chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `th model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `th model`, complete the provider setup, then start a new session.
...
When you authenticate via `th model` → Anthropic OAuth (or via `th auth add anthropic --type oauth`), TeamHermes routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by TeamHermes — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
th chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `th model`
# TeamHermes will use Claude Code's credential store directly when available
th model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
th chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
th chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `th model`, TeamHermes prefers Claude Code's own credential store over copying the token into `~/.teamhermes/.env`. That keeps refreshable Claude credentials refreshable.
...
```bash
th chat --provider copilot --model gpt-5.4
```
...
If no token is found, `th model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `th model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `th model` to authenticate via OAuth instead.
:::
...
```bash
th chat --provider copilot-acp --model copilot-acp
...
xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.teamhermes/.env` and pick xAI in `th model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `th model`, or run `th auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if TeamHermes runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `th tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `th doctor` and `th chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `th migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
th migrate xai          # preview replacements
th migrate xai --apply  # rewrite ~/.teamhermes/config.yaml in place
```
...
# Short alias
th chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `th model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and TeamHermes auto-discovers the available models.
...
th chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 th chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `th model`, sign in through the browser, and TeamHermes persists the refresh token.
...
th chat   # uses portal.qwen.ai/v1 endpoint
```
...
```bash
th chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `th model`, sign in through the browser, and TeamHermes persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
th chat   # uses api.minimax.io/anthropic endpoint
```
...
th chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.teamhermes/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 th chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
```bash
# GMI Cloud
th chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.teamhermes/.env
```
...
```bash
# StepFun
th chat --provider stepfun --model step-3-mini
...
```bash
# Use any available model
th chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.teamhermes/.env

# Short alias
th chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
`LLM_MODEL` in `.env` is **removed** — `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `th model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the next `th setup` or config migration.
:::
...
:::warning th model vs /model
**`th model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.
...
**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `th model`, set up the new provider, then start a new session.
:::
...
Supported parsers: `th` (Qwen 2.5, TeamHermes 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure TeamHermes with `th model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure TeamHermes with `th model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `th model` → Custom endpoint, or in `config.yaml`:
...
`th model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # set explicitly by `th model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `th model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
You can also select named custom providers from the interactive `th model` menu.
...
Together's `/v1/models` endpoint works, so `th model` can auto-discover available models.
...
:::tip Troubleshooting
- `th doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `th model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `th model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point TeamHermes at your instance (no API key needed):
   ```bash
   th config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `th fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
