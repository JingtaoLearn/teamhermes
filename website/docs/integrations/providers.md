...
When you choose Anthropic OAuth through `hermes model`, TeamHermes prefers Claude Code's own credential store over copying the token into `~/.teamhermes/.env`. That keeps refreshable Claude credentials refreshable.
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `hermes model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and TeamHermes auto-discovers the available models.
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `hermes model`, sign in through the browser, and TeamHermes persists the refresh token.
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `hermes model`, sign in through the browser, and TeamHermes persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
Supported parsers: `hermes` (Qwen 2.5, TeamHermes 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure TeamHermes with `hermes model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure TeamHermes with `hermes model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
