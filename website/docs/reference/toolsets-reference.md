...
description: "Reference for TeamHermes core, composite, platform, and dynamic toolsets"
---
...
```bash
thm  chat --toolsets web,file,terminal
thm  chat --toolsets debugging        # composite — expands to file + terminal + web
thm  chat --toolsets all              # everything
```
...
```bash
thm  tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call TeamHermes tools programmatically. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `thm tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
...
| `thm-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
...
## Relationship to `thm tools`

The `thm tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
