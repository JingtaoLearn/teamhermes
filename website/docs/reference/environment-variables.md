...
| `HERMES_HOME` | Override Hermes config directory (default: `~/.teamhermes`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |
