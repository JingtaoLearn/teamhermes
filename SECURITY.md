# TeamHermes Agent Security Policy

This document describes TeamHermes Agent's trust model, names the one
...
security vulnerabilities. **TeamHermes Agent does not operate a bug
bounty program.**
...
- Environment details (`thm version`, commit SHA, OS, Python
  version).
...
TeamHermes Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running TeamHermes Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted TeamHermes Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in TeamHermes Agent's documentation
...
TeamHermes Agent supports two OS-level isolation postures. They address
...
TeamHermes Agent supports this in two ways:

- **TeamHermes Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, TeamHermes Agent's in-process heuristics
...
TeamHermes Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in TeamHermes Agent
itself. Bugs in TeamHermes Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   TeamHermes Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, TeamHermes Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  TeamHermes Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  teamhermes-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not TeamHermes Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in TeamHermes Agent. Bugs in TeamHermes
...
- TeamHermes Agent includes supply-chain guards for MCP server
