...
# Audio file extensions TeamHermes recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through TeamHermes' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. TeamHermes-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.teamhermes/.env manually."
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The TeamHermes home itself contains credentials (auth.json, .env) — only the
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    TeamHermes-managed cache, under an operator-allowlisted root
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.teamhermes/.env,
    # ~/.teamhermes/auth.json, etc. — so the obvious prompt-injection sites
