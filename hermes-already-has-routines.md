# TeamHermes Agent Has Had "Routines" Since March
...
TeamHermes equivalent — works today:
```bash
thm cron create "0 2 * * *" \
...
TeamHermes equivalent — works today:
```bash
thm webhook subscribe auth-watch \
  --events "pull_request" \
...
TeamHermes equivalent — works today:
```bash
thm webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working TeamHermes implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | TeamHermes Agent |
|---|---|---|
...
## Things TeamHermes Does That Routines Can't
...
TeamHermes has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
TeamHermes Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
thm gateway setup    # enable webhooks
thm webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [teamhermes.nousresearch.com/docs/guides/automation-templates](https://hermes-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [teamhermes.nousresearch.com](https://hermes-agent.nousresearch.com)

GitHub: [github.com/NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)

---

*TeamHermes Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
