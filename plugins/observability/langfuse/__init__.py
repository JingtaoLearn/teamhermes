"""langfuse — TeamHermes plugin for Langfuse observability.

Traces TeamHermes conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the TeamHermes plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``th plugins enable
observability/langfuse``, or by checking the box in the interactive
``th plugins`` UI). At runtime the plugin also requires the
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the TeamHermes plugin system —
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="TeamHermes turn",
                tags=["hermes", "langfuse"],
            ):
                root_ctx = client.start_as_current_observation(
                    trace_context=trace_ctx,
                    name="TeamHermes turn",
                    as_type="chain",
                    input=trace_input,
                    metadata=metadata,
                    end_on_exit=False,
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older TeamHermes branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current TeamHermes also has a turn-scoped
    ...
    # messages is a list only for legacy TeamHermes branches that fired
    # pre_llm_call with API messages directly. Current TeamHermes fires
...
def register(ctx) -> None:
    ...
    # TeamHermes versions.  pre_api_request / post_api_request fire per API
