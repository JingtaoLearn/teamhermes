...
Let the th agent join a Google Meet call, transcribe it, optionally speak
...
│                                                                      │
│   NodeServer (from `th meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `th meet` CLI |
...
| `cli.py` | `th meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `th meet node {run,list,approve,remove,status,ping}` |
...
th meet install                                      # pip + Chromium
th meet setup                                        # preflight
th meet auth                                         # optional
th meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
th meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.teamhermes/.env
th meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
th meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
th meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
...
th plugins enable google_meet
th meet node run --display-name my-mac --host 0.0.0.0 --port 18789
