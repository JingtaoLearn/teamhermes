...
thm meet install                 # pip deps + Chromium (transcribe only)
thm meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
thm meet auth                    # optional; skips guest-lobby wait
thm meet setup                   # preflight checks
```

`thm meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
thm plugins enable google_meet
thm meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
thm meet node approve my-mac ws://<mac-ip>:18789 <token>
thm meet node ping my-mac                   # confirm reachable
```

Run `thm meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `thm meet auth` avoids this.
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `thm meet node approve`.
