...
th meet install                 # pip deps + Chromium (transcribe only)
th meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
th meet auth                    # optional; skips guest-lobby wait
th meet setup                   # preflight checks
```

`th meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or HERMES_MEET_REALTIME_KEY in ~/.teamhermes/.env
```
...
pip install playwright websockets && python -m playwright install chromium
th plugins enable google_meet
th meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
th meet node approve my-mac ws://<mac-ip>:18789 <token>
th meet node ping my-mac                   # confirm reachable
```

Run `th meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `th meet auth` avoids this.
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `th meet node approve`.
