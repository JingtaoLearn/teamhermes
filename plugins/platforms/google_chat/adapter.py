...
# ``model_tools`` import time, so ``th status``, ``th chat``, etc.
...
#     fresh "TeamHermes is thinking…" card during the small window between
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            ...
            _hermes_home = _Path.home() / ".teamhermes"
        ...
        # they don't sit in the chat forever as "TeamHermes is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HERMES_HOME", str(_Path.home() / ".teamhermes"))
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'TeamHermes is thinking…' marker message.
        ...
        "TeamHermes is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "TeamHermes is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "TeamHermes is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "TeamHermes is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "TeamHermes is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``th setup``.
    ...
    prompt for env vars, persist them to ``~/.teamhermes/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.teamhermes/.env")
    print_info("Restart the gateway: th gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``th cron`` running as a
    separate process from ``th gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the TeamHermes plugin system at startup.
    ...
    ctx.register_platform(
        ...
        # the "TeamHermes is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'TeamHermes is thinking…' marker message "
