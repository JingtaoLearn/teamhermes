"""
LINE Messaging API platform adapter for TeamHermes Agent.
...
to TeamHermes Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        try:
            ...
            hermes_home = Path.home().joinpath(".teamhermes").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``thm status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``thm status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``thm setup line``.
    ...
    try:
        ...
        print("hermes_cli.config not available; set LINE_* vars manually in ~/.teamhermes/.env")
        return
