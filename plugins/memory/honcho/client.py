...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated TeamHermes instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a TeamHermes profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active TeamHermes profile.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      ...
      2. ~/.teamhermes/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into TeamHermes system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active TeamHermes profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. TeamHermes session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — TeamHermes session_id ({timestamp}_{hex})
        ...
        # per-session: inherit TeamHermes session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'th honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run th honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `th honcho setup` to configure)."
        )
