...
``{title, url, description, position}`` rows every other TeamHermes web
provider produces.
...
prefers TeamHermes-managed xAI Grok OAuth (via ``thm auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.teamhermes/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``thm tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `thm auth` to sign in with "
