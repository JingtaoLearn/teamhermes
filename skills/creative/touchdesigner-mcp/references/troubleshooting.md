...
## 5. TeamHermes Configuration

### Config location

`$HERMES_HOME/config.yaml` (defaults to `~/.teamhermes/config.yaml` when `HERMES_HOME` is unset)
...
Restart the TeamHermes session for changes to take effect. The MCP connection is
