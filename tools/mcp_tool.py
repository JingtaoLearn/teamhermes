...
# per-profile log file (~/.teamhermes/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "HERMES_HOME", os.path.join(os.path.expanduser("~"), ".teamhermes")
                )
            )
            candidates = [
                ...
                # image (which the TeamHermes Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → TeamHermes MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that TeamHermes gateways know how to render.
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ~/.teamhermes/logs/mcp-stderr.log.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until TeamHermes is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `thm mcp login {server_name}` (or delete the tokens "
            f"file under ~/.teamhermes/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the TeamHermes config file.
    ...
    ``os.environ`` (which includes ``~/.teamhermes/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through TeamHermes' MEDIA: tag convention and out to messaging
            ...
            # TeamHermes' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves TeamHermes's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the TeamHermes registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``thm tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the TeamHermes registry.
