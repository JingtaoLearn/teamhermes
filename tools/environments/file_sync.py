...
def iter_sync_files(container_base: str = "/root/.teamhermes") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.teamhermes to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.teamhermes", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.teamhermes/`` directory as a tar archive,
        ...
        # against an uninitialized remote .teamhermes/ directory.
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.teamhermes/skills/a.md`` →
        ``~/.teamhermes/skills/a.md``, a new remote file at
        ``/root/.teamhermes/skills/b.md`` maps to ``~/.teamhermes/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
