...
remediation hint pointing at ``th tools`` or the manual pip command.
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``th update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``th update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``th update``. Never raises; lazy-install failures
