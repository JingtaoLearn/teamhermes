"""
Lazy dependency installer for opt-in TeamHermes Agent backends.

Many TeamHermes features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
remediation hint pointing at ``thm tools`` or the manual pip command.
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``thm update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``thm update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``thm update``. Never raises; lazy-install failures
