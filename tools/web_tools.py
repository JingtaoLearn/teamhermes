...
Backend is selected during ``th tools`` setup (web.backend in config.yaml).
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``th tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `th tools` repaint.
        try:
