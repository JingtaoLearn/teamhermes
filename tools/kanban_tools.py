...
orchestrator work. A normal ``th chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``th kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``th kanban complete …``
   inside the container, where ``th`` isn't installed and the DB
...
Humans continue to use the CLI (``th kanban …``), the dashboard
(``th dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``th chat`` without the kanban toolset see zero
