...
orchestrator work. A normal ``thm chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``thm kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``thm kanban complete …``
   inside the container, where ``thm`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.teamhermes/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``thm kanban …``), the dashboard
(``thm dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``thm chat`` without the kanban toolset see zero
...
def _connect(board: Optional[str] = None):
    ...
    the env-pinned active board without restarting TeamHermes.
    """
