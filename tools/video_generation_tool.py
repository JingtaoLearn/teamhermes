...
provider** — turn on a backend by enabling a plugin (``th plugins
enable video_gen/<name>``) and selecting it in ``th tools`` → Video
Generation.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `th tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `th plugins list` to see "
            f"installed video gen backends, or `th tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `th tools` → "
...
# `th tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`th tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `th tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
