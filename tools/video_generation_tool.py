...
provider** — turn on a backend by enabling a plugin (``thm plugins
enable video_gen/<name>``) and selecting it in ``thm tools`` → Video
Generation.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `thm tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `thm plugins list` to see "
            f"installed video gen backends, or `thm tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `thm tools` → "
...
# `thm tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`thm tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `thm tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
