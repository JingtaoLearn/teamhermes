...
selectable via ``th tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `th tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`th setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `th tools` "
        "→ Image Generation (run `th plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
