...
~/.teamhermes/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `thm config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.teamhermes/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`thm curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.teamhermes/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER TeamHermes profiles.
    ...
    # Default profile (~/.teamhermes/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.teamhermes/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`thm -p "
            ...
            base += (
                ...
                f"Switch profiles (`thm -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "will refuse with a message pointing the user to `thm curator unpin <name>`. "
