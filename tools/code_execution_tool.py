...
Lets the LLM write a Python script that calls TeamHermes tools via RPC,
...
# available on every platform TeamHermes itself runs on.
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-TeamHermes-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-TeamHermes-profile soft guard."""',
...
"""Auto-generated TeamHermes tools RPC stubs."""
...
"""Auto-generated TeamHermes tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    to a subset of TeamHermes tools.
    ...
    try:
        ...
        # HERMES_TIMEZONE is an internal TeamHermes setting and must not leak
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.teamhermes/.env')).
...
def _load_config() -> dict:
    ...
    ``th --tui`` where it is never used.  Read the lightweight raw config
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``th tools`` (e.g. web is turned off),
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.teamhermes/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call TeamHermes tools programmatically. "
