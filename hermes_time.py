"""
Timezone-aware clock for TeamHermes.
...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.teamhermes/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — TeamHermes never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — TeamHermes requires 3.9+)
