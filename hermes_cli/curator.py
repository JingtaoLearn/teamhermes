"""CLI subcommand: `th curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `th curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`th curator status` and run `th curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.teamhermes/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`th curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.teamhermes/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.teamhermes/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.teamhermes/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="th curator")
    register_cli(parser)
