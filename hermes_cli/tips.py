...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or thm -c.",
    ...
    "Ctrl+Z suspends TeamHermes to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "thm -c resumes your most recent CLI session. thm -c \"project name\" resumes by title.",
    "thm -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "thm -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "thm chat -t web,terminal enables only specific toolsets for a focused session.",
    "thm chat -s github-pr-workflow preloads a skill at launch.",
    "thm chat -q \"query\" runs a single non-interactive query and exits.",
    "thm chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "thm chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "thm --yolo bypasses all dangerous command approval prompts for the entire session.",
    "thm chat --source telegram tags the session for filtering in thm sessions list.",
    "thm -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "thm doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "thm dump outputs a compact setup summary — great for bug reports.",
    "thm config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "thm config edit opens config.yaml in your default editor.",
    "thm config check scans for missing or stale configuration options.",
    "thm sessions browse opens an interactive session picker with search.",
    "thm sessions stats shows session counts by platform and database size.",
    "thm sessions prune --older-than 30 cleans up old sessions.",
    "thm skills search react --source skills-sh searches the skills.sh public directory.",
    "thm skills check scans installed hub skills for upstream updates.",
    "thm skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "thm skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "thm mcp add github --command npx adds MCP servers from the command line.",
    "thm mcp serve runs TeamHermes itself as an MCP server for other agents.",
    "thm auth add lets you add multiple API keys for credential pool rotation.",
    "thm completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "thm logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "thm backup creates a zip backup of your entire TeamHermes home directory.",
    "thm profile create coder creates an isolated profile that becomes its own command.",
    "thm profile create work --clone copies your current config and keys to a new profile.",
    "thm update syncs new bundled skills to ALL profiles automatically.",
    "thm gateway install sets up TeamHermes as a system service (systemd/launchd).",
    "thm memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "thm webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: thm tools disables unused tools, thm skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "thm models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as thm -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call TeamHermes tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'thm profile create coder' creates the 'coder' command.",
    "thm profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, TeamHermes prints a resume command with session ID and stats.",
    "thm sessions export backup.jsonl exports all sessions for backup or analysis.",
    "thm -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "thm skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: thm cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "TeamHermes runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "thm gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to TeamHermes via a local macOS server.",
    ...
    "TeamHermes loads project context from .teamhermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "thm mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "thm mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "thm mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — thm tools can toggle them per platform.",
    ...
    "thm chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "thm plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "thm acp runs TeamHermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "thm auth add nous or thm auth add openai-codex sets up OAuth-based providers.",
    ...
    "thm status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make TeamHermes your own.",
    ...
    "Before a gateway session reset, TeamHermes auto-flushes important facts to memory in the background.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'thm profile create ls' would be rejected.",
    "thm profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "thm claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via thm tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — TeamHermes auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of TeamHermes config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while TeamHermes is working.',
    ...
    # --- Curator ---
    'thm curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "thm curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'thm curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'thm auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'thm dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'thm dashboard --tui embeds the full TeamHermes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'thm -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'thm chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'thm chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'thm chat --ignore-user-config skips ~/.teamhermes/config.yaml — reproducible bug reports and CI runs.',
    "thm chat --source tool tags programmatic chats so they don't clutter thm sessions list.",
    'thm dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'thm sessions rename <ID> "new title" renames any past session; thm sessions delete <ID> removes one.',
    'thm import restores a session export or profile archive produced by sessions export or profile export.',
    'thm fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'thm pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'thm setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'thm status --deep runs the full health sweep across every component; plain thm status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes TeamHermes block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from thm sessions list by default — set --source explicitly to see them.',
