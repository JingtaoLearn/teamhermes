...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or th -c.",
    ...
    # --- CLI Flags ---
    "th -c resumes your most recent CLI session. th -c \"project name\" resumes by title.",
    "th -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "th -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "th chat -t web,terminal enables only specific toolsets for a focused session.",
    "th chat -s github-pr-workflow preloads a skill at launch.",
    "th chat -q \"query\" runs a single non-interactive query and exits.",
    "th chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "th chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "th --yolo bypasses all dangerous command approval prompts for the entire session.",
    "th chat --source telegram tags the session for filtering in th sessions list.",
    "th -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "th doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "th dump outputs a compact setup summary — great for bug reports.",
    "th config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "th config edit opens config.yaml in your default editor.",
    "th config check scans for missing or stale configuration options.",
    "th sessions browse opens an interactive session picker with search.",
    "th sessions stats shows session counts by platform and database size.",
    "th sessions prune --older-than 30 cleans up old sessions.",
    "th skills search react --source skills-sh searches the skills.sh public directory.",
    "th skills check scans installed hub skills for upstream updates.",
    "th skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "th skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "th mcp add github --command npx adds MCP servers from the command line.",
    "th mcp serve runs TeamHermes itself as an MCP server for other agents.",
    "th auth add lets you add multiple API keys for credential pool rotation.",
    "th completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "th logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "th backup creates a zip backup of your entire TeamHermes home directory.",
    "th profile create coder creates an isolated profile that becomes its own command.",
    "th profile create work --clone copies your current config and keys to a new profile.",
    "th update syncs new bundled skills to ALL profiles automatically.",
    "th gateway install sets up TeamHermes as a system service (systemd/launchd).",
    "th memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "th webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: th tools disables unused tools, th skills config trims skills down.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as th -w).",
    ...
    "Profile names become shell commands — 'th profile create coder' creates the 'coder' command.",
    "th profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "th sessions export backup.jsonl exports all sessions for backup or analysis.",
    "th -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "th skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: th cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "th gateway install sets it up as a system service that starts on boot.",
    ...
    # --- MCP ---
    "th mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "th mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "th mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — th tools can toggle them per platform.",
    ...
    "th chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "th plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "th acp runs TeamHermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "th auth add nous or th auth add openai-codex sets up OAuth-based providers.",
    ...
    "th status --deep runs deeper diagnostic checks across all components.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'th profile create ls' would be rejected.",
    "th profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "th claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via th tools.",
    ...
    # --- Curator ---
    'th curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "th curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'th curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'th auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'th dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'th dashboard --tui embeds the full TeamHermes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'th -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'th chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'th chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'th chat --ignore-user-config skips ~/.teamhermes/config.yaml — reproducible bug reports and CI runs.',
    "th chat --source tool tags programmatic chats so they don't clutter th sessions list.",
    'th dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'th sessions rename <ID> "new title" renames any past session; th sessions delete <ID> removes one.',
    'th import restores a session export or profile archive produced by sessions export or profile export.',
    'th fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'th pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'th setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'th status --deep runs the full health sweep across every component; plain th status is the quick view.',
    ...
    '--source tool chats are excluded from th sessions list by default — set --source explicitly to see them.',
