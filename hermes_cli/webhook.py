"""th webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.teamhermes/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: th gateway run
"""
...
def webhook_command(args):
    """Entry point for 'th webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: th webhook {subscribe|list|remove|test}")
        print("Run 'th webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (th gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: th webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from th webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (th gateway run)")
