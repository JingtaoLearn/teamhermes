"""thm webhook — manage dynamic webhook subscriptions from the CLI.
...
def _setup_hint() -> str:
  ...
  Then start the gateway: thm gateway run
"""
...
def webhook_command(args):
    """Entry point for 'thm webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: thm webhook {subscribe|list|remove|test}")
        print("Run 'thm webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (thm gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: thm webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from thm webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (thm gateway run)")
