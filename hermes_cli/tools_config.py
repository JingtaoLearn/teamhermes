...
`th tools` and `th setup tools` both enter this module.
...
# who want it opt in via `th tools` → Video Generation, which walks
...
# `th tools` → X (Twitter) Search setup walks users through credential
...
# Platform-scoped toolsets: only appear in the `th tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``th tools`` → "reconfigure existing" would
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``th update`` and
      by ``th computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `th update` calls this for every
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `th update` does not silently pull
        ...
        # Run the full `th auth spotify` flow — if the user has no
        ...
        try:
            ...
            _print_info("    Run manually: th auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `th auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: th auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: th auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `th auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: th auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: th auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # through ``th tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `th tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `th tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `th tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by th tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `th tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `th tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `th tools` and `th setup tools`.
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, th mcp
