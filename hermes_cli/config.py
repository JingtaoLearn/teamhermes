"""
Configuration management for TeamHermes Agent.

Config files are stored in ~/.teamhermes/ for easy access:
- ~/.teamhermes/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.teamhermes/.env         - API keys and secrets

This module provides:
- th config          - Show current configuration
- th config edit     - Open config in editor
- th config set      - Set a specific value
- th config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.teamhermes/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``th logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  th config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` TeamHermes makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. TeamHermes itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``th update``, the TUI build.
...
#   on every plugin install / ``th update``.
...
#   ``HERMES_ENV`` — TeamHermes runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # TeamHermes runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or TeamHermes runtime location "
            "(HERMES_HOME, HERMES_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.teamhermes/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `th plugins enable
...
def is_managed() -> bool:
    """Check if TeamHermes is running in package-manager-managed mode.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how TeamHermes was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.teamhermes/.install_method`` file (written by installers)
    ...
    so ``th update`` bailed with "doesn't apply inside the Docker
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.teamhermes/.install_method."""
    stamp = get_hermes_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* TeamHermes lives in a ``uv tool`` layout.
    ...
    the *active* TeamHermes is a regular pip/venv install, causing
    ``th update`` to upgrade the wrong copy. It would also block on a
...
# Long-form text for ``th update`` / ``--check`` when running inside the
...
#     — that script installs a *new* host-side TeamHermes, it doesn't update
...
✗ ``th update`` doesn't apply inside the Docker container.

TeamHermes Agent runs as a published image (nousresearch/hermes-agent), not a
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``th update`` inside Docker.
...
def format_managed_message(action: str = "modify this TeamHermes installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this TeamHermes installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.teamhermes-agent.settings in your configuration.nix and run:\n"
    ...
    return (
        f"Cannot {action}: this TeamHermes installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall TeamHermes."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/th")
...
def _is_container() -> bool:
    ...
    When TeamHermes runs in a container with volume-mounted config files, forcing
...
def ensure_hermes_home():
    """Ensure ~/.teamhermes directory structure exists with secure permissions.
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the TeamHermes-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps TeamHermes
    ...
    "terminal": {
        ...
        # skipped. When empty, TeamHermes auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), TeamHermes sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when TeamHermes
        ...
        # bundled TeamHermes image, which drops to the `th` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, TeamHermes sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and TeamHermes should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``th chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.teamhermes/checkpoints/`` size (MB).  When
    ...
    # single read_file page exceeds these limits, TeamHermes truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``th kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``th kanban decompose`` and the kanban
        ...
        # ``th profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `th model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `th --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `th -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `th --tui` drops a one-time hint
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.teamhermes/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.teamhermes/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, TeamHermes feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before TeamHermes auto-pauses the goal and
    ...
    # always goes to ~/.teamhermes/skills/.
    "skills": {
    ...
    # cron daemon.
    #
    # See `th curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.teamhermes/skills/ into
        # ~/.teamhermes/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `th curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *TeamHermes Agent*" header.
    ...
    # stored approval from ~/.teamhermes/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `th doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hermes_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow TeamHermes to lazy-install opt-in backend packages from PyPI
    ...
    # todos to ready, and fires `th -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `th` launches with
        ...
        # decomposition is manual via `th kanban decompose <id>` or
    ...
    #
    # Core TeamHermes tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.teamhermes/logs/.
    ...
    "model_catalog": {
        ...
        # next /model or `th model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.teamhermes/.env,
        ...
        # behavior: files must live under the TeamHermes cache, under
        ...
        # uploaded as native gateway attachments. Files inside the TeamHermes
        # cache (~/.teamhermes/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.teamhermes/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.teamhermes/sessions/session_{sid}.json`` on every turn
    ...
    # ``th update`` behaviour.
    "updates": {
        # Run a full ``th backup``-style zip of HERMES_HOME before every
        # ``th update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        # can be restored with ``th import <path>``.  Off by default —
    ...
    # `th tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.teamhermes/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.teamhermes/.env (or your shell) and never in config.yaml.
            ...
            # ~/.teamhermes/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'th model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the TeamHermes auth store)",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote TeamHermes API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote TeamHermes API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote TeamHermes API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.teamhermes.config`` entries, then checks
    ...
    try:
        ...
        # should never break `th update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older TeamHermes versions) write ``models`` as
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'th setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — TeamHermes won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'th doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HERMES_HOME", "~/.teamhermes")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `th plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `th curator status` has no
    ...
    #   3. Creates `~/.teamhermes/logs/curator/` if missing (belt-and-suspenders
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: th config set <key> <value>")
    ...
    # metadata.teamhermes.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.teamhermes/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.teamhermes/config.yaml.
...
#   openai-codex (OAuth — th auth) — OpenAI Codex
#   nous         (OAuth — th auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.teamhermes/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.teamhermes/.env.
    ...
    interactive menu render (`th tools`, `th setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `th tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real TeamHermes env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.teamhermes/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.teamhermes/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.teamhermes/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.teamhermes/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to TeamHermes — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known TeamHermes vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.teamhermes/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ TeamHermes Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  th config edit     # Edit config file", Colors.DIM))
    print(color("  th config set <key> <value>", Colors.DIM))
    print(color("  th setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: th config set <key> <value>")
            print()
            print("Examples:")
            print("  th config set model anthropic/claude-sonnet-4")
            print("  th config set terminal.backend docker")
            print("  th config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'th config migrate' to add them")
        ...
        print("  th config           Show current configuration")
        print("  th config edit      Open config in editor")
        print("  th config set <key> <value>   Set a config value")
        print("  th config check     Check for missing/outdated config")
        print("  th config migrate   Update config with new options")
        print("  th config path      Show config file path")
        print("  th config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``th config`` UI so users
