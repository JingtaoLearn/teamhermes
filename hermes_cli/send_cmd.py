"""CLI subcommand: ``th send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"th send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"th send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"th send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"th send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"th send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `th gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.teamhermes/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.teamhermes/.env`` AND bridge top-level
    ...
    startup that ``th send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.teamhermes/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.teamhermes/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``th send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.teamhermes/.env and ~/.teamhermes/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "th send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  th send --to telegram \"hello\"\n"
            "  th send --to discord:#ops --file report.md\n"
            "  th send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "th send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `th send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform TeamHermes "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.teamhermes/.env + ~/.teamhermes/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `th send --list telegram`.",
    )
