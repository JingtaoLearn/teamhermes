"""CLI subcommand: ``thm send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"thm send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"thm send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"thm send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"thm send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"thm send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `thm gateway setup`, or run the gateway once so")
...
def _load_hermes_env() -> None:
    ...
    startup that ``thm send`` must replicate when invoked standalone:
    ...
    intentionally reimplement the minimum needed here so ``thm send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if not target:
        print(
            "thm send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  thm send --to telegram \"hello\"\n"
            "  thm send --to discord:#ops --file report.md\n"
            "  thm send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "thm send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `thm send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `thm send --list telegram`.",
    )
