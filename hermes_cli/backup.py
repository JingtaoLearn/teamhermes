...
`th backup` creates a zip archive of the entire ~/.teamhermes/ directory
(excluding the hermes-agent repo and transient files).

`th import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
def run_backup(args) -> None:
    """Create a zip backup of the TeamHermes home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: TeamHermes home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    print(f"\nRestore with: th import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a TeamHermes backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be a TeamHermes backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.teamhermes/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        if prefix in {".teamhermes", "hermes"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has TeamHermes configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: th profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "hermes-agent").is_dir():
            print("Note: The hermes-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: th update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  th -p {pname} gateway install")

        print("Done. Your TeamHermes configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and th backup --quick)
# ---------------------------------------------------------------------------
...
# — `th update` snapshots this set before pulling so approved-user lists
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``th update``.
    ...
    Args:
        ...
        hermes_home: Override for the TeamHermes home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for th backup --quick."""
...
def create_pre_update_backup(
    ...
    (``th update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `th claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `th import` and the
