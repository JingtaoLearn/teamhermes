...
`thm import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to thm root) should be skipped."""
...
def run_backup(args) -> None:
    ...
    print(f"\nRestore with: thm import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a thm home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a thm dir name
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: thm profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "hermes-agent").is_dir():
            print("Note: The hermes-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: thm update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  thm -p {pname} gateway install")

        print("Done. Your TeamHermes configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and thm backup --quick)
# ---------------------------------------------------------------------------
...
def run_quick_backup(args) -> None:
    """CLI entry point for thm backup --quick."""
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `thm claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `thm import` and the
