"""CLI handlers for ``thm secrets bitwarden ...``.
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Status:  [cyan]thm secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]thm secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]thm secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]thm secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — TeamHermes will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`thm secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]thm secrets bitwarden setup[/cyan] and pick EU or "
