...
entries via ``thm mcp catalog`` or the interactive ``thm mcp picker``,
and install them with ``thm mcp install <name>`` (or by toggling in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``thm mcp install <name>`` to
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this TeamHermes install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this TeamHermes understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their TeamHermes is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this TeamHermes
        understands. Update TeamHermes to install this entry.
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``thm mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `thm mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `thm mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `thm mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `thm mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `thm auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`thm auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new TeamHermes session to load its tools.",
        Colors.GREEN,
    ))
