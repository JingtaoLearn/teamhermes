"""``th debug`` debug tools for TeamHermes Agent.

Currently supports:
    th debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[th debug share: log content redacted at upload time. "
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``th debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``th debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `th debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``th debug share`` invocation added ~20 MB of resident Python
    ...
    ``th debug share`` also runs an opportunistic sweep as a fallback
...
def _capture_dump() -> str:
    """Run ``th dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``th dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "th debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  th debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: th debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'th debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``th debug`` call.
    ...
    # best-effort — any failure is swallowed so ``th debug`` stays
