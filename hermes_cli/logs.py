"""``th logs`` — view and filter TeamHermes log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.teamhermes/logs/``.

Usage examples::

    th logs                    # last 50 lines of agent.log
    th logs -f                 # follow agent.log in real time
    th logs errors             # last 50 lines of errors.log
    th logs gateway -n 100    # last 100 lines of gateway.log
    th logs --level WARNING    # only WARNING+ lines
    th logs --session abc123   # filter by session ID substring
    th logs --component tools  # only tool-related lines
    th logs --since 1h         # lines from the last hour
    th logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when TeamHermes runs — try 'th chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'th chat' to generate logs)")
