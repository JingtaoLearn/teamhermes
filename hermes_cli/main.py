...
Usage:
    ...
    th chat                # Interactive chat
    th gateway             # Run gateway in foreground
    th gateway start       # Start gateway as service
    th gateway stop        # Stop gateway service
    th gateway status      # Show gateway status
    th gateway install     # Install gateway service
    th gateway uninstall   # Uninstall gateway service
    th setup               # Interactive setup wizard
    th logout              # Clear stored authentication
    th status              # Show status of all components
    th cron                # Manage cron jobs
    th cron list           # List cron jobs
    th cron status         # Check if cron scheduler is running
    th doctor              # Check configuration and dependencies
    th honcho setup                    # Configure Honcho AI memory integration
    th honcho status                   # Show Honcho config and connection status
    th honcho sessions                 # List directory → session name mappings
    th honcho map <name>               # Map current directory to a session name
    th honcho peer                     # Show peer names and dialectic settings
    th honcho peer --user NAME         # Set user peer name
    th honcho peer --ai NAME           # Set AI peer name
    th honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    th honcho mode                     # Show current memory mode
    th honcho mode [hybrid|honcho|local]  # Set memory mode
    th honcho tokens                   # Show token budget settings
    th honcho tokens --context N       # Set session.context() token cap
    th honcho tokens --dialectic N     # Set dialectic result char cap
    th honcho identity                 # Show AI peer identity representation
    th honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    th honcho migrate                  # Step-by-step migration guide: OpenClaw native → TeamHermes + Honcho
    th version             Show version
    th update              Update to latest version
    th uninstall           Uninstall TeamHermes Agent
    th acp                 Run as an ACP server for editor integration
    th sessions browse     Interactive session picker with search

    th claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``th update``
...
# ``th update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``th tools`` or
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not (os.environ.get("HERMES_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`th --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``th --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (th tools, th setup, th model) use
...
def _apply_profile_override() -> None:
    ...
    # `th profile use` and the gateway should honour that choice.
    # See issue #22502.
...
# Initialize centralized file logging early — all `th` subcommands
...
def _print_tui_exit_summary(
    ...
    print(f"  th --tui --resume {target}")
    if title:
        print(f'  th --tui -c "{title}"')
    print()
...
def _launch_tui(
    ...
    # in the user's shell would otherwise make a plain `th --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `th update` so
...
def _pin_kanban_board_env() -> None:
    ...
    (``th kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``th kanban boards switch`` from another session can flip the file
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'th sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'th doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'th setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `th gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: th gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `th gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  th gateway")
        ...
        print()
        print("  Or install as a service: th gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'th whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``th model``) and the setup wizard
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'th model' for "
                "available providers, or run 'th doctor' to diagnose config "
                "issues."
            )
...
# `th model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `th model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``th model`` flow, then route aux tasks to them here.
    """
    from hermes_cli.config import load_config
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # bad paste short of re-running `th setup` from scratch. OpenRouter
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `th auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``th model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``th model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``th auth add xai-oauth``).  Fall
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `th model` to set a model.")
...
# the catalog eagerly costs ~55ms on every `th` invocation — including
# fast paths like `th --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``th setup`` / ``th model``.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `th setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'th model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_slack(args):
    ...
    Dispatches ``th slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: th slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `th slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
# Critical files that every ``th`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``th update`` again to roll forward. The post-pull syntax
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``th`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``th update --gateway`` so interactive prompts (stash restore,
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``th`` launcher present but ``hermes_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``th update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``th web``).
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (th web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``th dashboard`` processes other than ourselves.

    ``th dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``th update`` replaces files on disk, the running
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `th update`.
    ...
    print("  Preview now:  th curator run --dry-run")
    print("  Pause it:     th curator pause")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``th update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``th update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: th curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``th dashboard`` processes.

    Called at the end of ``th update`` (default ``reason``) and also
    from ``th dashboard --stop`` (which overrides ``reason``).  The
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `th update "
            f"--branch {branch}`, or update against main with `th update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    # optional — a failure here only affects ``th dashboard``. Make
...
# =========================================================================
# Fork detection and upstream management for `th update`
# =========================================================================
...
def _invalidate_update_cache():
    ...
    ``th update``, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``th update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    so during ``th update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``th update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``th update``
    ...
    #      hermes.exe sitting *under* a non-TeamHermes parent (e.g. a TeamHermes
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close TeamHermes Desktop, exit any open `th` REPLs, and")
    lines.append("  stop the gateway (`th gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `th update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``th update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close TeamHermes Desktop, exit other `th` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `th update`."
        )
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``th update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `th update` once the upstream issue is resolved.")
...
class _UpdateOutputStream:
    """Stream wrapper used during ``th update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``th update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``th update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``th update --gateway``) the update is already
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``th update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``th update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``th update`` without requiring a reinstall.  The
    ...
    ``th`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/th.
    ...
    # /usr/local/bin/th, code at /usr/local/lib/hermes-agent).
    fhs_link = Path("/usr/local/bin/th")
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``th update``
    ...
    print(f"  Restore:  th import {out_path}")
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `th update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # every user who ran ``th update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``th update`` again later once a fix lands.")
                    else:
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: th config migrate")
            else:
                print()
                print("Skipped. Run 'th config migrate' later to configure.")
        else:
        ...
        # check `th curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``th update`` gives users a predictable
        ...
        # When running as ``th update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: th gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: th -p <profile> gateway run  for each profile)"
                        )
        ...
        # When both th.service (from a pre-rename install) and the
        # current hermes-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `th update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("  These pre-rename units (th.service) fight the current")
                print("  hermes-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    th gateway migrate-legacy")
                print()
        ...
        print("  th model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``th -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `th profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → th -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  th profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     th -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `th profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  th -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      th -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `th profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"th -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
...
def _report_dashboard_status() -> int:
    """Print ``th dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``th dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No th dashboard processes running.")
        return 0

    print(f"{len(pids)} th dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No th dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `th update`.
...
# so that in ``th -m gpt5 chat``, ``gpt5`` is correctly skipped as a
...
def _first_positional_argv() -> str | None:
    ...
    ``th -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``th`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `th --tui` is the hot path on phones. The full parser setup imports
...
def main():
    ...
    # ``th update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `th model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy th.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy TeamHermes gateway unit files "
            "(e.g. th.service) left over from older installs. Profile "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `th proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        "Run a specific section: th setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'th setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for TeamHermes Agent. Use 'th debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    th debug share              Upload debug report and print URL
    th debug share --lines 500  Include more log lines
    th debug share --expire 30  Keep paste for 30 days
    th debug share --local      Print report locally (no upload)
    th debug share --no-redact  Disable upload-time secret redaction
    th debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'th debug share'",
    )
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "so future 'th update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `th plugins enable <name>`",
    )
    ...
    # subcommand — ``th --help``, ``th version``, ``th logs``,
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'th tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # th tools disable <name...> [--platform cli]
    ...
    # th tools enable <name...> [--platform cli]
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `th computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `th tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: th computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: th computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'th mcp add' to connect to a new server, or\n"
            "'th mcp serve' to expose TeamHermes conversations over MCP."
        ),
    )
    ...
    # flag is omitted, causing `th mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `th mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `th mcp install n8n`)",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch th --resume <id> by replacing the current process
    ...
    claw_migrate.add_argument(
        ...
        "before apply; restorable with 'th import').",
    )
    claw_migrate.add_argument(
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'th config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `th update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `th profile describe`.",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `th --tui` via PTY/WebSocket). "
    ...
    # for `th dashboard` cmdlines and SIGTERM them directly — the same
    # path `th update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running th dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running th dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    th logs                    Show last 50 lines of agent.log
    th logs -f                 Follow agent.log in real time
    th logs errors             Show last 50 lines of errors.log
    th logs gateway -n 100     Show last 100 lines of gateway.log
    th logs --level WARNING    Only show WARNING and above
    th logs --session abc123   Filter by session ID
    th logs --component tools  Only show tool-related lines
    th logs --since 1h         Lines from the last hour
    th logs --since 30m -f     Follow, starting from 30 min ago
    th logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``th -c Pokemon Agent Dev`` → ``th -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'th -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (th hooks list, cron
