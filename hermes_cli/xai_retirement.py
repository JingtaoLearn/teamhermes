...
and reusable from both `thm doctor` and a future `thm migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a TeamHermes config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a TeamHermes config and return retirement issues.
