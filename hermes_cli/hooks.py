"""th hooks — inspect and manage shell-script hooks.

Usage::

    th hooks list
    th hooks test <event> [--for-tool X] [--payload-file F]
    th hooks revoke <command>
    th hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``th hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: th hooks {list|test|revoke|doctor}")
        print("Run 'th hooks --help' for details.")
        return
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `th hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `th hooks test` and `th hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `th hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `th hooks doctor` would execute
