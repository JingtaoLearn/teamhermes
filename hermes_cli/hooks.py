"""thm hooks — inspect and manage shell-script hooks.

Usage::

    thm hooks list
    thm hooks test <event> [--for-tool X] [--payload-file F]
    thm hooks revoke <command>
    thm hooks doctor

Consent records live under ``~/.teamhermes/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.teamhermes/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``thm hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: thm hooks {list|test|revoke|doctor}")
        print("Run 'thm hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.teamhermes/config.yaml.")
        print("See `thm hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `thm hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `thm hooks test` and `thm hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (TeamHermes wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `thm hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `thm hooks doctor` would execute
