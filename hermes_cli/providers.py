"""
Single source of truth for provider identity in TeamHermes Agent.
...
2. **TeamHermes overlays** — transport type, auth patterns, aggregator flags,
...
# -- TeamHermes overlay ----------------------------------------------------------
# TeamHermes-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """TeamHermes-specific provider metadata layered on top of models.dev."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. TeamHermes overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + TeamHermes overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + thm extra
    ...
    if overlay is not None:
        # TeamHermes-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
