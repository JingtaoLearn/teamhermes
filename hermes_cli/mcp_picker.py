"""MCP picker — interactive `thm mcp picker` (also the default `thm mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``thm mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `thm plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new TeamHermes session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `thm mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: thm mcp install <name>    Picker: thm mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their TeamHermes is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer TeamHermes — run `thm update` "
...
def show_catalog() -> None:
    """`thm mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`thm mcp picker` (and default `thm mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`thm mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `thm mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
