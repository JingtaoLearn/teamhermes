"""MCP picker — interactive `th mcp picker` (also the default `th mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``th mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `th plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `th mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: th mcp install <name>    Picker: th mcp",
        Colors.DIM,
    ))
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer TeamHermes — run `th update` "
...
def show_catalog() -> None:
    """`th mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`th mcp picker` (and default `th mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`th mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `th mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
