...
def check_for_updates() -> Optional[int]:
    """Check whether a TeamHermes update is available.
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active TeamHermes git checkout, or None if this isn't a git install.
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    TeamHermes checkout. Cached per-process. Release URL always points at the
...
def format_banner_version_label() -> str:
    ...
    base = f"TeamHermes Agent v{VERSION} ({RELEASE_DATE})"
