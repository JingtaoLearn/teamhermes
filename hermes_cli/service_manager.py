...
``th gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``th gateway start/stop/restart``).
    """
...
# active (set via the ``th -p <profile>`` flag before the call). This
...
#
# Per-profile gateways are registered dynamically when `th profile create`
...
# UID/GID of the in-image ``th`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``th`` user — UID 10000 — running every TeamHermes runtime
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``th -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`th profile create {profile}` first, or pass "
    ...
    def _render_run_script(
        ...
        The script:
          ...
          4. Drops to the hermes user and exec's
             ``th -p <profile> gateway run`` (or just ``hermes
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``th gateway
        ...
        if profile == "default":
            lines.append("exec s6-setuidgid hermes th gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid hermes th -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``th logs`` reads and what
