...
Usage:
    thm pairing list              # Show all pending + approved users
    thm pairing approve <platform> <code>  # Approve a pairing code
    thm pairing revoke <platform> <user_id> # Revoke user access
    thm pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle thm pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: thm pairing {list|approve|revoke|clear-pending}")
        print("Run 'thm pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.teamhermes/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'thm pairing list' to see pending codes.\n")
