"""`th checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.teamhermes/checkpoints/``.  Actions:

    th checkpoints               # same as `status`
    th checkpoints status        # total size, project count, breakdown
    th checkpoints list          # per-project checkpoint counts + workdir
    th checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    th checkpoints clear [-f]    # nuke the entire base (asks first)
    th checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    th checkpoints
    th checkpoints prune --retention-days 3 --max-size-mb 200
    th checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: th checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``th checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `th checkpoints` → status
