"""`thm checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.teamhermes/checkpoints/``.  Actions:

    thm checkpoints               # same as `status`
    thm checkpoints status        # total size, project count, breakdown
    thm checkpoints list          # per-project checkpoint counts + workdir
    thm checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    thm checkpoints clear [-f]    # nuke the entire base (asks first)
    thm checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    thm checkpoints
    thm checkpoints prune --retention-days 3 --max-size-mb 200
    thm checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: thm checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``thm checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `thm checkpoints` → status
