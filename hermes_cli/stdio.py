...
on Japanese locales, etc.).  TeamHermes's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. TeamHermes is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the TeamHermes-managed Git install directories so
    ...
    # install without TeamHermes-managed PortableGit).
    ...
    # stdin is re-configured for completeness; TeamHermes's interactive
...
def _default_windows_editor() -> str:
    ...
    ``thm config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching TeamHermes (User env var in Windows Settings, or
    export in a PowerShell profile) and TeamHermes picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known TeamHermes-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``thm`` from right after install) retain their old PATH.
    ...
    Patch-up strategy: add the known TeamHermes-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # TeamHermes venv Scripts directory — host of the hermes.exe shim itself,
