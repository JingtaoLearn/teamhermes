...
Handles: th gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hermes_constants is cached from a pre-update version during `th update`.
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``th gateway status`` from falsely counting the ``th`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``th gateway status``) is never mistaken for a running
    # gateway.  See #13242.
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``th update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            TeamHermes profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to TeamHermes profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``th update`` users
    # with no running gateway until they re-invoke ``th gateway``
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_hermes_home` falls back to ``/root/.teamhermes`` — the wrong
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}th gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}th gateway restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `th gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``th gateway status`` output so users with
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our TeamHermes Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``th gateway run`` must remain interruptible from
...
SERVICE_DESCRIPTION = "TeamHermes Agent Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None) -> str:
    ...
    For ``~/.teamhermes/profiles/<name>``, returns ``"--profile <name>"``.
...
def get_service_name() -> str:
    ...
    Default ``~/.teamhermes`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.teamhermes/profiles/coder`` returns ``hermes-gateway-coder``.
...
# Legacy service names from older TeamHermes installs that predate the
...
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("th.service",)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy TeamHermes gateway units.

    Detects unit files installed by older TeamHermes versions that used a
    different service name (e.g. ``th.service`` before the rename to
    ...
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``th.service`` running an unrelated
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy TeamHermes gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy TeamHermes gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy TeamHermes gateway unit(s) detected from an older install:")
    ...
    print_info("    th gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy TeamHermes gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy TeamHermes gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: th gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo th gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    th gateway uninstall")
    print_info("    sudo th gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so TeamHermes can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo th gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo th gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo th gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode TeamHermes often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.teamhermes`` → ``ai.teamhermes.gateway.plist`` (backward compatible).
    Profile ``~/.teamhermes/profiles/coder`` → ``ai.teamhermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.teamhermes.gateway-{suffix}" if suffix else "ai.teamhermes.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.teamhermes/hermes-agent  -> /home/alice/.teamhermes/hermes-agent
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      /root/.teamhermes                    → /home/alice/.teamhermes
      /root/.teamhermes/profiles/coder     → /home/alice/.teamhermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_hermes_home().resolve()
    current_default = (Path.home() / ".teamhermes").resolve()
    target_default = Path(target_home_dir) / ".teamhermes"

    # Default ~/.teamhermes → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.teamhermes → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.teamhermes) — keep as-is
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``th update`` later relocates/removes — the path
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current TeamHermes install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo th gateway uninstall --system")
    print_info("         th gateway install")
    print_info("         th gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (th.service from pre-rename installs)
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}th gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}th gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}th gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `th gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `th gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}th gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}th gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}th gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}th gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.teamhermes.gateway-{suffix}" if suffix else "ai.teamhermes.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current TeamHermes install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  th gateway status             # Check status")
...
def launchd_stop():
    ...
    # `th gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current TeamHermes install")
    else:
        print("⚠ Service definition is stale relative to the current TeamHermes install")
        print("  Run: th gateway start")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the TeamHermes gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the TeamHermes command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `th gateway run` still
    ...
    # /restart) rather than through `th gateway restart` which already
    ...
    # `th gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ TeamHermes Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and TeamHermes will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where TeamHermes delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your TeamHermes agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address TeamHermes will use (e.g., hermes@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. TeamHermes connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: th pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and TeamHermes will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``th setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.teamhermes/plugins/ still require
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'th pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: th pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'th gateway setup'")
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. TeamHermes will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. TeamHermes will store the returned account_id/token in ~/.teamhermes/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `th gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `th pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `th pairing approve`.")
    elif access_idx == 1:
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.teamhermes/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.teamhermes/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: th gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: th gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: th gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo th gateway install --system")
                    print_info("  Or run in foreground:  th gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: th gateway run")
                print_info("  For persistence:   tmux new -s hermes 'th gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from hermes_constants import display_hermes_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: th gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup th gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: th gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'th gateway setup' when ready.")
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``th gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``th gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: th gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: th gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s hermes 'th gateway run'")
                print()
            ...
            print()
            print("  th gateway run                              # direct foreground")
            print("  tmux new -s hermes 'th gateway run'         # persistent via tmux")
            print("  nohup th gateway run > ~/.teamhermes/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  th profile create <name>     # creates the s6 service slot")
                print("  th -p <name> gateway start   # bring it up via s6")
                print("  th status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: th gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: th gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: th gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  th profile delete <name>     # tears down the s6 service slot")
                print("  th -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `th -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: th gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: th gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  th gateway install")
                else:
                    print("To install as a service:")
                    print("  th gateway install")
                    print("  sudo th gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  th gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup th gateway run > ~/.teamhermes/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'th gateway run'         # persistent via tmux")
                    print("  nohup th gateway run > ~/.teamhermes/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  th gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  th gateway install  # Install as user service")
                    print("  sudo th gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy TeamHermes gateway unit files from
        # pre-rename installs (e.g. th.service). Profile units and
        # unrelated third-party services are never touched.
