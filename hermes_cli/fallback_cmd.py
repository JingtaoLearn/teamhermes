"""
th fallback — manage the fallback provider chain.
...
Subcommands:
  th fallback [list]   Show the current fallback chain (default when no subcommand)
  th fallback add      Pick provider + model via the same picker as `th model`,
                           then append the selection to the chain
  th fallback remove   Pick an entry to delete from the chain
  th fallback clear    Remove all fallback entries
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `th model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  th fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `th model`, then append the selection to the chain."""
    ...
    print("  `th model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `th fallback list` to view, or `th fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``th fallback [subcommand]``."""
