...
# xAI's public API is OpenAI-compatible for the endpoints TeamHermes commonly
# uses. The Responses endpoint is included because TeamHermes' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via TeamHermes-managed OAuth credentials."""

    auth_hint = "thm auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`thm auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`thm auth reset xai-oauth` or re-authenticate with "
                    "`thm auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `thm auth add xai-oauth --type oauth`."
            )
