...
``thm -p <profile>`` joins the same board as the dispatcher that
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``thm kanban boards
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``thm kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``thm kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``thm kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``thm kanban log`` unambiguous even when multiple
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``thm kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``thm --skills ...`` alongside the built-in
    ...
    # `thm --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``thm kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `thm kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``thm kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``thm kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``thm kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``thm kanban complete <id>``) works without requiring
...
#      via ``thm kanban show <id>`` and the dashboard.
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``thm kanban
...
def dispatch_once(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        # `_default_spawn` invokes ``thm -p <assignee>`` which fails
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `thm kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    ...
    # pyproject.toml, NOT a top-level ``thm`` package — there is no
    # ``thm`` package to import.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``thm`` invocation as argv parts for ``Popen``.
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       TeamHermes is launched from a venv and the ``thm`` shim is not on
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``thm -p <name>``), its ``SKILLS_DIR``
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``thm -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `thm -p <name>` the
    ...
    # that even when the worker activates a profile (`thm -p <name>`
    ...
    # `thm -p <assignee>` activates the profile, but the env var is
    ...
    # `thm kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`thm` executable not found on PATH. "
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``thm kanban daemon`` is systemd-friendly.
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``thm kanban assignees`` for the terminal.
