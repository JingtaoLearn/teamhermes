...
``<root>`` is the **shared TeamHermes root** (the parent of any active
...
``th -p <profile>`` joins the same board as the dispatcher that
...
the profile's TeamHermes home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``th kanban boards
...
def kanban_home() -> Path:
    """Return the shared TeamHermes root that anchors the kanban board.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``th kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``th kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``th kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``th kanban log`` unambiguous even when multiple
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources TeamHermes treats as user-controlled-but-trusted on the
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``th kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``th --skills ...`` alongside the built-in
    ...
    # `th --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``th kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `th kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``th kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``th kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``th kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``th kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold TeamHermes' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside TeamHermes-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``th kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a TeamHermes
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``th kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real TeamHermes profile.
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real TeamHermes profile.
        # `_default_spawn` invokes ``th -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a TeamHermes
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `th kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound TeamHermes CLI invocation."""
    # ``hermes_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``th`` package — there is no
    # ``th`` package to import.
    return [sys.executable, "-m", "hermes_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved TeamHermes shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved TeamHermes executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``th`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("th")`` — the console-script shim, normalized to
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       TeamHermes is launched from a venv and the ``th`` shim is not on
    ...
    hermes_bin = _safe_which_no_cwd("hermes") if _IS_WINDOWS else shutil.which("th")
    if hermes_bin:
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``th -p <name>``), its ``SKILLS_DIR``
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``th -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `th -p <name>` the
    ...
    # that even when the worker activates a profile (`th -p <name>`
    ...
    # `th -p <assignee>` activates the profile, but the env var is
    ...
    # `th kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`th` executable not found on PATH. "
            "Install TeamHermes Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``th kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default TeamHermes root exists
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``th kanban assignees`` for the terminal.
