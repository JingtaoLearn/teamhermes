...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``th skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``th skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``th skills list`` and
...
def slack_subcommand_map() -> dict[str, str]:
    ...
    Maps both canonical names and aliases so /th bg do stuff works
