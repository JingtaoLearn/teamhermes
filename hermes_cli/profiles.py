...
Usage::
    ...
    th profile create coder          # fresh profile + bundled skills
    th profile create coder --clone  # also copy config, .env, SOUL.md, skills
    th profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    th -p coder chat                 # or via flag
    th profile use coder             # set as sticky default
    th profile delete coder          # remove profile + alias + service
"""
...
# Marker file written by `th profile create --no-skills`.  When present in
...
# `th skills install` or drop SKILL.md files into the profile's skills/.
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``th``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``th`` profile inside ``~/.teamhermes/``) or get refused
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``th profile list`` for an unrelated profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``th update`` skips re-seeding this profile's
    ...
    # Write the opt-out marker so seed_profile_skills() and `th update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`th profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `th update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `th profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `th -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``th profile create
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            ...
            "To remove everything, use: th uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: th profile create {canon}"
        )
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: th profile import <archive> --name <name>"
        )
    ...
    if canon == "default":
        raise ValueError(
            ...
            "Specify a different name: th profile import <archive> --name <name>"
        )
