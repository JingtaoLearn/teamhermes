...
def _hermes_home_path() -> Path:
    ...
    try:
        ...
        return Path(os.path.expanduser("~/.teamhermes"))


def _hermes_root_path() -> Path:
    """Resolve the TeamHermes root dir (always the parent of any profile, never per-profile)."""
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # TeamHermes control-plane files: block both the ACTIVE profile's view
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied TeamHermes path.

    Three categories are blocked:

      * Internal TeamHermes cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global TeamHermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.teamhermes/.env`` and exfiltrate the file. The read-deny exists
    ...
    # TeamHermes root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal TeamHermes cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a TeamHermes credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the TeamHermes MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a TeamHermes MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# TeamHermes profiles are separate HERMES_HOME dirs under
...
# edited skills under both ``~/.teamhermes/profiles/hermes-security/skills/``
# AND ``~/.teamhermes/skills/`` (the default profile's skills) without realizing
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside TeamHermes scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    TeamHermes entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to TeamHermes profile {info['target_profile']!r}, but the "
