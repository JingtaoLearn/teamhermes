...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``th model`` / ``th auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another TeamHermes process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``th`` invocation that defaults to the active provider
        (e.g. setup wizard, ``th auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another TeamHermes
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another TeamHermes process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other TeamHermes profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `th model` / `th auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `th model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``th auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `th auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `th auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, TeamHermes PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `th setup` ("Claude
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `th auth add nous --label <name>`).  Fall back to the
        ...
        # TeamHermes auth store or env vars, so resolve them here.
        ...
        # standard `th auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `th auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the TeamHermes auth store.  Without this gate the removal is instantly
        ...
        # TeamHermes owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``th model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``th auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.teamhermes/.env over os.environ — the user's config file is the
    # authoritative source for TeamHermes credentials. Stale env vars from parent
    ...
    # Honour user suppression — `th auth remove <provider> <N>` for an
    ...
    if provider == "openrouter":
        # Prefer ~/.teamhermes/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # TeamHermes PKCE is TeamHermes-owned/persistable while present, but it is
