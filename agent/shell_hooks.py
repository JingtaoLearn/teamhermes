...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``th hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
# ---------------------------------------------------------------------------
# Introspection (used by `th hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``th hooks test`` and ``th hooks doctor``.
    ...
    produce — otherwise scripts tested via ``th hooks test`` could
