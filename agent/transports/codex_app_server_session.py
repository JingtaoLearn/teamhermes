...
Owns one Codex thread per TeamHermes session. Drives `turn/start`, consumes
...
# TeamHermes' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse TeamHermes/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per TeamHermes session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        into TeamHermes' messages shape.
        ...
        # supports rich content but TeamHermes' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into TeamHermes' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved TeamHermes' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map TeamHermes approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    TeamHermes returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort TeamHermes version string for codex's userAgent line."""
    try:
