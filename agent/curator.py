...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``th update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``th curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `th curator run --dry-run` to preview now"
            )
            save_state(state)
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every th install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: th curator status
        keep an umbrella stable: th curator pin document-tools
    ...
    lines.append("full report: th curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: th curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "safety and can be restored via `th curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Restore any via `th curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `th curator restore <name>`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `th curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `th curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``th model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `th model` → auxiliary
