...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.teamhermes/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``thm update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``thm curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `thm curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as TeamHermes' background skill CURATOR. This is an "
    ...
    "into ~/.teamhermes/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.teamhermes/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.teamhermes/logs/curator/``)
    ...
    authored skill data in ``~/.teamhermes/skills/``.
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every thm install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: thm curator status
        keep an umbrella stable: thm curator pin document-tools
    ...
    lines.append("full report: thm curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: thm curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.teamhermes/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.teamhermes/skills/.archive/` for "
            "safety and can be restored via `thm curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.teamhermes/skills/.archive/`. "
            "Restore any via `thm curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `thm curator restore <name>`")
    lines.append("- All archives live under `~/.teamhermes/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `thm curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `thm curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``thm model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `thm model` → auxiliary
