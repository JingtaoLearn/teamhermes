...
appropriate.  All installs go to a TeamHermes-owned bin staging dir,
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``thm lsp
  status``.
...
def hermes_lsp_bin_dir() -> Path:
    """Return the TeamHermes-owned bin staging dir for LSP servers."""
    home = os.environ.get("HERMES_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".teamhermes")
    p = Path(home) / "lsp" / "bin"
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``thm lsp status`` CLI to give users a quick
