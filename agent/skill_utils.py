...
# which becomes the dominant cost of ``thm`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.teamhermes/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``thm`` cold-start time
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.teamhermes/skills/`` first, then external.
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes TeamHermes metadata, VCS, virtualenv/dependency, and cache
