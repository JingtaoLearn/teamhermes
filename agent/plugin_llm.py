...
Plugins built on TeamHermes Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing TeamHermes
...
embedded slugs, no shorthands. This mirrors TeamHermes' main config
...
handles every provider, fallback chain, and per-task override TeamHermes
supports.
"""
