...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time TeamHermes starts and finds ``~/.openclaw/``.

    Points users at ``thm claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``thm claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to TeamHermes, run "
        "`thm claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `thm claw cleanup` (renames it to ~/.openclaw.pre-migration — "
