...
*after* the image is built (`th profile create coder` →
...
- Dynamic registration is limited to per-profile gateways — one service per
  ...
  profile so `th gateway start` (no `-p`) has somewhere to land.
...
1. `gosu` privilege drop from root → `th` UID.
...
4. Optionally backgrounded `th dashboard` in a subshell when
...
`th gateway start`, `coder gateway start` (profile alias), and
`th -p <profile> gateway start` all spawn a gateway process scoped to a
given profile. See
...
The contract "container exits when `th` exits" is preserved via a
...
|---|---|
| (no args) | `th` with no args, container exits when hermes exits |
| `chat -q "..."` | `th chat -q "..."`, container exits with th exit code |
...
(`th -p <profile> gateway run`, or `th gateway run` for the default
...
When `th profile create <name>` runs inside the container, the
profile-creation code path calls
`ServiceManager.register_profile_gateway(<name>)` if
`supports_runtime_registration()` is True. When `th profile delete
...
Existing per-profile `th -p <profile> gateway start/stop/restart` CLI
...
`th gateway start` (no `-p`) targets a special `gateway-default` slot
...
`--all` lifecycle (`th gateway stop --all`, `... restart --all`)
...
main-th service so the hermes CMD becomes that main program.
...
exec s6-setuidgid th hermes ${HERMES_ARGS:-}
```
...
| A `running` gateway that's actually broken auto-restarts into a crash loop after every container restart | Low | Medium | s6 `finish` script `max_restarts` cap (planned); follow-up: `th doctor` alerts when N consecutive container restarts ended in `startup_failed` |
...
- [x] `th profile create test` inside a container creates
      `/run/service/gateway-test/`
- [x] `th -p test gateway start` inside a container dispatches through s6
- [x] `th -p test gateway stop` inside a container cleanly stops via s6
- [x] `th profile delete test` inside a container removes
      `/run/service/gateway-test/`
- [x] Profile gateway logs persist at
      `$HERMES_HOME/logs/gateways/test/current`
- [x] `th status` inside the container shows `Manager: s6`
- [x] `th gateway start` (no `-p`) inside a container targets
      `gateway-default` and runs against the root profile
- [x] `th gateway stop --all` / `... restart --all` iterate every
...
- [x] `th gateway install/start/stop` on Linux host and macOS host
