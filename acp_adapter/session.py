"""ACP session manager — maps ACP sessions to TeamHermes AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.teamhermes/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when TeamHermes itself is running in WSL.

    Windows ACP clients can launch ``thm acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch TeamHermes from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["thm-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed TeamHermes agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by TeamHermes AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current TeamHermes runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.teamhermes/state.db``) is lazily created.
        """
