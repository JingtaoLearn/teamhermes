"""ACP auth helpers — detect and advertise TeamHermes authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active TeamHermes runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if TeamHermes can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for TeamHermes.
    ...
    not have TeamHermes provider credentials configured yet, so TeamHermes always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate TeamHermes using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure TeamHermes provider",
            description=(
                "Open TeamHermes' interactive model/provider setup in a terminal. "
                "Use this when TeamHermes has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
