...
def load_cli_config() -> Dict[str, Any]:
    ...
    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``thm chat --ignore-user-config``),
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && thm` to control it.
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``thm --help`` paths inside the chat command flow).  See
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run thm -w")
        return None
...
class HermesCLI:
    ...
    def __init__(
        ...
        # by `thm chat --ignore-rules` in hermes_cli/main.py. When true we
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: thm setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: thm setup")
            return False
        ...
        # `thm chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `thm auth add openai-codex`
        # without `thm model`), fall back to the provider's first catalog
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (thm sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (thm sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``thm doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``thm doctor`` so the banner stays
        small.
        """
        try:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: thm --checkpoints")
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run thm chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'thm setup' to configure[/]")
        except Exception:
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `thm gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `thm sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `thm sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        ...
        DB, so the live session remains resumable via ``thm --resume <id>``
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: thm --resume {self.session_id}")
        except Exception as e:
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `thm model` and gateway pickers).
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to hermes_cli.curator so the CLI and the `thm curator`
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``thm bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: thm bundles create "
        ...
        _cprint(
            ...
            f"Manage with `thm bundles`.{_RST}"
        )
    ...
    def _handle_update_command(self) -> bool:
        ...
        ``thm update`` so the user sees update output directly and gets
        ...
        raw = self._prompt_text_input_modal(
            ...
            detail="This will exit the current session and run `thm update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  thm --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  thm -c \"{session_title}\"{profile_flag}")
            print()
    ...
    def run(self):
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: thm setup"
            )
            _run_cleanup()
...
def main(
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``thm chat -q "work kanban task <id>"``;
        ...
        if quiet:
            ...
            # Single-query mode (`thm chat -q "…"`): skip the welcome
