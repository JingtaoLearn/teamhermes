...
th kanban watch --tenant <project-slug>

# Snapshot of the board
th kanban list --tenant <project-slug>
th kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
th kanban stats --tenant <project-slug>

# Visual dashboard (browser)
th dashboard

# Inspect a specific task (includes comments + events)
th kanban show <task-id>

# Follow a single task's event stream
th kanban tail <task-id>
```

Verify available subcommands with `th kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `th kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `th kanban block <id>` then `th kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `th kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `th kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `th profile list` (profile exists?), `th status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(th kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
th kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
th kanban block <task-id>
th kanban archive <task-id>

# Diagnose what happened
th kanban show <task-id>      # task body, comments, recent events
th kanban tail <task-id>      # follow the live event stream
th kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `th kanban create` for the director
...
while true; do
    clear
    th kanban list --tenant <slug>
    echo "---"
    th kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `th kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `th profile list` and start
