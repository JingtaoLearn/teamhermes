...
Maps each role archetype to the TeamHermes skills it should `always_load` and the
...
## TeamHermes skills relevant to video production
...
These are **not** TeamHermes skills but external CLIs / APIs that profiles invoke.
...
## Built-in TeamHermes tools for media review

These are native TeamHermes tools — not invoked via terminal but through their own
...
key is present in `~/.teamhermes/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every TeamHermes profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.teamhermes/.env` → environment variable.
...
When a new TeamHermes-public video skill ships:
