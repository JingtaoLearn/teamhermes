# Contributing to TeamHermes Agent

Thank you for contributing to TeamHermes Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want TeamHermes to work everywhere.
...
Bundled skills (in `skills/`) ship with every TeamHermes install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `thm skills browse` (labeled "official") and install it with `thm skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `thm skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.teamhermes/plugins/` (or via a pip entry point).
...
```bash
mkdir -p ~/.teamhermes/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.teamhermes/config.yaml
touch ~/.teamhermes/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.teamhermes/.env
```
...
│
├── skills/                   # Bundled skills (copied to ~/.teamhermes/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.teamhermes/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.teamhermes/`)

| Path | Purpose |
|------|---------|
| `~/.teamhermes/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.teamhermes/.env` | API keys and secrets |
| `~/.teamhermes/auth.json` | OAuth credentials (Nous Portal) |
| `~/.teamhermes/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.teamhermes/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.teamhermes/state.db` | SQLite session database |
| `~/.teamhermes/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.teamhermes/cron/` | Scheduled job data |
| `~/.teamhermes/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.teamhermes/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
The user may skip setup and keep loading the skill. TeamHermes only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `thm setup` or update `~/.teamhermes/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native TeamHermes tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "TeamHermes Agent" is the secondary collaborator. If the contributor's commit shows "TeamHermes Agent" as author because they used TeamHermes to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing TeamHermes tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `thm --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

TeamHermes uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.teamhermes/skins/<name>.yaml`:
...
TeamHermes runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
TeamHermes has terminal access. Security matters.
...
2. **Test manually**: Run `thm` and exercise the code path you changed
...
- Include: OS, Python version, TeamHermes version (`thm version`), full error traceback
